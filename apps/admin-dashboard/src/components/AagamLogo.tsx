...
export default function AagamLogo({
  ...
  return (
    <Link href={href} className="inline-flex min-w-0 items-center gap-3" aria-label="Aagam home">
      <span className={`flex ${compact ? 'h-10 w-10' : 'h-12 w-12'} shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-white p-1 shadow-xl ${inverse ? 'shadow-black/30' : 'border border-slate-100 shadow-slate-950/15'}`}>
        <Image src="/brand/aagam-mark.png" width={96} height={96} alt="Aagam" className="h-full w-full object-contain" priority />
      </span>
      <span className="min-w-0">
        <span className={`block font-black tracking-[-0.05em] ${compact ? 'text-xl' : 'text-2xl'} ${inverse ? 'text-white' : 'text-slate-950'}`}>Aagam</span>
