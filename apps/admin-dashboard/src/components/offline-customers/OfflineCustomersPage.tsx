      ...
      <>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 ">
          <div className="flex items-start justify-between">
            <div>
              <h2 className="text-lg font-semibold text-slate-900">{selectedCustomer.name || 'Unnamed Customer'}</h2>
              ...
              {viewMode === 'recycleBin' && canManage ? (
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 rounded-xl bg-rose-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-rose-700 disabled:opacity-50"
                  >
          ...
          {selectedCustomer.addresses[0] && (
            <div className="mt-3 rounded-xl bg-slate-50 p-3">
              <div className="text-[10px] font-semibold uppercase tracking-wider text-slate-500">Default Address</div>
          ...
          <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
            <div className="rounded-xl bg-emerald-50 p-3 text-center">
              <div className="text-lg font-semibold text-emerald-700">{selectedCustomer.summary.activeSubscriptions}</div>
              <div className="text-[10px] font-bold text-emerald-600">Active Subs</div>
            </div>
            <div className="rounded-xl bg-blue-50 p-3 text-center">
              <div className="text-lg font-semibold text-blue-700">{selectedCustomer.summary.totalOrders}</div>
              <div className="text-[10px] font-bold text-blue-600">Total Orders</div>
            </div>
            <div className="rounded-xl bg-amber-50 p-3 text-center">
              <div className="text-lg font-semibold text-amber-700">{formatPaise(selectedCustomer.summary.totalCollectedPaise)}</div>
              <div className="text-[10px] font-bold text-amber-600">Collected</div>
            </div>
            <div className="rounded-xl bg-red-50 p-3 text-center">
              <div className="text-lg font-semibold text-red-700">{formatPaise(selectedCustomer.summary.totalDuePaise)}</div>
              ...
              <div key={sub.id} className="rounded-xl border border-slate-200 bg-white p-4 ">
                <div className="flex items-start justify-between">
                  <div>
                    <div className="flex items-center gap-1.5">
                      ...
                      {sub.isCustom && <span className="inline-flex items-center rounded-lg bg-purple-100 px-1.5 py-0.5 text-[10px] font-semibold text-purple-700">Custom</span>}
                      {sub.storeDelivery && <span className="inline-flex items-center rounded-lg bg-orange-100 px-1.5 py-0.5 text-[10px] font-semibold text-orange-700">Store Delivery</span>}
                    </div>
                  ...
                  <button onClick={() => openTracker(sub.id)} className="inline-flex items-center gap-1 rounded-lg bg-emerald-50 px-2.5 py-1.5 text-[10px] font-semibold text-emerald-700 hover:bg-emerald-100">
                ...
                <div className="mt-3 grid grid-cols-3 gap-2 sm:grid-cols-6">
                  <div className="text-center">
                    <div className="text-sm font-semibold text-slate-900">{sub._count.deliveries}</div>
                    <div className="text-[10px] font-bold text-slate-400">Total</div>
                  </div>
                  <div className="text-center">
                    <div className="text-sm font-semibold text-emerald-700">{sub.completedDeliveries}</div>
                    <div className="text-[10px] font-bold text-slate-400">Delivered</div>
                  </div>
                  <div className="text-center">
                    <div className="text-sm font-semibold text-amber-700">{sub._count.deliveries - sub.completedDeliveries - sub.failedDeliveries - sub.skippedDeliveries}</div>
                    <div className="text-[10px] font-bold text-slate-400">Pending</div>
                  </div>
                  <div className="text-center">
                    <div className="text-sm font-semibold text-red-700">{sub.failedDeliveries}</div>
                    <div className="text-[10px] font-bold text-slate-400">Failed</div>
                  </div>
                  <div className="text-center">
                    <div className="text-sm font-semibold text-blue-700">{formatPaise(sub.amountCollectedPaise)}</div>
                    <div className="text-[10px] font-bold text-slate-400">Collected</div>
                  </div>
                  <div className="text-center">
                    <div className="text-sm font-semibold text-red-700">{formatPaise(sub.amountDuePaise)}</div>
    ...
    <>
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <button onClick={handleSearch} className="rounded-xl bg-emerald-700 px-4 py-2.5 text-sm font-semibold text-white hover:bg-emerald-800">Search</button>
          {viewMode === 'active' && canManage && (
            <button onClick={() => setCreateModalOpen(true)} className="inline-flex items-center gap-1.5 rounded-xl bg-amber-400 px-4 py-2.5 text-sm font-semibold text-slate-900 hover:bg-amber-300">
        ...
        <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center">
          {viewMode === 'recycleBin' ? (
            <>
              ...
              <p className="mt-3 text-sm font-semibold text-slate-400">Recycle Bin is empty</p>
            ...
            <>
              ...
              <p className="mt-3 text-sm font-semibold text-slate-400">No offline customers found</p>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {customers.map((c) => (
              <div
                ...
                className="group relative cursor-pointer rounded-xl border border-slate-200 bg-white p-4 text-left  transition-all hover:border-emerald-300 hover:shadow-md"
              >
                <div className="flex items-start justify-between">
                  <div>
                    <h3 className="text-sm font-semibold text-slate-900 group-hover:text-emerald-700">{c.name || 'Unnamed'}</h3>
                  ...
                  <div className="flex items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
                    {viewMode === 'recycleBin' ? (
                      ...
                      <span className="inline-flex items-center gap-1 rounded-lg bg-emerald-100 px-2 py-1 text-xs font-semibold text-emerald-700">
                ...
                <div className="mt-2 grid grid-cols-3 gap-1.5 text-center">
                  <div>
                    <div className="text-xs font-semibold text-slate-900">{c.summary.totalSubscriptions}</div>
                    <div className="text-[10px] font-bold text-slate-400">Subs</div>
                  </div>
                  <div>
                    <div className="text-xs font-semibold text-slate-900">{c.summary.totalOrders}</div>
                    <div className="text-[10px] font-bold text-slate-400">Orders</div>
                  </div>
                  <div>
                    <div className="text-xs font-semibold text-emerald-700">{formatPaise(c.summary.totalCollectedPaise)}</div>
                    <div className="text-[10px] font-bold text-slate-400">Collected</div>
                  </div>
                </div>
                ...
                {viewMode === 'recycleBin' && canManage && (
                  <div className="mt-3 flex items-center gap-2 border-t border-slate-100 pt-2.5" onClick={(e) => e.stopPropagation()}>
                    <button
                      ...
                      className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-lg bg-emerald-50 px-2.5 py-1.5 text-xs font-semibold text-emerald-700 hover:bg-emerald-100 transition-colors disabled:opacity-50"
                    >
                      <RotateCcw className="h-3.5 w-3.5" /> Restore
                    </button>
                    <button
                      ...
                      className="inline-flex items-center justify-center gap-1 rounded-lg bg-rose-50 px-2.5 py-1.5 text-xs font-semibold text-rose-700 hover:bg-rose-100 transition-colors disabled:opacity-50"
                    >
                      <Trash2 className="h-3.5 w-3.5" /> Delete Forever
                    </button>
                  </div>
      ...
      {createModalOpen && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/50 p-0  sm:items-center sm:p-4" role="dialog" aria-modal="true" aria-label="Create offline customer">
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-t-2xl bg-white  sm:rounded-xl">
            <div className="flex items-center justify-between border-b border-slate-200 p-4">
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-wider text-amber-700">Offline Customer</p>
                <h2 className="mt-0.5 text-lg font-semibold text-slate-900">Create New Customer</h2>
              </div>
            ...
            <div className="space-y-3 p-4">
              ...
              <div className="rounded-xl border border-dashed border-emerald-300 bg-white p-2">
                <div className="mb-1 text-[10px] font-semibold uppercase tracking-wider text-emerald-600">Delivery Location (Map)</div>
            ...
            <div className="flex justify-end gap-2 border-t border-slate-200 p-4">
              <button onClick={() => setCreateModalOpen(false)} className="min-h-10 rounded-xl border border-slate-200 px-4 text-xs font-semibold">Cancel</button>
              <button disabled={creating} onClick={() => void handleCreateCustomer()} className="inline-flex min-h-10 items-center gap-1.5 rounded-xl bg-amber-500 px-4 text-xs font-semibold text-slate-950 disabled:opacity-50">
      ...
      {deleteModalOpen && customerToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4 -xs" role="dialog" aria-modal="true">
          <div className="w-full max-w-md rounded-xl bg-white p-6 ">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-semibold text-slate-900">Move to Recycle Bin?</h3>
            ...
            <div className="mt-6 flex gap-3">
              <button
                ...
                className="flex-1 rounded-xl bg-slate-100 px-4 py-2.5 text-xs font-semibold text-slate-700 hover:bg-slate-200 disabled:opacity-50"
              >
                Cancel
              </button>
              <button
                ...
                className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-xl bg-rose-600 px-4 py-2.5 text-xs font-semibold text-white hover:bg-rose-700 disabled:opacity-50 "
              >
      ...
      {purgeModalOpen && customerToPurge && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4 -xs" role="dialog" aria-modal="true">
          <div className="w-full max-w-md rounded-xl bg-white p-6  border border-rose-200">
            <div className="flex items-center gap-3">
              <div className="rounded-xl bg-rose-100 p-3 text-rose-700">
                <AlertTriangle className="h-6 w-6" />
              </div>
              <div>
                <h3 className="text-lg font-semibold text-rose-950">Delete Permanently?</h3>
            ...
            <div className="mt-6 flex gap-3">
              ...
              <button
                ...
                className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-xl bg-rose-700 px-4 py-2.5 text-xs font-semibold text-white hover:bg-rose-800 disabled:opacity-50 "
              >
                {actionInProgress ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}
                Delete Forever
              </button>
