  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-6 right-6 z-40 inline-flex min-h-12 items-center gap-2 rounded-xl bg-emerald-800 px-4 font-semibold text-white  hover:bg-emerald-900"
      ...
      {open ? (
        <div className="fixed inset-0 z-[80] bg-slate-950/50 " role="dialog" aria-modal="true" aria-label="Tomorrow subscription preparation">
          <div className="absolute inset-y-0 right-0 flex w-full max-w-xl flex-col bg-slate-50 ">
            <header className="border-b border-slate-200 bg-emerald-950 p-5 text-white">
              <div className="flex items-start justify-between gap-4">
                <div><p className="text-xs font-semibold uppercase tracking-[.2em] text-emerald-300">D-1 stock readiness</p><h2 className="mt-2 text-2xl font-semibold">Prepare before delivery day</h2><p className="mt-2 text-sm font-semibold text-emerald-100">Confirm forecast stock now. Inventory is deducted only when the real subscription order is generated.</p></div>
            ...
            <div className="flex-1 space-y-4 overflow-y-auto p-5">
              {rows.length ? rows.map((row) => (
                <article key={row.id} className={`rounded-xl border bg-white p-5  ${row.readiness.status === 'SHORTAGE' ? 'border-red-300' : 'border-slate-200'}`}>
                  <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wide text-emerald-700">{dateLabel(row.serviceDate)}</p><h3 className="mt-1 text-lg font-semibold text-slate-950">{row.plan.name}</h3><p className="mt-1 text-xs font-bold text-slate-500">{humanize(row.deliveryStatus)} · {row.inventoryReservation === 'RESERVED_BY_ORDER' ? 'Inventory reserved' : 'Forecast only'}</p></div><span className={`rounded-full px-3 py-1 text-xs font-semibold ${row.readiness.status === 'READY' ? 'bg-emerald-100 text-emerald-800' : row.readiness.status === 'SHORTAGE' ? 'bg-red-100 text-red-800' : 'bg-amber-100 text-amber-800'}`}>{humanize(row.readiness.status)}</span></div>

                  <div className="mt-4 rounded-xl bg-slate-50 p-4"><p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-slate-500"><Box className="h-4 w-4" /> Required items</p><div className="mt-2 flex flex-wrap gap-2">{row.items.map((item) => <span key={item.productId} className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-slate-800 ring-1 ring-slate-200">{item.quantity}× {item.name}</span>)}</div></div>

                  <div className="mt-3 grid gap-2 sm:grid-cols-2"><div className="rounded-xl border border-slate-200 p-3"><p className="flex items-center gap-2 text-xs font-semibold text-slate-500"><Phone className="h-4 w-4" /> RECIPIENT</p><p className="mt-2 font-semibold text-slate-900">{row.address.recipientName || row.customer.name || 'Customer'}</p><p className="mt-1 text-sm font-bold text-slate-600">{row.customer.deliveryPhone || row.address.phone || 'Phone unavailable'}</p></div><div className="rounded-xl border border-slate-200 p-3"><p className="flex items-center gap-2 text-xs font-semibold text-slate-500"><MapPin className="h-4 w-4" /> DELIVERY</p><p className="mt-2 text-sm font-bold leading-5 text-slate-700">{row.address.formattedAddress || 'Address unavailable'}</p></div></div>

                  <div className="mt-3 flex items-center gap-3 rounded-xl bg-slate-950 p-3 text-white"><Route className="h-5 w-5 text-emerald-300" /><div className="min-w-0 flex-1"><p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Operational state</p><p className="mt-1 truncate text-sm font-semibold">{row.run?.routeCode || (row.generatedAt ? 'Route planning pending' : 'Order generation pending')}</p><p className="mt-1 text-xs text-slate-300">{row.run?.rider?.user?.name ? `Final rider: ${row.run.rider.user.name}` : 'Rider is finalized close to the delivery slot after live eligibility checks.'}</p></div></div>

                  {row.readiness.status === 'SHORTAGE' ? <div className="mt-3 flex items-start gap-2 rounded-xl bg-red-50 p-3 text-sm font-bold text-red-800"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" /> {row.readiness.note || 'Shortage reported'}</div> : null}
                  ...
                  <div className="mt-4 space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2"><button disabled={working.startsWith(row.id)} onClick={() => void decide(row, 'READY')} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-emerald-700 text-sm font-semibold text-white disabled:opacity-50"><PackageCheck className="h-4 w-4" /> Stock ready</button><button disabled={working.startsWith(row.id)} onClick={() => void decide(row, 'SHORTAGE')} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-red-50 text-sm font-semibold text-red-700 disabled:opacity-50"><AlertTriangle className="h-4 w-4" /> Report shortage</button></div>
                  </div>
