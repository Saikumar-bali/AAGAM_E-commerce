  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-6 right-6 z-40 inline-flex min-h-12 items-center gap-2 rounded-xl bg-slate-950 px-4 font-semibold text-white  ring-1 ring-white/10 hover:bg-emerald-900"
      ...
      {open ? (
        <div className="fixed inset-0 z-[80] bg-slate-950/50 " role="dialog" aria-modal="true" aria-label="Tomorrow subscription operations">
          <div className="absolute inset-y-0 right-0 flex w-full max-w-2xl flex-col bg-slate-50 ">
            <header className="border-b border-slate-200 bg-white p-5">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[.2em] text-emerald-700">D-1 operational control</p>
                  <h2 className="mt-2 text-2xl font-semibold text-slate-950">Tomorrow subscriptions</h2>
            ...
            <div className="flex-1 space-y-4 overflow-y-auto p-5">
              <section className={`rounded-xl border p-4 ${belowPolicy.length ? 'border-amber-200 bg-amber-50' : 'border-slate-200 bg-white'}`}>
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className={`font-semibold ${belowPolicy.length ? 'text-amber-950' : 'text-slate-950'}`}>Order materialization lead</p>
                ...
                <div className="mt-3 space-y-2">
                  {overview.plans.map((plan) => (
                    <div key={plan.id} className="grid gap-3 rounded-xl bg-white p-3 ring-1 ring-slate-200 sm:grid-cols-[1fr_100px_auto] sm:items-center">
                      <div><p className="font-semibold text-slate-900">{plan.name}</p><p className="text-xs font-semibold text-slate-500">Current {plan.orderGenerationHoursBefore}h · {humanize(plan.status)}</p></div>
                      <label className="text-xs font-semibold text-slate-600">Hours<input aria-label={`${plan.name} order generation lead hours`} type="number" min={overview.minimumPreparationHours} max={72} step={1} value={planHours[plan.id] ?? String(plan.orderGenerationHoursBefore)} onChange={(event) => setPlanHours((current) => ({ ...current, [plan.id]: event.target.value }))} className="mt-1 min-h-10 w-full rounded-lg border border-slate-200 px-3 text-sm font-semibold text-slate-900" /></label>
                      <button disabled={workingPlan === plan.id || Number(planHours[plan.id] ?? plan.orderGenerationHoursBefore) === plan.orderGenerationHoursBefore} onClick={() => void savePolicy(plan)} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl bg-emerald-700 px-3 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-500"><Save className="h-4 w-4" /> Save</button>
                    </div>
                  ))}
              ...
              {overview.rows.length ? overview.rows.map((row) => (
                <article key={row.id} className="rounded-xl border border-slate-200 bg-white p-5 ">
                  <div className="flex items-start justify-between gap-4">
                    <div><p className="text-xs font-semibold uppercase tracking-wider text-emerald-700">{dayLabel(row.serviceDate)}</p><h3 className="mt-1 text-lg font-semibold text-slate-950">{row.plan.name}</h3><p className="mt-1 text-sm font-semibold text-slate-500">{row.store?.name || 'Store unresolved'} · {humanize(row.deliveryStatus)}</p></div>
                    <span className={`rounded-full px-3 py-1 text-xs font-semibold ${row.readiness.status === 'READY' ? 'bg-emerald-100 text-emerald-800' : row.readiness.status === 'SHORTAGE' ? 'bg-red-100 text-red-800' : 'bg-amber-100 text-amber-800'}`}>{humanize(row.readiness.status)}</span>
                  </div>

                  <div className="mt-4 grid gap-3 sm:grid-cols-2">
                    <div className="rounded-xl bg-slate-50 p-4"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-slate-500"><Phone className="h-4 w-4" /> Delivery contact</div><p className="mt-2 font-semibold text-slate-900">{row.address.recipientName || row.customer.name || 'Customer'}</p><p className="mt-1 text-sm font-bold text-slate-700">{row.customer.deliveryPhone || row.address.phone || 'No delivery phone'}</p>{row.address.alternatePhone ? <p className="mt-1 text-xs text-slate-500">Alternate: {row.address.alternatePhone}</p> : null}</div>
                    <div className="rounded-xl bg-slate-50 p-4"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-slate-500"><MapPin className="h-4 w-4" /> Delivery address</div><p className="mt-2 text-sm font-bold leading-6 text-slate-800">{row.address.formattedAddress || 'Address unavailable'}</p></div>
                  </div>

                  <div className="mt-3 rounded-xl border border-slate-200 p-4"><div className="flex items-center justify-between"><span className="inline-flex items-center gap-2 text-sm font-semibold text-slate-800"><Store className="h-4 w-4 text-emerald-700" /> Preparation</span><span className="text-xs font-semibold text-slate-500">{row.inventoryReservation === 'RESERVED_BY_ORDER' ? 'Inventory reserved' : 'Forecast only'}</span></div><div className="mt-2 flex flex-wrap gap-2">{row.items.map((item) => <span key={item.productId} className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-800">{item.quantity}× {item.name}</span>)}</div>{row.readiness.note ? <p className="mt-2 text-xs font-semibold text-slate-500">Store note: {row.readiness.note}</p> : null}</div>

                  <div className="mt-3 flex items-center gap-3 rounded-xl bg-slate-950 p-4 text-white"><Route className="h-5 w-5 text-emerald-300" /><div className="min-w-0 flex-1"><p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Route / rider</p><p className="mt-1 truncate font-semibold">{row.run?.routeCode || (row.generatedAt ? 'Route planning pending' : 'Order generation pending')}</p><p className="mt-1 text-xs text-slate-300">{row.run?.rider?.user?.name ? `Final rider: ${row.run.rider.user.name}` : `Live rider assignment waits until ~${overview.finalAssignmentHoursBefore}h before the slot.`}</p></div></div>
                </article>
