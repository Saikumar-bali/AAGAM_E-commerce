...
function isLoginPhoneInput(target: EventTarget | null): target is HTMLInputElement {
  ...
  return target.inputMode === 'tel' && (hint.includes('mobile') || hint.includes('phone'));
}
...
export default function TenDigitPhoneGuard() {
  useEffect(() => {
    const configureVisibleInputs = () => {
      ...
      document.querySelectorAll<HTMLInputElement>('input[inputmode="tel"], input[autocomplete="tel"]').forEach((input) => {
