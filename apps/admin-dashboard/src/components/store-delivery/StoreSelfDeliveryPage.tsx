    ...
    <DashboardLayout allowedRole="ADMIN">
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-12 text-center">
          ...
          <p className="mt-4 text-lg font-semibold text-slate-400">Select a store to view deliveries</p>
        </div>
      ) : deliveries.length === 0 ? (
        <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-12 text-center">
          <CheckCircle2 className="mx-auto h-12 w-12 text-slate-300" />
          <p className="mt-4 text-lg font-semibold text-slate-400">No store deliveries today</p>
        ...
        <div className="space-y-8">
          {grouped.inProgress.length > 0 && (
            <section>
              <h3 className="mb-4 flex items-center gap-2 text-lg font-semibold text-orange-700">
      ...
      {verifyModal && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/50 p-0  sm:items-center sm:p-5" role="dialog" aria-modal="true">
          <div className="max-h-[96vh] w-full max-w-lg overflow-y-auto rounded-t-[28px] bg-white  sm:rounded-[28px]">
            <div className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-200 bg-white p-5">
              <div>
                <p className="text-xs font-semibold uppercase tracking-wider text-emerald-700">Verify Customer</p>
                <h2 className="mt-1 text-xl font-semibold text-slate-900">Confirm Delivery Handoff</h2>
              </div>
            ...
            <div className="space-y-4 p-5">
              <div className="rounded-xl bg-slate-50 p-4">
                <div className="text-xs font-semibold uppercase tracking-wider text-slate-500">Customer Info</div>
              ...
              <div className="space-y-3">
                <label className="block text-sm font-semibold text-slate-700">Verified Customer Name</label>
              ...
              <div className="space-y-3">
                <label className="block text-sm font-semibold text-slate-700">Verified Phone (last 4 digits)</label>
              ...
              <div className="space-y-3">
                <label className="block text-sm font-semibold text-slate-700">Cash Collected (₹)</label>
            ...
            <div className="sticky bottom-0 flex justify-end gap-3 border-t border-slate-200 bg-white p-5">
              <button onClick={() => setVerifyModal(null)} className="min-h-12 rounded-xl border border-slate-200 px-5 font-semibold">Cancel</button>
              <button disabled={!verifyName.trim() || !verifyPhone.trim() || processingId === verifyModal.id} onClick={completeDelivery} className="inline-flex min-h-12 min-w-40 items-center justify-center gap-2 rounded-xl bg-emerald-700 px-5 font-semibold text-white disabled:opacity-50">
    ...
    <div className={`rounded-xl border bg-white p-5  ${
      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-2">
            <span className="font-semibold text-slate-900">{delivery.customer.name}</span>
      ...
      {delivery.order && (
        <div className="mt-3 rounded-xl bg-slate-50 p-3">
          <div className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Order Items</div>
          ...
          <div className="mt-2 border-t border-slate-200 pt-2 text-right text-sm font-semibold text-slate-900">
      ...
      <div className="mt-4 flex items-center gap-2">
        ...
        {phase === 'scheduled' && (
          <button disabled={processing} onClick={onStart} className="flex-1 rounded-xl bg-blue-600 px-3 py-2 text-xs font-semibold text-white hover:bg-blue-700 disabled:opacity-50">
        ...
        {phase === 'progress' && (
          <>
            <button disabled={processing} onClick={onVerify} className="flex-1 rounded-xl bg-emerald-600 px-3 py-2 text-xs font-semibold text-white hover:bg-emerald-700 disabled:opacity-50">
        ...
        {phase === 'failed' && (
          <button disabled={processing} onClick={onStart} className="flex-1 rounded-xl bg-amber-600 px-3 py-2 text-xs font-semibold text-white hover:bg-amber-700 disabled:opacity-50">
            Retry
          </button>
        )}
      </div>
