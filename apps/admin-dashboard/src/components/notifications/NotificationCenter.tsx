...
function roleFallback(role: Role, item: InboxItem) {
  ...
  if (role === "RIDER")
    return item.deliveryJobId
      ? `/rider/delivery?job=${encodeURIComponent(item.deliveryJobId)}`
      : "/rider/notifications";
...
export default function NotificationCenter({
  ...
  return (
    <div className="space-y-3 sm:space-y-5">
      <section className="flex flex-col gap-4 rounded-3xl border border-slate-800 bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 p-4 text-white shadow-xl sm:p-6 lg:flex-row lg:items-center lg:justify-between">
        <div>
        ...
        <div className="grid w-full grid-cols-2 gap-2 [&>*]:justify-center lg:w-auto lg:flex lg:flex-wrap">
          {rightSlot}
          {pushSupported && (
            <button
              type="button"
              ...
              className={`col-span-2 inline-flex items-center gap-2 rounded-xl px-3 py-2.5 text-xs font-black sm:text-sm lg:col-span-1 ${
                pushEnabled
          ...
          <button
            ...
            onClick={fetchInbox}
            disabled={loading}
            className="inline-flex items-center gap-2 rounded-xl bg-white px-3 py-2.5 text-xs font-black text-slate-950 disabled:opacity-60 sm:text-sm"
          >
      ...
      <section className="grid grid-cols-2 gap-3 xl:grid-cols-4">
        <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:rounded-3xl sm:p-5">
          ...
          <p className="mt-2 text-2xl font-black text-indigo-700 sm:text-3xl">
            {unreadCount}
          </p>
        </article>
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-3 shadow-sm sm:rounded-[2rem] sm:p-6">
        <div className="mb-4 flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 hidden text-sm font-semibold text-slate-500 sm:block">
              Read state and delivery attempts are stored separately from order
              history.
            </p>
          </div>
        ...
        {loading ? (
          ...
          <div className="rounded-3xl border border-dashed border-slate-200 p-8 text-center sm:p-12">
          ...
          <div className="space-y-3">
            {items.map((item) => (
              <article
                key={item.id}
                className={`group rounded-2xl border p-3 transition hover:-translate-y-0.5 hover:shadow-md sm:rounded-3xl sm:p-4 ${
                  item.readAt
