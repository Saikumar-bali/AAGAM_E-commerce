...
function roleFallback(role: Role, item: InboxItem) {
  if (role === 'ADMIN') return item.orderId ? `/admin/orders/${item.orderId}` : '/admin/notifications';
...
export default function NotificationCenter({
  ...
  const fetchInbox = useCallback(async () => {
    ...
    try {
      ...
      const preferences = Array.isArray(preferenceResponse.data) ? preferenceResponse.data : [];
      const globalPreference = preferences.find((preference: any) => preference.eventType === '*');
  ...
  const markRead = async (item: InboxItem, navigate = false) => {
    try {
      ...
      if (item.recipientId) {
        await apiClient.patch(`/notifications/${item.recipientId}/opened`).catch(() => undefined);
      }
      setItems((current) => current.map((entry) => (
        entry.id === item.id ? { ...entry, readAt: entry.readAt || new Date().toISOString(), status: 'READ' } : entry
      )));
      setUnreadCount((current) => Math.max(0, current - (item.readAt ? 0 : 1)));
      if (navigate) router.push(item.deepLink || roleFallback(role, item));
    } catch (error: any) {
      setMessage(error?.response?.data?.message || 'Could not update the notification.');
    }
  };
  ...
  const enablePush = async () => {
    ...
    try {
      ...
      setMessage(result.enabled ? 'Background notifications are enabled on this device.' : result.reason || 'Push setup failed.');
    } catch (error: any) {
      setMessage(error?.response?.data?.message || error?.message || 'Push setup failed.');
    } finally {
  ...
  const toggleGlobalPush = async () => {
    ...
    try {
      ...
      setMessage(next ? 'Push delivery preference enabled.' : 'Push delivery paused. In-app notifications remain available.');
    } catch (error: any) {
      setGlobalPush(!next);
      setMessage(error?.response?.data?.message || 'Could not update notification preferences.');
    }
  };

  const recentCount = useMemo(
    () => items.filter((item) => Date.now() - new Date(item.createdAt).getTime() < 24 * 60 * 60 * 1000).length,
    [items],
  );
  ...
  return (
    <div className="space-y-5">
      <section className="flex flex-col gap-5 rounded-[2rem] border border-slate-800 bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 p-6 text-white shadow-xl lg:flex-row lg:items-center lg:justify-between">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.22em] text-indigo-300">Communication centre</p>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button
            ...
            className={`inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-black ${
              pushEnabled ? 'bg-emerald-100 text-emerald-800' : 'bg-indigo-500 text-white hover:bg-indigo-400'
            } disabled:cursor-default`}
          >
            {pushLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : pushEnabled ? <CheckCircle2 className="h-4 w-4" /> : <BellRing className="h-4 w-4" />}
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-xs font-black uppercase tracking-wide text-slate-400">Unread</p>
          <p className="mt-2 text-3xl font-black text-indigo-700">{unreadCount}</p>
        </article>
        <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-xs font-black uppercase tracking-wide text-slate-400">Last 24 hours</p>
          <p className="mt-2 text-3xl font-black text-slate-950">{recentCount}</p>
        </article>
        <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-xs font-black uppercase tracking-wide text-slate-400">Device delivery</p>
          <p className={`mt-2 text-lg font-black ${pushEnabled ? 'text-emerald-700' : 'text-amber-700'}`}>{pushEnabled ? 'Enabled' : 'Not enabled'}</p>
        </article>
        <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-xs font-black uppercase tracking-wide text-slate-400">Push preference</p>
              <p className="mt-2 text-lg font-black text-slate-950">{globalPush ? 'On' : 'Paused'}</p>
            </div>
            <button
              type="button"
              onClick={toggleGlobalPush}
              aria-label="Toggle push preference"
              className={`relative h-7 w-12 rounded-full transition ${globalPush ? 'bg-emerald-500' : 'bg-slate-300'}`}
            >
              <span className={`absolute top-1 h-5 w-5 rounded-full bg-white shadow transition ${globalPush ? 'left-6' : 'left-1'}`} />
            </button>
          </div>
        </article>
      </section>
      ...
      <section className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
        <div className="mb-4 flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm font-semibold text-slate-500">Read state and delivery attempts are stored separately from order history.</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="rounded-3xl border border-dashed border-slate-200 p-12 text-center">
            ...
            <p className="mt-4 text-lg font-black text-slate-950">No notifications yet</p>
            <p className="mt-1 text-sm font-semibold text-slate-500">New order and delivery events will appear here.</p>
          </div>
        ) : (
            ...
            {items.map((item) => (
              <article
                ...
                className={`group rounded-3xl border p-4 transition hover:-translate-y-0.5 hover:shadow-md ${
                  item.readAt ? 'border-slate-200 bg-slate-50/70' : 'border-indigo-200 bg-indigo-50/55'
                }`}
              >
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  <button type="button" onClick={() => markRead(item, true)} className="min-w-0 flex-1 text-left">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className={`rounded-full px-2.5 py-1 text-[10px] font-black uppercase tracking-wide ${
                        item.readAt ? 'bg-slate-200 text-slate-600' : 'bg-indigo-600 text-white'
                      }`}>
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-3 text-xs font-bold text-slate-400">
                      <span className="inline-flex items-center gap-1"><Clock3 className="h-3.5 w-3.5" /> {new Date(item.createdAt).toLocaleString('en-IN')}</span>
                      {item.orderId && <span>Order #{item.orderId.slice(-8).toUpperCase()}</span>}
                    </div>
                  </button>
