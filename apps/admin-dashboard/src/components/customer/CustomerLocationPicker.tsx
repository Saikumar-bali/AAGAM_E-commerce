...
export default function CustomerLocationPicker({ latitude, longitude, onChange, fullHeight }: Props) {
  ...
  const handleSearch = useCallback(async (query: string) => {
    ...
    try {
      ...
      // Bounding box strictly around Anakapalle and surrounding villages
      const bbox = '82.85,17.55,83.15,17.85';
  ...
  return (
    <div className={fullHeight ? 'flex h-full flex-col overflow-hidden bg-white' : 'overflow-hidden rounded-xl border border-teal-200 bg-white'}>
      <div className="relative">
        ...
        {searchResults.length > 0 && (
          <div className="absolute left-0 right-0 top-full z-50 max-h-56 overflow-y-auto border-b border-slate-100 bg-white  divide-y divide-slate-100">
            {searchResults.map((result, i) => (
              ...
              >
                <span className={`mt-0.5 shrink-0 rounded-md border px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wider ${getBadgeStyle(result.type)}`}>
                  {result.type}
                </span>
