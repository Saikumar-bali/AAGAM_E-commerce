...
import { Heart, Plus, Minus, Clock, Zap } from 'lucide-react';
...
export default function ProductCard({ product, qty, onAdd, onIncrement, onDecrement, wished, onToggleWish }: ProductCardProps) {
  ...
  return (
    <div className="group relative flex flex-col rounded-2xl border border-slate-100 bg-white shadow-[0_2px_12px_rgba(15,23,42,0.04)] overflow-hidden transition-all hover:shadow-[0_8px_30px_rgba(15,23,42,0.08)] hover:-translate-y-0.5">
        ...
        <div className="relative aspect-[4/3] bg-gradient-to-br from-teal-50 to-amber-50 overflow-hidden">
          ...
          {!inStock && (
            <div className="absolute inset-0 bg-white/70 backdrop-blur-[1px] flex items-center justify-center">
              <span className="rounded-full bg-red-100 px-3 py-1 text-xs font-black text-red-700 border border-red-200">Out of stock</span>
            </div>
          )}
      ...
      <div className="flex flex-1 flex-col p-3">
        <Link href={`/shop/products/${product.id}`} className="block">
          <h3 className="text-[13px] font-extrabold text-slate-950 leading-snug line-clamp-2 min-h-[2.5rem]">
            {product.name}
          </h3>
        </Link>
        ...
        <div className="mt-auto pt-3 flex items-end justify-between gap-2">
          <div>
            <div className="text-base font-black text-slate-950">{formatINR(price)}</div>
          </div>
          ...
          <div className="flex items-center gap-1.5">
            ...
            {qty > 0 ? (
              ...
              <button
                onClick={(e) => { e.preventDefault(); if (inStock) onAdd(); }}
                disabled={!inStock}
