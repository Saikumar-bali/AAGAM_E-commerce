    ...
    <div className="rounded-xl border border-slate-100 bg-white overflow-hidden">
      ...
      <div className="p-5">
        {showDeliveryOffer ? <div className={`mb-4 rounded-xl border px-3 py-3 ${firstOrderFreeDelivery || freeDeliveryUnlocked ? 'border-emerald-200 bg-emerald-50' : 'border-amber-200 bg-amber-50'}`}>
          <p className={`text-xs font-semibold ${firstOrderFreeDelivery || freeDeliveryUnlocked ? 'text-emerald-800' : 'text-amber-900'}`}>{firstOrderFreeDelivery ? 'First order — FREE delivery' : freeDeliveryUnlocked ? 'Free delivery unlocked' : `Add ${formatINR(freeDeliveryRemainingPaise / 100)} for free delivery`}</p>
        ...
        <div className="space-y-2.5">
          {items.map((it, i) => (
            <div key={i} className="flex items-start justify-between gap-3 text-sm">
              ...
              <div className="font-semibold text-slate-950 shrink-0">{formatINR(it.lineTotal)}</div>
            </div>
          ))}
        </div>
        ...
        <div className="space-y-2 text-sm">
          ...
          <div className="flex items-center justify-between">
            <span className="flex items-center gap-1.5 text-slate-500 font-semibold">
              ...
              {firstOrderFreeDelivery ? 'Delivery fee · First order free' : 'Delivery fee'}
            </span>
            <span className={`font-semibold ${deliveryFee === 0 ? 'text-teal-600' : 'text-slate-950'}`}>
          ...
          {discountAmount > 0 && (
            <div className="flex items-center justify-between">
              ...
              <span className="font-semibold text-teal-600">-{formatINR(discountAmount)}</span>
            </div>
          )}
          {taxAmount > 0 && (
            <div className="flex items-center justify-between">
        ...
        <div className="flex items-center justify-between rounded-xl bg-slate-950 px-4 py-3">
          ...
          <span className="text-xl font-semibold text-white">{formatINR(grandTotal)}</span>
        </div>
      </div>
    </div>
  );
