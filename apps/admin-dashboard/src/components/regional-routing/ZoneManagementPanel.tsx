    ...
    {open ? <div className="fixed inset-0 z-[60] flex items-end justify-center bg-slate-950/60 p-0 sm:items-center sm:p-5"><div className="max-h-[96vh] w-full max-w-7xl overflow-hidden rounded-t-3xl bg-slate-50  sm:rounded-xl"><header className="flex items-center justify-between border-b border-slate-200 bg-white p-5"><div><p className="text-xs font-semibold uppercase tracking-wider text-emerald-700">Geographic service policy</p><h2 className="mt-1 text-2xl font-semibold text-slate-950">Delivery zones</h2><p className="mt-1 text-sm text-slate-500">Polygon first, radius fallback. Historical order snapshots remain unchanged.</p></div><button onClick={() => setOpen(false)} className="grid h-11 w-11 place-items-center rounded-xl bg-slate-100"><X className="h-5 w-5"/></button></header>
      <div className="grid max-h-[calc(96vh-96px)] overflow-y-auto lg:grid-cols-[320px_1fr]"><aside className="border-b border-slate-200 bg-white p-4 lg:border-b-0 lg:border-r"><button onClick={create} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-emerald-700 font-semibold text-white"><Plus className="h-5 w-5"/>New zone</button><div className="mt-4 space-y-2">{zones.map((zone) => <button key={zone.id} onClick={() => edit(zone)} className={`w-full rounded-xl border p-4 text-left ${selectedId === zone.id ? 'border-emerald-400 bg-emerald-50' : 'border-slate-200 bg-white'}`}><div className="flex items-start gap-3"><MapPinned className="mt-0.5 h-5 w-5 text-emerald-700"/><div className="min-w-0 flex-1"><p className="truncate font-semibold text-slate-900">{zone.name}</p><p className="mt-1 text-xs font-bold text-emerald-700">{zone.code}</p><p className="mt-1 text-xs text-slate-500">{zone.maximumStopsPerRun ?? 15} stops · ₹{(Number(zone.cashRiskLimitPaise ?? 0) / 100).toLocaleString('en-IN')}</p></div><Edit3 className="h-4 w-4 text-slate-400"/></div></button>)}</div></aside>
        <main className="space-y-5 p-4 sm:p-6"><section className="rounded-xl border border-slate-200 bg-white p-5"><div className="grid gap-4 sm:grid-cols-3"><Field label="Zone name"><input value={form.name} onChange={(event) => set('name', event.target.value)} placeholder="PM Palem" className="input"/></Field><Field label="Stable code"><input value={form.code} onChange={(event) => set('code', event.target.value.toUpperCase())} placeholder="PMP" className="input uppercase"/></Field><Field label="IANA timezone"><input value={form.timezone} onChange={(event) => set('timezone', event.target.value)} placeholder="Asia/Kolkata" className="input"/></Field></div><Field label="Description"><textarea value={form.description} onChange={(event) => set('description', event.target.value)} rows={2} placeholder="Morning subscription service area" className="input min-h-20 p-3"/></Field><div className="mt-4 flex flex-wrap gap-4"><label className="flex items-center gap-2 text-sm font-bold text-slate-700"><input type="checkbox" checked={form.isActive} onChange={(event) => set('isActive', event.target.checked)}/>Active</label><Field label="Priority"><input value={form.priority} onChange={(event) => set('priority', event.target.value)} inputMode="numeric" className="input w-28"/></Field></div></section>
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-5"><h3 className="font-semibold text-slate-950">Neighbouring zones</h3><div className="mt-3 flex flex-wrap gap-2">{zones.filter((zone) => zone.id !== selectedId).map((zone) => <button key={zone.id} type="button" onClick={() => toggle('neighbouringZoneIds', zone.id)} className={`rounded-full border px-3 py-2 text-xs font-semibold ${form.neighbouringZoneIds.includes(zone.id) ? 'border-emerald-500 bg-emerald-50 text-emerald-800' : 'border-slate-200 text-slate-600'}`}>{form.neighbouringZoneIds.includes(zone.id) ? '✓ ' : ''}{zone.name}</button>)}</div></section>
          <button disabled={saving} onClick={() => void save()} className="flex min-h-14 w-full items-center justify-center gap-2 rounded-xl bg-emerald-700 px-5 font-semibold text-white disabled:opacity-60">{saving ? <Loader2 className="h-5 w-5 animate-spin"/> : <Save className="h-5 w-5"/>}{selectedId ? 'Save zone policy' : 'Create geographic zone'}</button>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) { return <label className="mt-3 block text-xs font-semibold uppercase tracking-wide text-slate-500">{label}<div className="mt-2 normal-case tracking-normal">{children}</div></label>; }
