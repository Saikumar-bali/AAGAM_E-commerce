...
export default function LandingPage() {
  ...
  return (
    <main className="min-h-screen overflow-hidden text-slate-950">
      ...
      <section className="relative mx-auto max-w-7xl px-4 pb-16 pt-32 sm:px-6 lg:px-8 lg:pb-24 lg:pt-44">
        ...
        <div className="relative grid gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
          <div>
            ...
            <p className="mt-6 max-w-2xl text-lg font-semibold leading-8 text-slate-600">{hero?.subtitle || hero?.description || 'Aagaam combines customer storefront, store inventory, order fulfilment, rider dispatch, live tracking, support, analytics, and notifications for local commerce teams.'}</p>
      ...
      <footer className="relative border-t border-slate-200/70 bg-white px-4 py-14 sm:px-6 lg:px-8"><div className="mx-auto grid max-w-7xl gap-10 md:grid-cols-2 lg:grid-cols-4"><div><AagamLogo compact label="Commerce OS" /><p className="mt-4 max-w-sm text-sm font-semibold leading-6 text-slate-600">Accountable local commerce for customers, stores, riders, and operations teams.</p><p className="mt-5 text-xs font-bold text-slate-400">© 2026 Aagaam Commerce OS</p></div><div><h4 className="text-sm font-black uppercase tracking-[0.18em] text-slate-400">Shop</h4><div className="mt-4 grid gap-3 text-sm font-bold text-slate-700"><Link href="/shop">Browse products</Link><Link href="/shop/deals">Deals & offers</Link><Link href="/login">Track your orders</Link></div></div><div><h4 className="text-sm font-black uppercase tracking-[0.18em] text-slate-400">Partners</h4><div className="mt-4 grid gap-3 text-sm font-bold text-slate-700"><Link href="/partner">Become a store partner</Link><Link href="/partner">Deliver with Aagaam</Link><Link href="/login">Partner sign in</Link></div></div><div><h4 className="text-sm font-black uppercase tracking-[0.18em] text-slate-400">Trust & support</h4><div className="mt-4 grid gap-3 text-sm font-bold text-slate-700"><Link href="/login?returnTo=%2Fshop%2Fsupport">Customer support</Link><span>Verified partner onboarding</span><span>Audited fulfilment and delivery</span></div></div></div></footer>
    </main>
  );
}
