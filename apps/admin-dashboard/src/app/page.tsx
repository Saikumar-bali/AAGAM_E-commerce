...
import { ArrowRight, BarChart3, Bike, CheckCircle2, Clock3, MapPin, PackageCheck, Search, ShieldCheck, ShoppingBag, Sparkles, Star, Store, Truck } from 'lucide-react';
    ...
    <main className="min-h-screen overflow-hidden text-slate-950">
      ...
      <section className="relative mx-auto max-w-7xl px-4 pb-16 pt-32 sm:px-6 lg:px-8 lg:pb-24 lg:pt-44">
        ...
        <div className="relative grid gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
          <div>
            <p className="enterprise-kicker"><Sparkles className="mr-2 h-3.5 w-3.5" /> Quick-commerce operating system</p>
            <h1 className="mt-6 max-w-4xl text-5xl font-black tracking-[-0.07em] text-slate-950 sm:text-6xl lg:text-7xl">Run customer shopping, store fulfillment, and delivery ops from one platform.</h1>
            <p className="mt-6 max-w-2xl text-lg font-semibold leading-8 text-slate-600">Aagam combines customer storefront, store inventory, order fulfillment, rider dispatch, live tracking, support, analytics, and notifications for local commerce teams.</p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row"><Link href="/shop" className="enterprise-button gap-2">Start shopping <ArrowRight className="h-4 w-4" /></Link><Link href="/partners" className="inline-flex items-center justify-center rounded-2xl border border-slate-200 bg-white/80 px-5 py-3 text-sm font-extrabold text-slate-800 shadow-xl shadow-slate-900/5 transition hover:-translate-y-0.5 hover:border-teal-300">Partner with us</Link></div>
          ...
          <div className="enterprise-panel relative p-4 sm:p-6">
            <div className="rounded-[1.5rem] bg-slate-950 p-5 text-white shadow-2xl shadow-slate-950/20"><div className="flex items-center justify-between"><div><p className="text-xs font-black uppercase tracking-[0.22em] text-teal-200">Live order stack</p><h2 className="mt-2 text-2xl font-black tracking-tight">Aagam Command</h2></div><span className="rounded-full bg-emerald-400/15 px-3 py-1 text-xs font-black text-emerald-200">ONLINE</span></div><div className="mt-6 grid gap-3">{['Inventory reserved', 'Store preparing', 'Rider tracking'].map((item, index) => <div key={item} className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/8 p-4"><div className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-white font-black text-slate-950">0{index + 1}</span><span className="font-bold">{item}</span></div><CheckCircle2 className="h-5 w-5 text-emerald-300" /></div>)}</div></div>
            <div className="mt-4 grid grid-cols-2 gap-4"><div className="rounded-[1.5rem] bg-white p-5 shadow-xl shadow-slate-900/5"><PackageCheck className="h-6 w-6 text-amber-600" /><p className="mt-4 text-3xl font-black">Live</p><p className="text-xs font-bold text-slate-500">Inventory controls</p></div><div className="rounded-[1.5rem] bg-white p-5 shadow-xl shadow-slate-900/5"><Store className="h-6 w-6 text-teal-700" /><p className="mt-4 text-3xl font-black">Multi</p><p className="text-xs font-bold text-slate-500">Store operations</p></div></div>
          </div>
        </div>
      </section>

      <section className="relative mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <div className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><p className="enterprise-kicker">Featured catalogue</p><h2 className="mt-4 text-4xl font-black tracking-[-0.05em]">Retail polish, operations depth.</h2></div><div className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-white/80 px-4 py-3 text-sm font-bold text-slate-500"><Search className="h-4 w-4" /> Search-ready catalogue experience</div></div>
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">{(products.length ? products : Array.from({ length: 4 })).map((product, index) => <div key={product?.id || index} className="enterprise-card group overflow-hidden p-4 transition hover:-translate-y-1"><div className="flex aspect-square items-center justify-center overflow-hidden rounded-[1.25rem] bg-gradient-to-br from-teal-50 to-amber-50">{product?.image ? <img src={product.image} alt={product.name} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" /> : <ShoppingBag className="h-12 w-12 text-teal-700" />}</div><div className="mt-4 flex items-start justify-between gap-3"><div><h3 className="line-clamp-1 text-base font-black text-slate-950">{product?.name || 'Premium grocery item'}</h3><p className="mt-1 text-xs font-bold uppercase tracking-[0.16em] text-slate-400">{product?.category?.name || 'Essentials'}</p></div><div className="flex items-center gap-1 rounded-full bg-amber-50 px-2 py-1 text-xs font-black text-amber-700"><Star className="h-3 w-3 fill-amber-500" /> 4.8</div></div><div className="mt-4 flex items-center justify-between"><p className="text-lg font-black">{product?.price ? formatINR(product.price) : '₹99'}</p><Link href={product?.id ? `/shop/products/${product.id}` : '/shop'} className="rounded-full bg-slate-950 px-4 py-2 text-xs font-black text-white transition hover:bg-teal-700">View</Link></div></div>)}</div>
      </section>
      ...
      <footer className="relative border-t border-slate-200/70 bg-white/70 px-4 py-10 backdrop-blur-xl sm:px-6 lg:px-8"><div className="mx-auto grid max-w-7xl gap-8 md:grid-cols-[1.2fr_1fr_1fr]"><div><AagamLogo compact label="Commerce OS" /><p className="mt-4 max-w-md text-sm font-semibold leading-6 text-slate-600">A customer, store, rider, and admin operating system for local commerce.</p></div><div><h4 className="text-sm font-black uppercase tracking-[0.18em] text-slate-400">Partners</h4><div className="mt-4 grid gap-2 text-sm font-bold text-slate-700"><Link href="/partners">Partner overview</Link><Link href="/partners/store">Store partner application</Link><Link href="/partners/rider">Rider partner application</Link></div></div><div><h4 className="text-sm font-black uppercase tracking-[0.18em] text-slate-400">Platform</h4><div className="mt-4 grid gap-2 text-sm font-bold text-slate-700"><Link href="/login">Login</Link><Link href="/signup">Customer signup</Link><Link href="/shop">Shop</Link></div></div></div></footer>
    </main>
  );
}
