  ...
  return <main className="relative h-[100dvh] overflow-hidden bg-[#03151c] px-4 py-3 text-white sm:px-6 lg:px-10">
    ...
    <div className="relative mx-auto flex h-full max-w-[1440px] flex-col">
      ...
      <div className="grid min-h-0 flex-1 items-center gap-8 py-1 lg:grid-cols-[1fr_460px]">
        <section className="hidden self-end pb-8 lg:block"><div className="max-w-sm rounded-xl border border-white/10 bg-slate-950/35 p-4 "><p className="text-sm font-semibold text-white">Verified before your first order.</p><p className="mt-1 text-xs font-semibold leading-5 text-teal-100">A single-use code protects your account while local farmers and stores remain close at hand.</p></div></section>
        <section className="relative mx-auto w-full max-w-[460px] rounded-[1.75rem] border border-white/20 bg-white/[.9] p-5 text-slate-950 shadow-[0_40px_120px_-30px_rgba(0,0,0,.9)]  sm:p-7 [@media(max-height:680px)]:scale-[.82] [@media(max-height:760px)]:scale-[.9] [@media(max-height:760px)]:origin-center">
          ...
          <div className="mb-4 flex items-start justify-between"><div><p className="enterprise-kicker">Join Aagaam</p><h1 className="mt-2 text-3xl font-semibold tracking-[-.04em]">{step === 'DETAILS' ? 'Create your account' : 'Check your email'}</h1><p className="mt-1 text-xs font-bold text-slate-500">{step === 'DETAILS' ? 'Fresh local shopping starts here.' : `We sent a six-digit code to ${masked}.`}</p></div><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-gradient-to-br from-teal-600 to-emerald-500 text-white  shadow-teal-700/25">{step === 'DETAILS' ? <ShieldCheck className="h-5 w-5" /> : <Mail className="h-5 w-5" />}</div></div>
          ...
          {step === 'DETAILS' ? <div className="space-y-3">
            <label className="block"><span className="mb-1 block text-xs font-semibold">Full name</span><span className="relative block"><User className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input required value={name} onChange={(event) => setName(event.target.value)} className="enterprise-input py-2.5 pl-11" placeholder="Your full name" autoComplete="name" /></span></label>
            <label className="block"><span className="mb-1 block text-xs font-semibold">Email address</span><span className="relative block"><Mail className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-teal-700" /><input required type="email" value={email} onChange={(event) => setEmail(event.target.value)} className="enterprise-input py-2.5 pl-11" placeholder="you@example.com" autoComplete="email" /></span></label>
            <label className="block"><span className="mb-1 block text-xs font-semibold">Password</span><span className="relative block"><Lock className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input required minLength={8} maxLength={72} type={showPassword ? 'text' : 'password'} value={password} onChange={(event) => setPassword(event.target.value)} className="enterprise-input py-2.5 pl-11 pr-11" autoComplete="new-password" placeholder="At least 8 characters" /><button type="button" aria-label={showPassword ? 'Hide password' : 'Show password'} onClick={() => setShowPassword((value) => !value)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400">{showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button></span></label>
            <label className="block"><span className="mb-1 block text-xs font-semibold">Confirm password</span><span className="relative block"><Lock className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input required type={showPassword ? 'text' : 'password'} value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} className="enterprise-input py-2.5 pl-11" autoComplete="new-password" placeholder="Repeat your password" /></span></label>
          ...
          </div> : <div className="space-y-4"><div className="rounded-xl border border-teal-100 bg-teal-50 p-4 text-center"><KeyRound className="mx-auto h-6 w-6 text-teal-700" /><p className="mt-2 text-xs font-bold text-teal-900">Enter your single-use verification code</p></div><input value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, '').slice(0, 6))} className="enterprise-input text-center text-2xl font-semibold tracking-[.5em]" inputMode="numeric" maxLength={6} autoFocus autoComplete="one-time-code" placeholder="000000" /><button onClick={() => void verifyCode()} disabled={loading || code.length !== 6} className="enterprise-button w-full gap-2 disabled:opacity-60">{loading ? <Loader2 className="h-5 w-5 animate-spin" /> : <><CheckCircle2 className="h-4 w-4" /> Verify and create account</>}</button><button onClick={() => countdown === 0 ? void requestCode() : undefined} disabled={countdown > 0 || loading} className="w-full text-xs font-semibold text-teal-700 disabled:text-slate-400">{countdown ? `Resend in 00:${String(countdown).padStart(2, '0')}` : 'Resend email code'}</button><button onClick={() => { setStep('DETAILS'); setCode(''); setError(''); }} className="w-full text-xs font-bold text-slate-500">Edit account details</button></div>}
          <p className="mt-4 text-center text-xs font-semibold text-slate-500">Already registered? <Link href={customerAuthHref('/login', returnTo)} className="font-semibold text-teal-700">Sign in</Link></p>
        </section>
      </div>
    </div>
  </main>;
