...
export default function SignupPage() {
  ...
  const requestCode = async () => {
    ...
    if (!/^\+[1-9]\d{7,14}$/.test(normalized)) return setError('Enter a valid 10-digit mobile number or an international number with country code.');
    if (email.trim() && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim())) return setError('Enter a valid email address or leave it blank.');
    setLoading(true); setError('');
    try {
      const { data } = await apiClient.post('/auth/phone/request', { phoneE164: normalized, purpose: 'SIGNUP' });
  ...
  return <main className="relative min-h-screen overflow-hidden px-4 py-12 text-slate-950 sm:px-6 lg:px-8">
    ...
    <div className="relative mx-auto max-w-md">
      ...
      <p className="mt-2 text-center text-sm font-semibold text-slate-500">Already registered? <Link href="/login" className="font-black text-teal-700">Sign in</Link></p>
      <section className="enterprise-panel mt-8 p-6 sm:p-8">
        ...
        {!masked ? <div className="space-y-5">
          <label className="block"><span className="mb-2 block text-sm font-black text-slate-700">Full name</span><span className="relative block"><User className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-400" /><input required value={name} onChange={(event) => setName(event.target.value)} className="enterprise-input pl-12" placeholder="Your full name" /></span></label>
          <label className="block"><span className="mb-2 block text-sm font-black text-slate-700">Mobile number</span><span className="relative block"><Phone className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-teal-700" /><input required value={phone} onChange={(event) => setPhone(event.target.value)} className="enterprise-input pl-12" placeholder="10-digit mobile number" inputMode="tel" /></span></label>
          ...
          <button onClick={requestCode} disabled={loading} className="enterprise-button w-full gap-2 disabled:opacity-60">{loading ? <Loader2 className="h-5 w-5 animate-spin" /> : <>Send OTP <ArrowRight className="h-4 w-4" /></>}</button>
        </div> : <div className="space-y-5"><p className="text-center text-sm font-bold text-slate-600">Enter the six-digit code sent to {masked}</p><input value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, '').slice(0, 6))} className="enterprise-input text-center text-2xl font-black tracking-[.5em]" inputMode="numeric" maxLength={6} autoFocus placeholder="000000" /><button onClick={verifyCode} disabled={loading || code.length !== 6} className="enterprise-button w-full">{loading ? <Loader2 className="h-5 w-5 animate-spin" /> : 'Verify and create account'}</button><button onClick={() => countdown === 0 ? requestCode() : undefined} disabled={countdown > 0} className="w-full text-sm font-black text-teal-700 disabled:text-slate-400">{countdown > 0 ? `Resend in 00:${String(countdown).padStart(2, '0')}` : 'Resend OTP'}</button><button onClick={() => { setMasked(''); setCode(''); }} className="w-full text-xs font-bold text-slate-500">Edit account details</button></div>}
      </section>
    </div>
  </main>;
}
