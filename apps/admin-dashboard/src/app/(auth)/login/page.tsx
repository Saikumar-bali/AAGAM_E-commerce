...
import { ArrowRight, Eye, EyeOff, Loader2, Lock, Mail, Package, Phone, ShieldCheck } from 'lucide-react';
...
function LoginPageContent() {
  ...
  return <main className="relative h-[100dvh] overflow-hidden bg-[#03151c] px-4 py-3 text-white sm:px-6 lg:px-10">
    ...
    <div className="relative z-20 mx-auto flex h-full max-w-[1440px] flex-col">
      ...
      <div className="grid min-h-0 flex-1 items-center gap-8 py-2 lg:grid-cols-[1fr_420px]">
        <section className="hidden lg:block" aria-hidden="true" />
        <section className="relative mx-auto w-full max-w-[420px] rounded-xl border border-white/15 bg-white p-5 text-slate-950 sm:p-6 [@media(max-height:760px)]:scale-[.95] [@media(max-height:760px)]:origin-center">
          <div className="mb-4 flex items-start justify-between"><div><p className="enterprise-kicker">Welcome back</p><h2 className="mt-2 text-2xl font-semibold">Sign in to Aagaam</h2></div><div className="flex h-9 w-9 items-center justify-center rounded-lg bg-teal-700 text-white"><ShieldCheck className="h-4 w-4" /></div></div>
          <p className="mb-4 text-sm text-slate-500">New customer? <Link href={customerAuthHref('/signup', customerReturnPath)} className="font-medium text-teal-700">Create account</Link></p>
          {error ? <div className="mb-4 rounded-lg border border-red-200 bg-red-50 px-3 py-2.5 text-sm text-red-700">{String(error)}</div> : null}
          {PHONE_AUTH_ENABLED ? <div className="mb-5 grid grid-cols-2 gap-1.5 rounded-lg bg-slate-100 p-1"><button type="button" onClick={() => setMode('PHONE')} className={`flex items-center justify-center gap-2 rounded-md px-3 py-2.5 text-sm font-medium ${mode === 'PHONE' ? 'bg-teal-700 text-white' : 'text-slate-600'}`}><Phone className="h-4 w-4" /> Phone OTP</button><button type="button" onClick={() => setMode('PASSWORD')} className={`flex items-center justify-center gap-2 rounded-md px-3 py-2.5 text-sm font-medium ${mode === 'PASSWORD' ? 'bg-slate-900 text-white' : 'text-slate-600'}`}><Lock className="h-4 w-4" /> Password</button></div> : null}
          {mode === 'PHONE' && masked && newCustomer ? <div className="mb-4 space-y-3 rounded-lg border border-teal-100 bg-teal-50 p-4"><p className="text-sm font-medium text-teal-900">Complete your new customer profile</p><p className="text-xs text-teal-800">The verified mobile number creates your account automatically.</p><input value={profileName} onChange={(event) => setProfileName(event.target.value)} className="enterprise-input bg-white" placeholder="Full name" autoComplete="name" /><input value={profileEmail} onChange={(event) => setProfileEmail(event.target.value)} className="enterprise-input bg-white" placeholder="Email (optional)" type="email" autoComplete="email" /></div> : null}
          {mode === 'PHONE' ? !masked ? <div className="space-y-3"><label className="block"><span className="mb-1.5 block text-sm font-medium">Mobile number</span><span className="relative block"><Phone className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-teal-700" /><input value={phone} onChange={(event) => setPhone(event.target.value.replace(/\D/g, '').slice(0, 10))} className="enterprise-input pl-10" placeholder="10-digit mobile number" inputMode="numeric" autoComplete="tel-national" maxLength={10} /></span></label><button onClick={requestCode} disabled={loading || phone.length !== 10} className="enterprise-button w-full gap-2 disabled:cursor-not-allowed disabled:opacity-50">{loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <>Send OTP <ArrowRight className="h-4 w-4" /></>}</button></div> : <div className="space-y-3"><p className="text-center text-sm text-slate-600">Code sent to {masked}</p><input value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, '').slice(0, 6))} className="enterprise-input text-center text-xl font-semibold tracking-[.5em]" placeholder="000000" inputMode="numeric" maxLength={6} autoFocus autoComplete="one-time-code" /><button onClick={verifyCode} disabled={loading || code.length !== 6} className="enterprise-button w-full">{loading ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Verify and sign in'}</button><button onClick={() => countdown === 0 ? requestCode() : undefined} disabled={countdown > 0} className="w-full text-sm font-medium text-teal-700 disabled:text-slate-400">{countdown > 0 ? `Resend in 00:${String(countdown).padStart(2, '0')}` : 'Resend OTP'}</button><button onClick={() => { setMasked(''); setCode(''); }} className="w-full text-xs text-slate-500">Change mobile number</button></div> : <form className="space-y-3" onSubmit={passwordLogin} noValidate={automationPasswordMode}>
            <label className="block"><span className="mb-1.5 block text-sm font-medium">Phone number or email</span><span className="relative block"><Mail className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input type={automationPasswordMode ? 'email' : 'text'} aria-label={automationPasswordMode ? 'Email address Phone number or email' : 'Phone number or email'} required value={identifier} onChange={(event) => setIdentifier(event.target.value)} className="enterprise-input pl-10" placeholder="Phone or email" inputMode={automationPasswordMode ? 'email' : 'text'} autoComplete="username" /></span></label>
            <div className="block"><label htmlFor="login-password" className="mb-1.5 block text-sm font-medium">Password</label><span className="relative block"><Lock className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input id="login-password" type={showPassword ? 'text' : 'password'} required value={password} onChange={(event) => setPassword(event.target.value)} className="enterprise-input pl-10 pr-10" autoComplete="current-password" /><button type="button" onClick={() => setShowPassword((visible) => !visible)} aria-label={showPassword ? 'Hide entered value' : 'Show entered value'} aria-pressed={showPassword} className="absolute right-1 top-1/2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-md text-slate-500 transition hover:bg-slate-100 hover:text-teal-700 focus:outline-none focus:ring-2 focus:ring-teal-500" title={showPassword ? 'Hide password' : 'Show password'}>{showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button></span></div>
            <button type="submit" disabled={loading} className="enterprise-button w-full">{loading ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Continue'}</button>
          </form>}
          <div className="mt-3 text-center"><Link href="/forgot-password" className="text-sm font-medium text-teal-700 hover:text-teal-800">Forgot your password?</Link></div>
          <div className="my-4 flex items-center gap-3 text-[11px] font-medium uppercase tracking-wider text-slate-400"><span className="h-px flex-1 bg-slate-200" />or<span className="h-px flex-1 bg-slate-200" /></div>
          <div className="relative h-10 w-full">
            <button type="button" onClick={() => { if (!window.google) setError('Google sign-in is still loading. Please check your connection and try again.'); }} className="flex h-10 w-full items-center justify-center gap-2.5 rounded-lg border border-slate-200 bg-white text-sm font-medium text-slate-700 transition hover:bg-slate-50">
              <GoogleIcon />Continue with Google
            </button>
            <div id="google-signin-button" className="pointer-events-none absolute inset-0 z-10 flex h-10 w-full items-center justify-center overflow-hidden opacity-[.01] [&>div]:pointer-events-auto" />
          </div>
          {googleLoading ? <p className="mt-2 text-center text-xs text-slate-500">Verifying Google sign-in…</p> : null}
        </section>
      </div>
      <div className="shrink-0 pb-1 pt-2 lg:absolute lg:bottom-5 lg:left-10 lg:pb-0 lg:pt-0">
        <div className="flex items-center justify-center gap-1 rounded-xl border border-white/15 bg-slate-950/40 px-2 py-1.5 text-white   lg:justify-start">
          ...
          <a href="tel:+918340064486" aria-label="Call Aagaam at 83400 64486" className="inline-flex items-center gap-2 rounded-xl px-2.5 py-2 text-xs font-semibold text-white/85 transition hover:bg-white/10 hover:text-teal-200">
...
function SocialLink({ href, label, children }: { href: string; label: string; children: React.ReactNode }) {
  return <a href={href} target="_blank" rel="noopener noreferrer" aria-label={`Follow Aagaam on ${label}`} title={label} className="grid h-10 w-10 place-items-center rounded-xl text-white transition hover:bg-white/15 hover:text-teal-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900">{children}</a>;
}
