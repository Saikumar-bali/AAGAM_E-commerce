...
import {
  formatDate,
  formatPaise,
} from "@/components/subscriptions/SubscriptionPlanCard";
import {
  CalendarDays,
  CheckCircle2,
  Loader2,
...
export default function SubscriptionDetailsPage() {
  ...
  const load = async () => {
    try {
      const [a, b] = await Promise.all([
        apiClient.get(`/customer/subscriptions/${encodeURIComponent(id)}`),
        apiClient.get(
          `/customer/subscriptions/${encodeURIComponent(id)}/deliveries`
        ),
      ]);
      setS(a.data);
      setDeliveries(Array.isArray(b.data) ? b.data : []);
    } catch (e) {
      toast.error(getToastErrorMessage(e, "Subscription could not be loaded."));
    } finally {
      setLoading(false);
    }
  };
  ...
  const total = Number(
    s?.planVersion?.totalDeliveries || deliveries.length || 1
  );
  const progress = Math.min(
    100,
    Math.round((Number(s?.completedDeliveries || 0) / total) * 100)
  );
  const next = useMemo(
    () =>
      deliveries.find((d) =>
  ...
  const action = async (kind: "skip" | "pause" | "resume") => {
    setBusy(kind);
    try {
      if (kind === "skip") {
        if (!next || next.status !== "SCHEDULED")
          throw new Error("No skippable future delivery is available.");
        await apiClient.post(
          `/customer/subscriptions/${id}/deliveries/${next.id}/skip`,
          { reason: "Customer requested skip" },
          { headers: { "Idempotency-Key": `web-skip:${crypto.randomUUID()}` } }
        );
      } else if (kind === "pause") {
        const d = new Date();
        ...
        await apiClient.post(
          ...
          { headers: { "Idempotency-Key": `web-pause:${crypto.randomUUID()}` } }
        );
      } else
        await apiClient.post(
          ...
          {},
          {
            headers: { "Idempotency-Key": `web-resume:${crypto.randomUUID()}` },
          }
        );
      toast.success(
        kind === "skip"
      ...
      toast.error(getToastErrorMessage(e, "Action failed."));
    } finally {
      setBusy("");
    }
  };
  const cancelBeforePayment = async () => {
    const reason = cancelReason.trim();
    if (reason.length < 5) {
      toast.warning("Please enter a short cancellation reason.");
      return;
    }
    setBusy("cancel");
    try {
      await apiClient.post(
        ...
        { reason },
        { headers: { "Idempotency-Key": `web-cancel:${crypto.randomUUID()}` } }
      );
      toast.success("Subscription cancelled before payment.");
      setShowCancel(false);
      setCancelReason("");
      await load();
    } catch (e) {
      toast.error(getToastErrorMessage(e, "Cancellation failed."));
    } finally {
      setBusy("");
    }
  };
  const track = async () => {
    try {
      const { data } = await apiClient.get(
        `/customer/subscriptions/${id}/tracking`
      );
      if (data?.orderId) router.push(`/shop/orders?orderId=${data.orderId}`);
      else toast.info("No active delivery order is available yet.");
    } catch (e) {
      toast.error(getToastErrorMessage(e, "Tracking is unavailable."));
    }
  };
  if (loading)
    return (
  ...
  if (!s)
    return (
      <DashboardLayout allowedRole="CUSTOMER">
        <div className="p-10 text-center">Subscription not found.</div>
      </DashboardLayout>
    );
  const canCancelBeforePayment =
    s.status === "PENDING_CASH_COLLECTION" &&
    Number(s.amountCollectedPaise || 0) === 0;
  return (
    <DashboardLayout allowedRole="CUSTOMER">
      <div className="mx-auto max-w-6xl space-y-5 p-4 sm:p-7">
        <section className="rounded-[30px] bg-gradient-to-br from-emerald-800 to-teal-700 p-6 text-white sm:p-8">
          <div className="flex flex-col justify-between gap-6 md:flex-row md:items-center">
            ...
            <div className="flex h-28 w-28 flex-col items-center justify-center rounded-full border-[10px] border-emerald-300/60 bg-emerald-900/20">
              ...
              <span className="text-xs text-emerald-100">complete</span>
            </div>
          </div>
          ...
          <div className="mt-4 flex flex-wrap justify-between gap-4 text-sm font-bold text-emerald-100">
            <span>
              {s.completedDeliveries} of {total} delivered
            </span>
            <span>{s.remainingFundedDeliveries} funded deliveries left</span>
            <span>{s.skippedDeliveries} skipped</span>
          </div>
        </section>
          ...
          <section className="flex flex-col gap-4 rounded-[24px] border border-red-200 bg-red-50 p-5 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm font-semibold text-slate-600">
                You can cancel this subscription now. No cash has been collected
                yet.
              </p>
            </div>
            <button
        ...
        <section className="grid grid-cols-3 gap-3">
          ...
          <Action
            icon={<TriangleAlert />}
            label="Report issue"
            onClick={() => router.push("/shop/support")}
          />
        </section>
            ...
            <div className="mt-4 divide-y divide-slate-100">
              {deliveries.map((d) => (
                <div key={d.id} className="flex items-center gap-4 py-4">
                  ...
                  <strong
                    className={
                      d.cashDuePaise > 0 ? "text-amber-700" : "text-emerald-700"
                    }
                  >
                    {formatPaise(d.cashDuePaise)}
                  </strong>
                </div>
              ))}
            </div>
          ...
          <aside className="space-y-5">
            <section className="rounded-[24px] border border-slate-200 bg-white p-5">
              ...
              <Info
                icon={<MapPin />}
                label="Address"
                value={s.address?.label || "Saved address"}
              />
              <Info
                icon={<WalletCards />}
                label="Funding"
                ...
                {s.fundingAllocations.map((a: any) => (
                  ...
                  >
                    ...
                    <p className="mt-1 text-xs text-emerald-700">
                      Deliveries {a.startsAtSequence}–{a.endsAtSequence} ·{" "}
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-2 text-sm font-semibold leading-6 text-slate-500">
                    No payment has been collected. Cancelling will remove all
                    future scheduled subscription deliveries and cannot be
                    resumed.
                  </p>
                </div>
                <button
