  ...
  return (
    <DashboardLayout allowedRole="CUSTOMER">
      <div className="mx-auto max-w-7xl space-y-5 p-4 pb-24">
        <section className="rounded-xl bg-slate-950 p-6 text-white"><div className="text-xs font-semibold uppercase text-teal-300">Phase 6 Shopping UX</div><h1 className="mt-2 text-3xl font-semibold">Serviceability, search, stock, cart and quote</h1></section>
        <section className="grid gap-3 rounded-xl border bg-white p-4 md:grid-cols-3"><select value={addressId} onChange={(e) => setAddressId(e.target.value)} className="rounded-xl border px-3 py-3 text-sm font-bold"><option value="">Select address</option>{addresses.map((a) => <option key={a.id} value={a.id}>{a.label || 'Address'} - {a.line1}, {a.city}</option>)}</select><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search products" className="rounded-xl border px-3 py-3 text-sm font-bold" /><select value={categoryId} onChange={(e) => setCategoryId(e.target.value)} className="rounded-xl border px-3 py-3 text-sm font-bold"><option value="">All categories</option>{categories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></section>
        ...
        <section className="grid gap-4 lg:grid-cols-[1fr_360px]"><div><div className="mb-3 text-sm font-semibold text-slate-700">{products.length} products</div><div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-4">{products.map((p) => <div key={p.id} className="rounded-xl border bg-white p-3 "><div className="aspect-[4/3] rounded-xl bg-slate-100" /><h3 className="mt-3 text-sm font-semibold text-slate-950">{p.name}</h3><p className="mt-1 text-xs font-bold text-slate-500">{p.category?.name || 'General'}</p><div className="mt-2 flex items-center justify-between"><span className="font-semibold">{formatINR(Number(p.price) || 0)}</span><span className={p.availability?.inStock === false ? 'text-xs font-semibold text-red-600' : 'text-xs font-semibold text-teal-700'}>{p.availability?.inStock === false ? 'Out' : `${p.availability?.availableQty ?? 'In'} stock`}</span></div><button onClick={() => add(p)} disabled={serviceability?.serviceable === false} className="mt-3 w-full rounded-xl bg-teal-700 py-2 text-xs font-semibold text-white disabled:bg-slate-300">{p.availability?.inStock === false ? 'Substitutes' : 'Add'}</button>{substitutes[p.id]?.map((s) => <button key={s.id} onClick={() => add(s)} className="mt-2 block w-full rounded bg-amber-50 px-2 py-1 text-left text-xs font-bold">Replace with {s.name}</button>)}</div>)}</div></div><aside className="h-fit rounded-xl border bg-white p-4 "><div className="flex items-center justify-between"><div className="text-lg font-semibold text-slate-950">Cart</div><div className="rounded-xl bg-slate-950 px-3 py-1 text-xs font-semibold text-white">{cartCount} items</div></div>{lines.length === 0 && <p className="mt-4 text-sm font-bold text-slate-500">Add products to calculate quote.</p>}{lines.map((line) => <div key={line.product.id} className="mt-3 rounded-xl bg-slate-50 p-3"><div className="text-sm font-semibold">{line.product.name}</div><div className="mt-2 flex items-center justify-between"><span className="text-xs font-bold text-slate-500">{formatINR(line.product.price)}</span><div className="flex items-center gap-2"><button onClick={() => updateQty(line.product.id, line.quantity - 1)} className="rounded bg-white px-2 font-semibold">-</button><span className="text-sm font-semibold">{line.quantity}</span><button onClick={() => updateQty(line.product.id, line.quantity + 1)} className="rounded bg-white px-2 font-semibold">+</button></div></div></div>)}{quote && <div className="mt-4 rounded-xl bg-teal-50 p-3 text-sm font-bold text-teal-900">{quote.store?.name || 'Store'} • {quote.distanceKm?.toFixed?.(1)} km • ETA {quote.etaMinutes || 10} min<br />Subtotal {formatINR(quote.invoice.subtotal)}<br />Delivery {formatINR(quote.invoice.deliveryFee)}<br /><span className="text-lg font-semibold">Total {formatINR(quote.invoice.grandTotal)}</span></div>}<button onClick={placeOrder} disabled={placing || lines.length === 0 || quote?.serviceable === false} className="mt-4 w-full rounded-xl bg-slate-950 py-3 text-sm font-semibold text-white disabled:bg-slate-300">{placing ? 'Placing...' : 'Place COD order'}</button></aside></section>
      </div>
    </DashboardLayout>
  );
}
