    ...
    <DashboardLayout allowedRole="CUSTOMER">
      <main className="mx-auto max-w-4xl space-y-5 p-4 pb-24">
        ...
        <section className="rounded-xl bg-slate-950 p-6 text-white">
          <p className="text-xs font-semibold uppercase text-teal-300">Post-delivery experience</p>
          <h1 className="mt-2 text-3xl font-semibold">Rate order or get support</h1>
        ...
        {loading && <div className="rounded-xl bg-slate-100 p-8 text-center text-sm font-bold text-slate-500">Loading...</div>}

        {!loading && order && (
          <div className="grid gap-5 lg:grid-cols-2">
            <section className="rounded-xl border bg-white p-5 ">
              <div className="mb-4 flex items-center gap-2"><Star className="h-5 w-5 text-amber-500" /><h2 className="text-lg font-semibold">Rate this order</h2></div>
              {!delivered && <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm font-bold text-slate-600">Rating opens after delivery.</div>}
              {ratingSubmitted && <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm font-semibold text-emerald-800">Rating already submitted.</div>}
              {delivered && !ratingSubmitted && <div className="space-y-4">
                {[['orderRating', 'Order'], ['storeRating', 'Store'], ['riderRating', 'Rider']].map(([key, label]) => (
                  <label key={key} className="block"><span className="text-sm font-semibold text-slate-700">{label} rating</span><select value={(rating as any)[key]} onChange={(e) => setRating((prev) => ({ ...prev, [key]: Number(e.target.value) }))} className="mt-1 w-full rounded-xl border px-3 py-3 text-sm font-bold"><option value={5}>5 - Excellent</option><option value={4}>4 - Good</option><option value={3}>3 - Okay</option><option value={2}>2 - Poor</option><option value={1}>1 - Bad</option></select></label>
                ))}
                ...
                <button onClick={submitRating} className="w-full rounded-xl bg-amber-500 py-3 text-sm font-semibold text-white">Submit rating</button>
              </div>}
            </section>

            <section className="rounded-xl border bg-white p-5 ">
              <div className="mb-4 flex items-center gap-2"><Headphones className="h-5 w-5 text-teal-600" /><h2 className="text-lg font-semibold">Need help?</h2></div>
              <div className="space-y-4">
                <label className="block"><span className="text-sm font-semibold text-slate-700">Issue category</span><select value={support.category} onChange={(e) => setSupport((prev) => ({ ...prev, category: e.target.value }))} className="mt-1 w-full rounded-xl border px-3 py-3 text-sm font-bold"><option value="DELIVERY_ISSUE">Delivery issue</option><option value="MISSING_ITEM">Missing item</option><option value="DAMAGED_ITEM">Damaged item</option><option value="PAYMENT_ISSUE">Payment issue</option><option value="OTHER">Other</option></select></label>
                <label className="block"><span className="text-sm font-semibold text-slate-700">Priority</span><select value={support.priority} onChange={(e) => setSupport((prev) => ({ ...prev, priority: e.target.value as SupportState['priority'] }))} className="mt-1 w-full rounded-xl border px-3 py-3 text-sm font-bold"><option value="NORMAL">Normal</option><option value="HIGH">High</option><option value="LOW">Low</option></select></label>
                ...
                <button onClick={submitSupport} disabled={support.message.trim().length < 5} className="w-full rounded-xl bg-slate-950 py-3 text-sm font-semibold text-white disabled:bg-slate-300">Open support ticket</button>
              </div>
              {postDelivery?.tickets?.length > 0 && <div className="mt-5 rounded-xl bg-teal-50 p-4"><p className="text-sm font-semibold text-teal-900">Previous tickets</p>{postDelivery.tickets.map((ticket: any) => <p key={ticket.id} className="mt-2 text-xs font-bold text-teal-800">{ticket.metadata?.category || 'Issue'} · {ticket.metadata?.status || 'OPEN'}</p>)}</div>}
            </section>
          </div>
        )}
      </main>
