...
const CustomerTrackingMap = dynamic(
  () => import("@/components/CustomerTrackingMap"),
  { ssr: false }
);
...
function customerMessage(
  ...
  if (status === "RIDER_ASSIGNED")
    return trackingState === "ASSIGNED_NO_LOCATION"
      ...
      : "Rider assigned and moving towards pickup.";
  if (status === "OUT_FOR_DELIVERY")
    return etaStale
      ? "Your order is on the way. Location is temporarily stale."
      : "Your order is on the way with live tracking.";
  if (status === "PICKING")
    return "The store is picking and packing your items.";
  if (status === "CONFIRMED") return "The store confirmed your order.";
  return "We are processing your order.";
}

function TrackingStateBanner({
  state,
  status,
  etaStale,
}: {
  ...
  > = {
    ...
    ASSIGNED_NO_LOCATION: {
      ...
      text: "text-blue-700",
      dot: "bg-blue-500",
      label: "Rider assigned — pickup pending",
    },
    LIVE: {
      bg: "bg-emerald-50",
    ...
    STALE: {
      ...
      text: "text-amber-700",
      dot: "bg-amber-500",
      label: "Tracking paused — waiting for rider update",
    },
    DELIVERED: {
      bg: "bg-emerald-50",
      border: "border-emerald-200",
...
export default function CustomerOrderDetailPage() {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      setTrackingPayload(trackingResponse.data);
    } catch (exception: any) {
      setError(exception?.response?.data?.message || "Failed to load order");
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    if (!orderId) return;
    void apiClient
      .get(`/orders/my/${orderId}/delivery-context`)
      .then((res) => {
  ...
  useEffect(() => {
    ...
    socket.on("riderMoved", (payload: any) => {
      if (payload.orderId === orderId) setLiveLocation(payload);
    });
    ...
    socket.on("orderStatusUpdated", (payload: any) => {
      if (payload.orderId === orderId) void refresh();
    });
  ...
  useEffect(() => {
    ...
    const poll = setInterval(() => {
      apiClient
        .get(`/tracking/my/order/${orderId}`)
        .then((response) => setTrackingPayload(response.data))
        .catch(() => undefined);
    }, 10000);
  ...
  const pricing = useMemo(
    () => normalizeOrderPricing(order, items),
    [order, items]
  );
  ...
  const livePoint =
    liveLocation || trackingPayload?.tracking?.latestLocation || null;
  const trackingMeta = trackingPayload?.tracking || {};
  const trackingState = trackingMeta.trackingState || "NOT_ASSIGNED";
  const etaLabel = trackingMeta.etaMinutes
    ? `ETA ${trackingMeta.etaMinutes} min`
    : null;
  const distanceLabel =
    trackingMeta.distanceKm != null
      ? `${trackingMeta.distanceKm} km away`
      : null;
  const lastPingAt = livePoint?.createdAt || trackingMeta.lastPingAt;
  const timeline = Array.isArray(trackingPayload?.timeline)
    ? trackingPayload.timeline
    : [];
  const proofEvent = timeline.find(
    (item: any) =>
      item?.metadata?.event === "DELIVERY_PROOF_RECORDED" ||
      item?.note === "Rider submitted delivery proof."
  );
  ...
  const markers = useMemo(() => {
    ...
    if (
      typeof trackingPayload?.store?.latitude === "number" &&
      typeof trackingPayload?.store?.longitude === "number"
    )
      values.push({
    ...
    <DashboardLayout allowedRole="CUSTOMER">
      <div className="mx-auto max-w-4xl">
        <button
          ...
          className="mb-4 inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-bold text-slate-700"
        >
              ...
              {(deliveryStatus === "OUT_FOR_DELIVERY" ||
                ...
                deliveryJobId && (
                  ...
                  >
                    ...
                    {deliveryStatus === "RIDER_AT_CUSTOMER"
                      ? "View delivery code"
                      : "Delivery code (rider approaching)"}
                  </button>
                )}
            </section>
            {showTrackingMap && (
                ...
                <div className="border-t border-slate-100 p-4">
                  <div className="flex items-center justify-between text-xs text-slate-500">
                    <span className="flex items-center gap-1">
                      ...
                      {lastPingAt
                        ? new Date(lastPingAt).toLocaleTimeString("en-IN")
                        : "Never"}
                    </span>
                  ...
                  {trackingMeta.etaStale && (
                    <div className="mt-3 flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs font-bold text-amber-800">
                      <AlertTriangle className="h-4 w-4" /> Rider location is
                      stale. ETA will resume after the next location update.
                    </div>
                  )}
                </div>
              </div>
                ...
                {proof && (
                  <div className="mt-3 rounded-xl bg-white/70 p-3 text-xs font-bold text-emerald-900">
                    <div>
                      Proof:{" "}
                      {proof.proofType ||
                        proof.deliveryProof?.method ||
                        "Rider confirmation"}
                    </div>
                    {proofCode && (
                      <div className="mt-1">Delivery code: {proofCode}</div>
                    )}
                    {proofNote && (
            ...
            {address && (
              <section className="rounded-2xl border border-slate-100 bg-white p-5">
                ...
                <div className="text-sm font-bold text-slate-800">
                  ...
                  <span className="text-slate-500">({address.phoneE164})</span>
                </div>
