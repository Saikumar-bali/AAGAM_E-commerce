...
export default function CustomerNotificationsPage() {
  ...
  const markRead = async (id: string) => {
    try {
      ...
      setMessage(err?.response?.data?.message || 'Could not mark notification read');
    }
  };
    ...
    <DashboardLayout allowedRole="CUSTOMER">
      <main className="mx-auto max-w-4xl space-y-5 p-4 pb-24">
        ...
        <section className="space-y-3">
          {items.map((item) => (
            <article key={item.id} className={`rounded-3xl border bg-white p-5 shadow-sm ${item.readAt ? 'opacity-70' : ''}`}>
              <div className="flex items-start justify-between gap-3"><div><p className="text-sm font-black text-slate-950">{item.title}</p><p className="mt-1 text-sm font-bold text-slate-600">{item.body}</p><p className="mt-2 text-xs font-bold text-slate-400">#{item.orderId.slice(-8).toUpperCase()} · {new Date(item.createdAt).toLocaleString('en-IN')}</p></div>{!item.readAt && <button onClick={() => markRead(item.sourceHistoryId)} className="inline-flex items-center gap-1 rounded-xl bg-teal-600 px-3 py-2 text-xs font-black text-white"><CheckCheck className="h-3 w-3" /> Read</button>}</div>
            </article>
          ))}
        </section>
      </main>
