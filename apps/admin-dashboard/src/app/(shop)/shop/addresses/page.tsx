...
export default function AddressesPage() {
  ...
  const [draft, setDraft] = useState(() => createCustomerAddressDraft());
  ...
  const fetchAddresses = async () => {
    try {
      ...
      setDraft((current) => ({
        ...current,
        recipientName:
          current.recipientName.trim().length >= 2
            ? current.recipientName
            : profile.name,
        phoneE164: current.phoneE164.trim() || profile.phone,
        isDefault: current.isDefault || list.length === 0,
      }));
    } catch (e: any) {
  ...
  const buildPayload = () =>
    buildCustomerAddressPayload(draft, {
    ...
    <DashboardLayout allowedRole="CUSTOMER">
      <div className="mx-auto max-w-3xl px-1 pb-24 sm:px-0">
        ...
        <div className="mb-6 space-y-3">
          {addresses.map((addr) => {
            const Icon =
              ADDRESS_ICONS[(addr.label || "").toLowerCase()] || Navigation;
            return (
              <div
                key={addr.id}
                className={`relative rounded-2xl border p-4 transition-all ${addr.isDefault ? "border-teal-300 bg-teal-50 shadow-sm shadow-teal-100" : "border-slate-100 bg-white hover:border-teal-200"}`}
              >
                <div className="flex items-start justify-between">
                  <div className="flex flex-1 items-start gap-3">
                    <div
                      className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl ${addr.isDefault ? "bg-teal-600 text-white" : "bg-slate-100 text-slate-500"}`}
                    >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        {addr.isDefault && (
                          <span className="rounded-lg bg-teal-600 px-1.5 py-0.5 text-[10px] font-black text-white">
                            Default
                          </span>
                        )}
                      </div>
                    ...
                    {menuOpenId === addr.id && (
                      <div className="absolute right-0 top-full z-20 mt-1 w-36 overflow-hidden rounded-xl border border-slate-100 bg-white shadow-xl">
                        ...
                        <button
                          ...
                          className="flex w-full items-center gap-2 px-3 py-2.5 text-sm font-semibold text-red-600 hover:bg-red-50"
                        >
          ...
          <div className="rounded-2xl border border-slate-100 bg-white p-5">
            <div className="mb-4 flex items-center justify-between">
              ...
              <button
                ...
                className="grid h-8 w-8 place-items-center rounded-xl transition-colors hover:bg-slate-100"
              >
              ...
              <StoreLocationPicker
                ...
                onAddressChange={(address) =>
                  setDraft((d) => ({
                    ...
                    pincode:
                      d.pincode || cleanIndianPincode(address.pincode || ""),
                  }))
                }
            ...
            <div className="mt-5 flex gap-3">
              <button
                ...
                className="flex-1 rounded-xl bg-slate-100 px-4 py-2.5 font-black text-slate-700 transition-colors hover:bg-slate-200"
              >
                Cancel
              </button>
              <button
                ...
                className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-teal-700 px-4 py-2.5 font-black text-white transition-colors hover:bg-teal-800 disabled:opacity-50"
              >
        ...
        {!showForm && addresses.length > 0 && (
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-slate-300 py-3.5 font-black text-slate-500 transition-colors hover:border-teal-400 hover:text-teal-600"
          >
            ...
            <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl">
              ...
              <div className="mt-5 flex gap-3">
                <button
                  ...
                  className="flex-1 rounded-xl bg-slate-100 px-4 py-2.5 font-black text-slate-700 hover:bg-slate-200"
                >
                  Cancel
                </button>
                <button
                  onClick={deleteAddress}
                  className="flex-1 rounded-xl bg-red-600 px-4 py-2.5 font-black text-white hover:bg-red-700"
                >
                  Delete
                </button>
              </div>
  ...
  return (
    <div className={className}>
      ...
      <input
        ...
        className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-900 transition-colors placeholder:text-slate-400 focus:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-500"
      />
    </div>
  );
}
