...
export default function CheckoutPage() {
  ...
  const [draft, setDraft] = useState(() =>
    createCustomerAddressDraft(undefined, true),
  );
    ...
    const load = async () => {
      ...
      try {
        ...
        const list = Array.isArray(addressResponse.data)
          ? (addressResponse.data as Address[])
          : [];
        const profile = {
          name: String(profileResponse.data?.name || "").trim(),
          phone: String(profileResponse.data?.phone || "").trim(),
        };
        setAddresses(list);
        setProfileDefaults(profile);
        setDraft(createCustomerAddressDraft(profile, list.length === 0));
        if (list.length > 0) {
          setSelectedAddressId(list.find((a) => a.isDefault)?.id || list[0].id);
        }
      } catch (e: any) {
        setError(getApiErrorMessage(e, "Failed to load addresses"));
      } finally {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        setDraft((d) => ({
          ...
          phoneE164: d.phoneE164.trim() || profileDefaults.phone,
          latitude,
          longitude,
        }));
        try {
          ...
          if (res.data?.ok && addr) {
            setDraft((d) => ({
              ...
              pincode: cleanIndianPincode(addr.pincode || d.pincode),
  ...
  const createAddress = async () => {
    ...
    try {
      const payload = buildCustomerAddressPayload(draft, {
        fallbackProfile: profileDefaults,
  ...
  const saveEditedAddress = async () => {
    ...
    try {
      ...
      setAddresses((prev) =>
        prev.map((a) =>
          a.id === editingAddressId ? (res.data as Address) : a,
        ),
      );
      ...
      setError(getApiErrorMessage(e, "Failed to update address"));
    }
    setSavingAddress(false);
  };
  ...
  const confirmDeleteAddress = async () => {
    ...
    try {
      ...
      setError(getApiErrorMessage(e, "Failed to delete address"));
    }
  };

  const resetDraft = () => {
    setDraft(
      createCustomerAddressDraft(profileDefaults, addresses.length === 0),
    );
  };
  ...
  const placeOrder = async () => {
    ...
    if (!quote) {
      setError("Please select an address to calculate totals.");
      return;
    }
    if (!quote.serviceable) {
      const dist =
        quote.distanceKm != null
          ? ` (${quote.distanceKm.toFixed(1)} km from nearest store — max 8 km)`
          : "";
      setError(`We don't deliver to your location yet.${dist}`);
      return;
    }
      ...
      <DashboardLayout allowedRole="CUSTOMER">
        <div className="max-w-lg mx-auto py-10 text-center">
          ...
          <button
            ...
            className="mt-6 rounded-xl bg-slate-950 px-5 py-3 text-sm font-black text-white hover:bg-teal-700 transition-colors"
          >
            Browse products
          </button>
        </div>
    ...
    <DashboardLayout allowedRole="CUSTOMER">
      <div className="max-w-5xl mx-auto pb-24 md:pb-8">
        <div className="flex items-center gap-3 mb-6">
          <button
            onClick={() => router.push("/shop")}
            className="grid h-9 w-9 place-items-center rounded-xl border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 transition-colors"
          >
          ...
          {quote && (
            <div className="ml-auto rounded-xl bg-teal-50 border border-teal-100 px-3 py-1.5">
              <span className="text-sm font-black text-teal-800">
                {formatINR(quote.invoice.grandTotal)}
              </span>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center gap-2 mb-6 overflow-x-auto pb-2">
          {steps.map((step, i) => (
            <React.Fragment key={step}>
              <div
                className={`flex items-center gap-2 shrink-0 rounded-xl px-3 py-2 text-xs font-black transition-colors ${
                  i <= currentStep
              ...
              {i < steps.length - 1 && (
                <div
                  className={`h-0.5 w-6 shrink-0 ${i < currentStep ? "bg-teal-600" : "bg-slate-200"}`}
                />
              )}
            </React.Fragment>
          ))}
            ...
            <section className="rounded-2xl border border-slate-100 bg-white p-5">
              <div className="flex items-center justify-between gap-3">
                ...
                <button
                  onClick={() => {
                    setEditingAddressId(null);
                    resetDraft();
                    setCreatingAddress((v) => !v);
                  }}
                  className="text-xs font-black px-3 py-2 rounded-xl border border-teal-200 bg-teal-50 text-teal-800 hover:bg-teal-100 transition-colors"
                >
              ...
              {loadingAddresses ? (
                ...
                <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {addresses.map((a) => {
                    const Icon =
                      ADDRESS_ICONS[(a.label || "").toLowerCase()] ||
                      Navigation;
                    return (
                      <button
                        key={a.id}
                      ...
                      >
                        <div className="flex items-start justify-between gap-2">
                          <div className="flex items-center gap-2">
                            <div
                              className={`grid h-8 w-8 place-items-center rounded-lg ${a.id === selectedAddressId ? "bg-teal-600 text-white" : "bg-slate-100 text-slate-500"}`}
                            >
                        ...
                        <div className="mt-1.5 inline-flex items-center gap-1.5 text-xs font-bold text-slate-500">
              ...
              {creatingAddress && (
                <div className="mt-4 rounded-2xl border border-teal-100 bg-teal-50/30 p-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        disabled={locating}
                        className="inline-flex items-center gap-2 text-xs font-black px-3 py-2 rounded-xl bg-teal-700 text-white hover:bg-teal-800 disabled:opacity-50"
                      >
                        {locating ? (
                  ...
                  <div className="mt-3">
                    <button
                      onClick={
                        editingAddressId ? saveEditedAddress : createAddress
                      }
                      disabled={savingAddress}
                      className="w-full md:w-auto px-5 py-3 rounded-xl bg-teal-700 text-white font-black hover:bg-teal-800 disabled:opacity-50 flex items-center justify-center gap-2"
                    >
                      {savingAddress && (
                        <Loader2 className="h-4 w-4 animate-spin" />
                      )}
            ...
            <section className="rounded-2xl border border-slate-100 bg-white p-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  >
                    <div
                      className={`grid h-10 w-10 place-items-center rounded-xl bg-gradient-to-br ${gradient} text-white mb-3`}
                    >
            ...
            {!quote?.serviceable && quote && (
              <div className="rounded-2xl border-2 border-amber-200 bg-amber-50 px-4 py-3 flex items-center gap-3">
                ...
                <p className="text-sm font-bold text-amber-800">
                  We don&apos;t deliver to your location yet. Try a different
                  address.
                </p>
              </div>
            )}
          ...
          <aside className="space-y-5">
            ...
            <div className="rounded-2xl border border-slate-100 bg-white p-5">
              {orderId ? (
                <div className="text-center">
                  ...
                  {paymentMethod === "ONLINE" ? (
                    <button
                      ...
                      disabled={paying}
                      className="mt-4 w-full rounded-xl bg-teal-700 py-3 text-sm font-black text-white hover:bg-teal-800 disabled:opacity-60"
                    >
                    ...
                    <button
                      ...
                      className="mt-4 w-full rounded-xl bg-slate-950 py-3 text-sm font-black text-white hover:bg-teal-700 transition-colors"
                    >
                      View my orders
                    </button>
                  )}
                ...
                <button
                  onClick={placeOrder}
                  disabled={placingOrder || !quote?.serviceable}
                  className="w-full flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-teal-700 to-teal-600 py-4 text-sm font-black text-white shadow-lg shadow-teal-900/15 transition-all hover:from-teal-800 hover:to-teal-700 hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {placingOrder ? (
                    <>
        ...
        {deletingAddressId && (
          <div className="fixed inset-0 bg-slate-950/50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl max-w-sm w-full shadow-2xl p-6">
              <div className="text-center">
                <div className="grid h-12 w-12 place-items-center rounded-full bg-red-100 text-red-600 mx-auto">
              ...
              <div className="mt-5 flex gap-3">
                <button
                  ...
                  className="flex-1 px-4 py-2.5 bg-slate-100 text-slate-700 rounded-xl font-black hover:bg-slate-200"
                >
                  Cancel
                </button>
                <button
                  onClick={confirmDeleteAddress}
                  className="flex-1 px-4 py-2.5 bg-red-600 text-white rounded-xl font-black hover:bg-red-700"
                >
                  Delete
                </button>
              </div>
  ...
  return (
    <label className={className}>
      ...
      <input
        ...
        className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-900 placeholder:text-slate-400 focus:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-500"
      />
    </label>
  );
}
