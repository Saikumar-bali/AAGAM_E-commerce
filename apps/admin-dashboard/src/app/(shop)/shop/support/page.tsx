...
export default function CustomerSupportPage() {
  ...
  const loadTicketHistory = async (orderId: string) => {
    if (!orderId) {
      setTickets([]);
      return;
    }
  ...
  const submitTicket = async () => {
    ...
    try {
      ...
      await loadTicketHistory(selectedOrderId);
    } catch {
      ...
      <div className="mx-auto max-w-5xl space-y-6 pb-8">
        <section className="relative overflow-hidden rounded-[2rem] bg-slate-950 px-5 py-7 text-white shadow-[0_24px_80px_rgba(15,23,42,0.24)] sm:px-8 sm:py-9">
          ...
          <div className="relative flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/10 p-4 backdrop-blur">
              ...
              <div><p className="font-black">Verified account</p><p className="text-xs font-semibold text-slate-300">Only your orders are available here.</p></div>
            </div>
          </div>
        </section>
        ...
        {loadingOrders ? (
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm">
            ...
            <h2 className="mt-4 text-xl font-black text-slate-950">No orders available for support</h2>
            <p className="mx-auto mt-2 max-w-lg text-sm font-semibold leading-6 text-slate-500">Customer tickets are linked to an order so the support team can see the right store, payment and delivery context.</p>
          </section>
        ) : (
          <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
            <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
              <div className="flex items-center justify-between gap-3">
                <div><p className="text-xs font-black uppercase tracking-[0.16em] text-teal-700">Step 1</p><h2 className="mt-2 text-xl font-black text-slate-950">Select an order</h2></div>
                <button type="button" onClick={() => void loadOrders()} className="grid h-10 w-10 place-items-center rounded-xl border border-slate-200 text-slate-500 transition hover:border-teal-300 hover:text-teal-700" aria-label="Refresh orders"><RefreshCw className="h-4 w-4" /></button>
              </div>
              <div className="mt-5 max-h-[430px] space-y-2 overflow-y-auto pr-1">
                {orders.map((order) => {
                  const selected = order.id === selectedOrderId;
                  return <button key={order.id} type="button" onClick={() => setSelectedOrderId(order.id)} className={`w-full rounded-2xl border p-4 text-left transition ${selected ? 'border-teal-500 bg-teal-50 shadow-sm' : 'border-slate-200 bg-white hover:border-teal-200 hover:bg-slate-50'}`}>
                    <div className="flex items-start justify-between gap-3"><div><p className="font-black text-slate-950">Order #{order.id.slice(-8).toUpperCase()}</p><p className="mt-1 text-xs font-semibold text-slate-500">{order.store?.name || 'Aagaam store'} · {new Date(order.createdAt).toLocaleDateString('en-IN')}</p></div><ChevronRight className={`h-5 w-5 ${selected ? 'text-teal-700' : 'text-slate-300'}`} /></div>
                    <div className="mt-3 flex items-center justify-between text-xs font-black"><span className="rounded-full bg-slate-100 px-2.5 py-1 text-slate-600">{order.status}</span><span className="text-slate-900">₹{Number(order.grandTotal ?? order.totalAmount ?? 0).toLocaleString('en-IN')}</span></div>
                  </button>;
                })}
              </div>
            </section>
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
              <p className="text-xs font-black uppercase tracking-[0.16em] text-teal-700">Step 2</p>
              <h2 className="mt-2 text-xl font-black text-slate-950">Tell us what happened</h2>
