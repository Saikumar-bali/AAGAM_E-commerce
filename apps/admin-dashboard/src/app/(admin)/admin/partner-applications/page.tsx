...
function ReviewWorkspace(props: any) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/65 p-3 backdrop-blur-sm sm:p-6" role="dialog" aria-modal="true" aria-label={`Review ${application.applicantName}`} onMouseDown={(event) => { if (event.currentTarget === event.target) props.close(); }}><div className="max-h-[92vh] w-full max-w-5xl overflow-y-auto rounded-3xl bg-white shadow-2xl"><header className="sticky top-0 z-20 flex justify-between border-b bg-white/95 p-5"><div><div className="flex gap-2"><span className={`rounded-full border px-2.5 py-1 text-[10px] font-black ${badge(application.status)}`}>{label(application.status)}</span><span className="text-[10px] font-black text-slate-400">{application.type}</span></div><h2 className="mt-3 text-2xl font-black">{application.applicantName}</h2><p className="font-mono text-xs text-slate-400">{application.applicationNumber}</p></div><button onClick={props.close} aria-label="Close application review"><XCircle className="h-5 w-5" /></button></header><div className="space-y-5 p-5">
    ...
    {deletable ? <Panel title={application.deletedAt ? 'Deleted application' : 'Delete draft'} subtitle={application.deletedAt ? 'Restore it or permanently remove it. Permanent deletion cannot be undone.' : 'Deletion is recoverable for 14 days.'}>{application.deletedAt ? <><Info label="Deletion reason" value={application.deletionReason || 'Not recorded'} /><Info label="Scheduled purge" value={date(application.scheduledPurgeAt)} /><button onClick={props.restoreDraft} disabled={Boolean(props.actionLoading)} className="inline-flex w-full justify-center gap-2 rounded-xl bg-slate-900 px-4 py-3 font-black text-white disabled:opacity-50"><RotateCcw className="h-4 w-4" /> Restore draft</button><div className="rounded-xl border border-red-200 bg-red-50 p-3 text-xs font-bold leading-5 text-red-800">Permanent delete removes this application, its onboarding documents, events and activation tokens. Provisioned Rider/Store accounts are blocked from this action.</div><button onClick={props.permanentlyDelete} disabled={Boolean(props.actionLoading)} className="inline-flex w-full justify-center gap-2 rounded-xl bg-red-800 px-4 py-3 font-black text-white disabled:opacity-50">{props.actionLoading === 'permanent-delete' ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />} Permanently delete</button></> : <><textarea value={props.deleteReason} onChange={(event) => props.setDeleteReason(event.target.value)} rows={2} placeholder="Required deletion reason" className="w-full rounded-xl border border-red-200 px-3 py-2.5" /><button onClick={props.deleteDraft} disabled={props.deleteReason.trim().length < 5 || props.actionLoading === 'delete'} className="inline-flex w-full justify-center gap-2 rounded-xl bg-red-700 px-4 py-3 font-black text-white disabled:opacity-50">{props.actionLoading === 'delete' ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />} Delete draft</button></>}</Panel> : null}
