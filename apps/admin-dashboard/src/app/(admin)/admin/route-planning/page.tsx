...
export default function RegionalRoutePlanningPage() {
  ...
  return <DashboardLayout allowedRole="ADMIN"><div className="space-y-6">
    <header className="overflow-hidden rounded-[2rem] bg-gradient-to-br from-emerald-950 via-emerald-900 to-teal-700 p-6 text-white ">
      <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[.24em] text-emerald-200">Live regional operations</p><h1 className="mt-2 text-3xl font-semibold sm:text-4xl">Region & Route Planning</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-emerald-100">Resolve geographic zones, split by operational feasibility, assign eligible nearby riders, and preserve one audited owner for every stop and cash ledger.</p></div><div className="flex flex-wrap gap-2"><input type="date" value={date} onChange={(event) => setDate(event.target.value)} className="min-h-11 rounded-xl border border-white/20 bg-white/10 px-3 font-bold text-white [color-scheme:dark]"/><button onClick={() => void load()} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-white/15 px-4 font-semibold"><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`}/>Refresh</button><button disabled={working === 'plan'} onClick={plan} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-white px-4 font-semibold text-emerald-800 disabled:opacity-60"><Play className="h-4 w-4"/>{working === 'plan' ? 'Planning…' : 'Run planner'}</button></div></div>
    ...
    <section><div className="mb-3 flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wider text-emerald-700">Regional readiness</p><h2 className="mt-1 text-2xl font-semibold text-slate-950">Zone summaries</h2></div><ZoneManagementPanel zones={data?.zones || []} stores={data?.stores || []} riders={data?.riders || []} onSaved={load}/></div><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{data?.zones.length ? data.zones.map((zone) => <ZoneCard key={zone.id} zone={zone}/>) : <Empty title="No geographic zones" text="Create active polygon or radius zones before planning subscription routes."/>}</div></section>

    <section className="grid gap-5 2xl:grid-cols-[minmax(0,1.35fr)_minmax(420px,.65fr)]">
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white "><div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 p-5"><div><p className="text-xs font-semibold uppercase tracking-wider text-emerald-700">Map and list split view</p><h2 className="mt-1 text-xl font-semibold text-slate-950">Zones, stores, riders & route stops</h2></div><div className="flex gap-3 text-[11px] font-bold text-slate-500"><Legend label="Store" className="bg-slate-950"/><Legend label="Rider" className="bg-blue-600"/><Legend label="Stop" className="bg-emerald-600"/></div></div><RegionalMap data={data}/></div>
      <div className="space-y-3"><div className="flex items-center justify-between"><h2 className="text-xl font-semibold text-slate-950">Planned delivery runs</h2><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">{data?.runs.length || 0}</span></div>{data?.runs.length ? data.runs.map((run) => <button key={run.id} onClick={() => { setSelectedRunId(run.id); setDrawerOpen(true); }} className={`w-full rounded-xl border bg-white p-5 text-left  transition ${selectedRunId === run.id ? 'border-emerald-400 ring-2 ring-emerald-100' : 'border-slate-200 hover:border-emerald-200'}`}><div className="flex items-start gap-3"><span className="grid h-11 w-11 place-items-center rounded-xl bg-emerald-100 text-emerald-700"><Route className="h-5 w-5"/></span><span className="min-w-0 flex-1"><span className="flex flex-wrap items-center gap-2"><strong className="text-slate-950">{run.routeCode}</strong><StatusChip status={run.status}/></span><span className="mt-1 block truncate text-xs text-slate-500">{run.deliveryZone?.name || 'Zone unresolved'} · {run.store.name}</span></span><ChevronRight className="h-5 w-5 text-slate-400"/></div><div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-5"><Metric label="Stops" value={String(run.totalStopCount)}/><Metric label="Weight" value={`${(Number(run.expectedWeightGrams || 0)/1000).toFixed(1)} kg`}/><Metric label="Distance" value={`${Number(run.estimatedDistanceKm || 0).toFixed(1)} km`}/><Metric label="ETA" value={`${run.estimatedDurationMinutes || 0} min`}/><Metric label="Cash" value={money(run.expectedCashPaise)}/></div><p className="mt-3 truncate text-xs font-bold text-slate-500">{run.rider?.user.name ? `Rider: ${run.rider.user.name}` : 'Rider needed'}{run.assignmentReasonSummary ? ` · ${run.assignmentReasonSummary}` : ''}</p></button>) : <Empty title="No planned routes" text="Run the regional planner after subscription orders are generated."/>}</div>
    </section>

    <section className="grid gap-5 xl:grid-cols-2"><div className="rounded-xl border border-slate-200 bg-white p-5"><h2 className="text-xl font-semibold text-slate-950">Unassigned delivery orders</h2><p className="mt-1 text-sm text-slate-500">Orders remain visible instead of silently overloading the first available rider.</p><div className="mt-4 space-y-2">{data?.unassigned.length ? data.unassigned.map((item) => <div key={item.id} className="flex items-center gap-3 rounded-xl bg-amber-50 p-3"><AlertTriangle className="h-5 w-5 text-amber-700"/><div className="min-w-0"><p className="truncate text-sm font-semibold text-amber-950">{item.order?.id || item.id}</p><p className="text-xs text-amber-800">{item.deliveryZone?.name || 'Zone unresolved'}</p></div></div>) : <div className="rounded-xl bg-emerald-50 p-5 text-center text-sm font-bold text-emerald-800">All generated deliveries have one route owner.</div>}</div></div><div className="rounded-xl border border-slate-200 bg-white p-5"><h2 className="text-xl font-semibold text-slate-950">Recent route events</h2><div className="mt-4 max-h-80 space-y-2 overflow-y-auto">{data?.recentEvents.length ? data.recentEvents.slice(0, 20).map((event) => <div key={event.id} className="flex gap-3 rounded-xl bg-slate-50 p-3"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-700"/><div><p className="text-xs font-semibold text-slate-800">{title(event.eventType)}</p><p className="mt-1 text-[11px] text-slate-500">{new Date(event.createdAt).toLocaleString('en-IN')}</p></div></div>) : <p className="rounded-xl bg-slate-50 p-5 text-center text-sm text-slate-500">No route changes in the past 24 hours.</p>}</div></div></section>

    {drawerOpen && selectedRun ? <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/55"><button aria-label="Close route drawer" onClick={() => setDrawerOpen(false)} className="absolute inset-0"/><aside className="relative h-full w-full max-w-2xl overflow-y-auto bg-slate-50 p-5 "><div className="flex items-start justify-between"><div><p className="text-xs font-semibold uppercase tracking-wider text-emerald-700">Controlled route management</p><h2 className="mt-1 text-2xl font-semibold text-slate-950">{selectedRun.routeCode}</h2><p className="mt-1 text-sm text-slate-500">{selectedRun.deliveryZone?.name || 'Zone unresolved'} · version {selectedRun.version}</p></div><button onClick={() => setDrawerOpen(false)} className="grid h-11 w-11 place-items-center rounded-xl bg-white"><X className="h-5 w-5"/></button></div>
      <div className="mt-5 grid grid-cols-2 gap-2 sm:grid-cols-5"><Metric label="Stops" value={String(selectedRun.totalStopCount)}/><Metric label="Weight" value={`${(Number(selectedRun.expectedWeightGrams||0)/1000).toFixed(1)} kg`}/><Metric label="Distance" value={`${selectedRun.estimatedDistanceKm.toFixed(1)} km`}/><Metric label="Duration" value={`${selectedRun.estimatedDurationMinutes} min`}/><Metric label="Cash" value={money(selectedRun.expectedCashPaise)}/></div>
      <label className="mt-5 block text-xs font-semibold uppercase tracking-wide text-slate-500">Required operational reason<textarea value={reason} onChange={(event) => setReason(event.target.value)} rows={2} className="mt-2 w-full rounded-xl border border-slate-200 bg-white p-3 text-sm normal-case tracking-normal text-slate-800"/></label>
      <section className="mt-5 rounded-xl border border-slate-200 bg-white p-5"><div className="flex items-center gap-2"><Scissors className="h-5 w-5 text-emerald-700"/><h3 className="font-semibold text-slate-950">Split run</h3></div><div className="mt-3 grid gap-3 sm:grid-cols-2"><select value={splitMethod} onChange={(event) => { setSplitMethod(event.target.value); setPreview(null); }} className="min-h-12 rounded-xl border border-slate-200 px-3 font-bold"><option value="AUTOMATIC_GEOGRAPHIC">Automatic geographic split</option><option value="SELECTED_STOPS">Selected stops</option><option value="MAX_STOPS">Maximum-stop split</option><option value="TIME_CAPACITY">Time-capacity split</option></select>{splitMethod === 'MAX_STOPS' ? <input value={maximumStops} onChange={(event) => setMaximumStops(event.target.value.replace(/\D/g, ''))} inputMode="numeric" placeholder="Maximum stops" className="min-h-12 rounded-xl border border-slate-200 px-3"/> : null}</div>{splitMethod === 'SELECTED_STOPS' ? <div className="mt-3 grid gap-2 sm:grid-cols-2">{selectedRun.stops.map((stop) => <label key={stop.id} className="flex items-center gap-2 rounded-xl bg-slate-50 p-3 text-sm font-bold"><input type="checkbox" checked={selectedStopIds.includes(stop.id)} onChange={(event) => setSelectedStopIds((current) => event.target.checked ? [...current, stop.id] : current.filter((id) => id !== stop.id))}/>Stop {stop.sequenceNumber} · {stop.deliveryJob.order.customer?.name || 'Customer'}</label>)}</div> : null}<div className="mt-3 flex gap-2"><button disabled={!editable(selectedRun) || working === 'preview'} onClick={() => void previewSplit()} className="min-h-11 flex-1 rounded-xl border border-emerald-300 bg-emerald-50 font-semibold text-emerald-800 disabled:opacity-50">{working === 'preview' ? 'Calculating…' : 'Preview split'}</button>{preview ? <button disabled={working === 'split'} onClick={applySplit} className="min-h-11 flex-1 rounded-xl bg-emerald-700 font-semibold text-white">Confirm split</button> : null}</div>{preview ? <div className="mt-4 grid gap-2 sm:grid-cols-2">{preview.resultingRuns.map((run) => <div key={run.index} className="rounded-xl bg-emerald-50 p-3"><p className="font-semibold text-emerald-900">Proposed run {run.index}</p><p className="mt-1 text-xs text-emerald-800">{run.stopCount} stops · {((run.expectedWeightGrams||0)/1000).toFixed(1)} kg · {run.estimatedDistanceKm.toFixed(1)} km · {run.estimatedDurationMinutes} min · {money(run.expectedCashPaise)}</p></div>)}</div> : null}</section>
      <section className="mt-4 rounded-xl border border-slate-200 bg-white p-5"><div className="flex items-center gap-2"><Bike className="h-5 w-5 text-blue-700"/><h3 className="font-semibold text-slate-950">Assign or reassign rider</h3></div><select value={riderId} onChange={(event) => setRiderId(event.target.value)} className="mt-3 min-h-12 w-full rounded-xl border border-slate-200 px-3 font-bold"><option value="">Select eligible rider</option>{data?.riders.map((rider) => <option key={rider.id} value={rider.id}>{rider.user.name || rider.id} · {rider.status}{rider.homeZone?.name ? ` · ${rider.homeZone.name}` : ''}</option>)}</select><button disabled={!editable(selectedRun) || !riderId || working === 'reassign'} onClick={reassign} className="mt-3 min-h-11 w-full rounded-xl bg-blue-700 font-semibold text-white disabled:opacity-50">Validate eligibility & assign</button></section>
      <section className="mt-4 rounded-xl border border-slate-200 bg-white p-5"><div className="flex items-center gap-2"><GitMerge className="h-5 w-5 text-violet-700"/><h3 className="font-semibold text-slate-950">Merge compatible runs</h3></div><div className="mt-3 space-y-2">{compatibleRuns.length ? compatibleRuns.map((run) => <label key={run.id} className="flex items-center gap-3 rounded-xl bg-slate-50 p-3 text-sm font-bold"><input type="checkbox" checked={selectedMergeIds.includes(run.id)} onChange={(event) => setSelectedMergeIds((current) => event.target.checked ? [...current, run.id] : current.filter((id) => id !== run.id))}/><span>{run.routeCode} · {run.totalStopCount} stops · {money(run.expectedCashPaise)}</span></label>) : <p className="text-sm text-slate-500">No compatible unstarted runs.</p>}</div><button disabled={!selectedMergeIds.length || working === 'merge'} onClick={merge} className="mt-3 min-h-11 w-full rounded-xl bg-violet-700 font-semibold text-white disabled:opacity-50">Merge selected routes</button></section>
      <section className="mt-4 rounded-xl border border-slate-200 bg-white p-5"><div className="flex items-center gap-2"><GitPullRequestArrow className="h-5 w-5 text-amber-700"/><h3 className="font-semibold text-slate-950">Move one pending stop</h3></div><div className="mt-3 grid gap-2 sm:grid-cols-2"><select value={selectedStopId} onChange={(event) => setSelectedStopId(event.target.value)} className="min-h-12 rounded-xl border border-slate-200 px-3 font-bold"><option value="">Select pending stop</option>{selectedRun.stops.filter((stop) => ['PLANNED','READY','RETRY_PENDING'].includes(stop.status)).map((stop) => <option key={stop.id} value={stop.id}>Stop {stop.sequenceNumber} · {stop.deliveryJob.order.customer?.name || 'Customer'}</option>)}</select><select value={destinationRunId} onChange={(event) => setDestinationRunId(event.target.value)} className="min-h-12 rounded-xl border border-slate-200 px-3 font-bold"><option value="">Destination run</option>{compatibleRuns.map((run) => <option key={run.id} value={run.id}>{run.routeCode}</option>)}</select></div><button disabled={!selectedStopId || !destinationRunId || working === 'move'} onClick={moveStop} className="mt-3 min-h-11 w-full rounded-xl bg-amber-700 font-semibold text-white disabled:opacity-50">Review capacity & move stop</button></section>
      <div className="mt-4"><RunStopOrderEditor run={selectedRun} reason={reason} disabled={!editable(selectedRun)} onSaved={load}/></div>
    <section className="mt-4 grid gap-3 sm:grid-cols-2"><button disabled={!editable(selectedRun) || working === 'reorder'} onClick={reorder} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border border-slate-300 bg-white font-semibold text-slate-800 disabled:opacity-50"><ArrowDownUp className="h-5 w-5"/>Reorder pending stops</button>{['READY_FOR_PICKUP','PICKED_UP','IN_PROGRESS'].includes(selectedRun.status) ? <button disabled={working === 'interrupt'} onClick={interrupt} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-red-700 font-semibold text-white"><ShieldAlert className="h-5 w-5"/>Interrupt & recover</button> : <button disabled={!editable(selectedRun) || working === 'cancel'} onClick={cancel} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-red-700 font-semibold text-white disabled:opacity-50"><X className="h-5 w-5"/>Cancel unstarted run</button>}</section>
    </aside></div> : null}
  </div></DashboardLayout>;
}
...
function RegionalMap({ data }: { data: Dashboard | null }) {
  ...
  if (!points.length && !polygons.length) return <div className="grid min-h-[430px] place-items-center bg-slate-50 p-8 text-center"><div><Map className="mx-auto h-12 w-12 text-slate-300"/><p className="mt-3 font-semibold text-slate-800">No geographic route data</p><p className="mt-1 text-sm text-slate-500">Resolved zones and generated routes will appear here.</p></div></div>;
  return <div className="overflow-x-auto bg-slate-50 p-3"><svg viewBox="0 0 1000 520" className="min-h-[430px] min-w-[720px] rounded-xl bg-[linear-gradient(135deg,#f8fafc,#ecfdf5)]" role="img" aria-label="Regional delivery routes map">
...
function HeroMetric({ label, value, icon }: { label: string; value: string; icon: React.ReactNode }) { return <div className="rounded-xl bg-white/10 p-4"><div className="flex items-center gap-2 text-emerald-100 [&>svg]:h-4 [&>svg]:w-4">{icon}<p className="text-xs font-bold">{label}</p></div><p className="mt-2 truncate text-xl font-semibold">{value}</p></div>; }
function Metric({ label, value }: { label: string; value: string }) { return <div className="rounded-xl bg-slate-50 p-2 text-center"><p className="truncate text-sm font-semibold text-slate-900">{value}</p><p className="mt-1 text-[9px] font-semibold uppercase tracking-wide text-slate-400">{label}</p></div>; }
...
function StatusChip({ status }: { status: string }) { const risk = ['RIDER_NEEDED','RECOVERY_REQUIRED','INTERRUPTED'].includes(status); return <span className={`rounded-full px-2.5 py-1 text-[9px] font-semibold ${risk ? 'bg-amber-100 text-amber-800' : status === 'COMPLETED' ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600'}`}>{title(status)}</span>; }
function ZoneCard({ zone }: { zone: Zone }) { const warning = ['RIDER_NEEDED','CASH_LIMIT_RISK','CAPACITY_RISK','SLOT_RISK'].includes(zone.status || ''); return <article className={`rounded-xl border bg-white p-5  ${warning ? 'border-amber-300' : 'border-slate-200'}`}><div className="flex items-start gap-3"><span className="grid h-11 w-11 place-items-center rounded-xl bg-emerald-100 text-emerald-700"><MapPin className="h-5 w-5"/></span><div className="min-w-0 flex-1"><p className="text-xs font-semibold uppercase tracking-wider text-emerald-700">{zone.code}</p><h3 className="mt-1 text-lg font-semibold text-slate-950">{zone.name}</h3></div><StatusChip status={zone.status || 'READY'}/></div><div className="mt-4 grid grid-cols-4 gap-2"><Metric label="Deliveries" value={String(zone.deliveryCount || 0)}/><Metric label="Riders" value={String(zone.availableRiderCount || 0)}/><Metric label="Duration" value={`${zone.estimatedDurationMinutes || 0} min`}/><Metric label="Cash" value={money(zone.expectedCashPaise || 0)}/></div><p className="mt-3 text-xs text-slate-500">Limits: {zone.maximumStopsPerRun} stops · {zone.maximumWeightKg ? `${zone.maximumWeightKg} kg · ` : ''}{zone.maximumRouteDistanceKm} km · {zone.maximumEstimatedDurationMinutes} min · {zone.slotEndBufferMinutes ?? 0} min buffer · {money(zone.cashRiskLimitPaise)}{zone.timezone ? ` · ${zone.timezone}` : ''}</p></article>; }
function Empty({ title: heading, text }: { title: string; text: string }) { return <div className="rounded-xl border border-dashed border-slate-300 bg-white p-8 text-center"><Route className="mx-auto h-10 w-10 text-slate-300"/><p className="mt-3 font-semibold text-slate-900">{heading}</p><p className="mt-1 text-sm text-slate-500">{text}</p></div>; }
