              ...
              columns={[
                ...
                { key: 'actions', header: 'Actions', align: 'center' as const, render: (c) => (
                  <div className="flex items-center justify-center gap-1">
                    ...
                    {c.effectiveStatus === "ACTIVE" || c.effectiveStatus === "SCHEDULED" ? (
                      <button onClick={() => setCampaignStatus(c, "PAUSED")} className="grid h-8 w-8 place-items-center rounded-xl bg-amber-50 text-amber-700" title="Pause"><Pause className="h-3.5 w-3.5" /></button>
                    ) : (
                      <button onClick={() => { if (c.effectiveStatus === "EXPIRED") { openCampaign(c); setError("Update the campaign end time before republishing it to customers."); return; } setCampaignStatus(c, c.startsAt && new Date(c.startsAt) > new Date() ? "SCHEDULED" : "ACTIVE"); }} className="grid h-8 w-8 place-items-center rounded-xl bg-emerald-50 text-emerald-700" title={c.effectiveStatus === "EXPIRED" ? "Edit schedule to republish" : "Publish"}><Play className="h-3.5 w-3.5" /></button>
                    )}
                    <button onClick={() => archive("campaigns", c.id)} className="grid h-8 w-8 place-items-center rounded-xl bg-slate-100 text-slate-500" title="Archive"><Archive className="h-3.5 w-3.5" /></button>
                  </div>
                ) },
              ]}
              data={campaigns}
              keyExtractor={(row) => row.id}
              emptyText="No campaigns yet. Upload a finished hero banner and publish it from this portal."
              loading={loading}
