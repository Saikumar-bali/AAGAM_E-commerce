...
export default function AdminProductsPage() {
  ...
  const handleSaveCategory = async (name: string, editingCat: Category | null) => {
    setSubmitting(true);
    setError('');
    try {
      const cleanName = name.trim().replace(/\s+/g, ' ');
      if (cleanName.length < 2) throw new Error('Category name must be at least 2 characters.');
      const payload = { name: cleanName, imageUrl: editingCat?.imageUrl || null };
      if (editingCat) await apiClient.patch(`/products/categories/${editingCat.id}`, payload);
  ...
  const handleReorderCategories = async (next: Category[]) => {
    setCategories(next);
    try {
      await apiClient.patch('/admin/categories/reorder', { ids: next.map((item) => item.id) });
      setMessage('Category order updated.');
    } catch (err: any) {
      setError(err?.response?.data?.message || err?.message || 'Failed to update category order.');
    }
  };
