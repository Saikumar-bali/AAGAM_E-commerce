...
export default function AdminProductsPage() {
  ...
  return (
    <DashboardLayout allowedRole="ADMIN">
      ...
      {categoryDialogOpen && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4"><div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-3xl bg-white shadow-2xl"><div className="flex items-center justify-between border-b border-gray-100 p-6"><div><h2 className="text-xl font-black text-gray-950">Manage Categories</h2><p className="text-sm font-semibold text-gray-500">Create categories and drag them into the exact customer-facing order.</p></div><button onClick={() => setCategoryDialogOpen(false)} className="rounded-lg p-2 hover:bg-gray-100"><X className="h-5 w-5" /></button></div><div className="grid gap-5 p-6 md:grid-cols-[1fr_1.1fr]"><form onSubmit={saveCategory} className="rounded-2xl border border-gray-100 bg-gray-50 p-4"><h3 className="text-sm font-black uppercase tracking-wider text-gray-500">{editingCategory ? 'Edit category' : 'Create category'}</h3>{error && <div className="mt-3 rounded-xl bg-red-50 px-3 py-2 text-sm font-bold text-red-700">{error}</div>}<label className="mt-4 block text-sm font-bold text-gray-700">Category name<input required value={categoryName} onChange={(e) => setCategoryName(e.target.value)} className="mt-1 w-full rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-gray-900 focus:ring-2 focus:ring-teal-500" /></label><div className="mt-4 flex gap-2"><button type="button" onClick={() => { setEditingCategory(null); setCategoryName(''); setError(''); }} className="flex-1 rounded-xl bg-white px-4 py-3 font-black text-gray-700">Clear</button><button disabled={submitting} className="flex-1 rounded-xl bg-teal-700 px-4 py-3 font-black text-white disabled:opacity-50">{submitting ? 'Saving...' : editingCategory ? 'Save' : 'Create'}</button></div></form><div className="space-y-2"><h3 className="text-sm font-black uppercase tracking-wider text-gray-500">Customer order</h3>{categories.length === 0 ? <p className="rounded-2xl border border-dashed p-4 text-sm font-semibold text-gray-500">No categories yet.</p> : <SortableCategories categories={categories} onReorder={reorderCategories} onEdit={(category) => { setEditingCategory(category); setDeletingCategory(null); setCategoryName(category.name); setError(''); }} onDelete={setDeletingCategory} productCounts={categoryCounts} />}</div></div>{deletingCategory && <div className="mx-6 mb-6 rounded-2xl border border-red-100 bg-red-50 p-4"><p className="font-black text-red-800">Delete {deletingCategory.name}?</p><p className="mt-1 text-sm font-semibold text-red-700">This is allowed only when the category has no products.</p><div className="mt-4 flex gap-2"><button onClick={() => setDeletingCategory(null)} className="flex-1 rounded-xl bg-white px-4 py-2.5 font-black text-red-700">Cancel</button><button onClick={deleteCategory} disabled={submitting} className="flex-1 rounded-xl bg-red-600 px-4 py-2.5 font-black text-white disabled:opacity-50">Delete</button></div></div>}</div></div>}
