  ...
  return (
    <DashboardLayout allowedRole="ADMIN">
      <div className="space-y-6 pb-24 lg:pb-10">
        <header className="rounded-[2rem] border border-slate-200 bg-white p-6 ">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
            <div>
              <p className="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-emerald-800"><Scale className="h-3.5 w-3.5" /> Routing data</p>
              <h1 className="mt-3 text-3xl font-semibold tracking-tight text-slate-950">Product routing weights</h1>
            ...
            <button onClick={() => void load()} disabled={loading} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700 hover:border-emerald-300 hover:text-emerald-800 disabled:opacity-50">
        ...
        {stats.missing > 0 ? (
          <div className="flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4 text-amber-950">
            ...
            <div>
              <p className="font-semibold">Subscriptions remain blocked for products without routing weight.</p>
          ...
          <div className="flex items-center gap-3 rounded-xl border border-emerald-200 bg-emerald-50 p-4 font-bold text-emerald-900"><CheckCircle2 className="h-5 w-5" /> Every catalog product has an authoritative routing weight.</div>
        )}
        ...
        <section className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white ">
          <div className="flex flex-col gap-3 border-b border-slate-100 p-4 lg:flex-row lg:items-center">
            ...
            <select aria-label="Routing weight filter" value={filter} onChange={(event) => setFilter(event.target.value as Filter)} className="rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-700"><option value="all">All · missing first</option><option value="missing">Missing only</option><option value="ready">Routing-ready only</option></select>
          </div>
              ...
              return (
                <article key={product.id} data-testid={`routing-weight-${product.id}`} className="grid gap-4 p-5 lg:grid-cols-[minmax(0,1fr)_220px_290px] lg:items-center">
                  <div className="flex min-w-0 items-center gap-4">
                    <div className="grid h-14 w-14 shrink-0 place-items-center overflow-hidden rounded-xl border border-slate-200 bg-slate-50">{product.image ? <img src={product.image} alt="" className="h-full w-full object-cover" /> : <Scale className="h-5 w-5 text-slate-400" />}</div>
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2"><h2 className="truncate font-semibold text-slate-950">{product.name}</h2><span className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${hasWeight ? 'bg-emerald-50 text-emerald-800' : 'bg-amber-100 text-amber-900'}`}>{hasWeight ? 'Routing-ready' : 'Weight required'}</span>{product.isActive === false ? <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-semibold text-slate-600">Inactive</span> : null}</div>
                      <p className="mt-1 truncate text-xs font-semibold text-slate-500">{product.category?.name || 'Uncategorized'} · {product.id}</p>
                    </div>
                  </div>

                  <div className="rounded-xl bg-slate-50 px-4 py-3">
                    <p className="text-[10px] font-semibold uppercase tracking-[0.15em] text-slate-500">Customer display weight</p>
                    <p className="mt-1 font-semibold text-slate-900">{displayPackWeight(product)}</p>
                    <p className="mt-1 text-[11px] font-semibold text-slate-500">Informational only; not used by routing.</p>
                  </div>

                  <div className="flex items-end gap-2">
                    <label className="min-w-0 flex-1 text-xs font-semibold uppercase tracking-wide text-slate-500">Routing unit weight (grams)<input aria-label={`Routing weight for ${product.name}`} inputMode="numeric" type="number" min={1} step={1} value={drafts[product.id] ?? ''} onChange={(event) => setDrafts((current) => ({ ...current, [product.id]: event.target.value.replace(/[^0-9]/g, '') }))} className="mt-1.5 min-h-11 w-full rounded-xl border border-slate-200 px-3 text-base font-semibold normal-case tracking-normal text-slate-950 focus:ring-2 focus:ring-emerald-500" /></label>
                    <button onClick={() => void saveWeight(product)} disabled={saving[product.id]} className="inline-flex min-h-11 min-w-24 items-center justify-center rounded-xl bg-emerald-700 px-4 text-sm font-semibold text-white hover:bg-emerald-800 disabled:opacity-50">{saving[product.id] ? <Loader2 className="h-4 w-4 animate-spin" /> : saved[product.id] ? 'Saved' : 'Save'}</button>
                  </div>
                </article>
              );
            })}
...
function Metric({ label, value, tone }: { label: string; value: number; tone: 'slate' | 'emerald' | 'amber' }) {
  ...
  return <div className={`rounded-xl border p-5  ${classes}`}><p className="text-xs font-semibold uppercase tracking-[0.14em] opacity-65">{label}</p><p className="mt-2 text-3xl font-semibold">{value}</p></div>;
}
