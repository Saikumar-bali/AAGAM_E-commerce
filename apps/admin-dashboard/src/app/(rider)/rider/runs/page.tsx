  ...
  return <DashboardLayout allowedRole="RIDER"><div className="space-y-6">
    ...
    <div className="grid gap-5 xl:grid-cols-[360px_1fr]">
      ...
      <main>{activeRun ? <div className="space-y-5">
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-black uppercase tracking-wider text-emerald-700">{activeRun.routeCode}</p><h2 className="mt-1 text-2xl font-black text-slate-950">{activeRun.store.name}</h2><p className="mt-1 flex items-center gap-2 text-sm text-slate-500"><Store className="h-4 w-4" />{activeRun.store.address}</p></div><span className="rounded-full bg-emerald-50 px-3 py-1.5 text-xs font-black text-emerald-800 ring-1 ring-emerald-200">{title(activeRun.status)}</span></div><div className="mt-5 flex items-center justify-between text-sm font-bold"><span>{activeRun.completedStopCount} of {activeRun.totalStopCount} complete</span><span className="text-emerald-700">{progress}%</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-emerald-500" style={{ width: `${progress}%` }} /></div><div className="mt-5 grid gap-3 sm:grid-cols-3"><Metric label="Retry pending" value={String(activeRun.retryPendingStopCount)} /><Metric label="Expected cash" value={money(activeRun.expectedCashPaise)} /><Metric label="Rider holding" value={money(Math.max(0, activeRun.collectedCashPaise - activeRun.depositedCashPaise))} /></div>
