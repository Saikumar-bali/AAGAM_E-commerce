...
async function run() {
    ...
    const api = async (path, init) => page.evaluate(async ({ p, i }) => {
    ...
    if (b && Array.isArray(b.riders)) {
        console.log('board riders:', JSON.stringify(b.riders.map((r) => ({ name: r.user?.name, status: r.status, activeJobCount: r.activeJobCount }))));
        console.log('waitingJobs:', (b.waitingJobs || []).map((w) => ({ id: w.id.slice(0, 8).toUpperCase(), status: w.status })));
    }
    await browser.close();
    console.log('\n=== DONE ===');
}
