...
async function main() {
    ...
    const snap = async (label) => {
        ...
        const me = rows.find((r) => r?.user?.phone === '+919874561230' || r?.user?.name === 'saikumarbali');
        if (me)
            console.log(`[HEARTBEAT ${label}] status=${me.status} updatedAt=${me.updatedAt} lat=${me.latitude} lng=${me.longitude}`);
        else
            console.log(`[HEARTBEAT ${label}] rider not found in ${rows.length} rows`);
    };
    await snap('T0');
