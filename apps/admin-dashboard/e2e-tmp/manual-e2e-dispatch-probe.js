...
async function main() {
    ...
    page.on('response', async (res) => {
        if (res.url().includes('/orders/dispatch/board')) {
            try {
                ...
                console.log('[BOARD] riders:', JSON.stringify(j.riders?.map((r) => ({
                ...
                console.log('[BOARD] openOffers:', (j.openOffers || []).map((o) => ({
                    job: o.deliveryJobId?.slice(0, 8).toUpperCase(),
                    rider: o.riderProfile?.userId?.slice(0, 8),
                    status: o.status,
                })));
                console.log('[BOARD] waitingJobs:', (j.waitingJobs || []).map((w) => ({
    ...
    console.log('[BOARD] openOffers:', JSON.stringify((j.openOffers || []).map((o) => ({
        job: o.deliveryJobId?.slice(0, 8).toUpperCase(),
        rider: o.riderProfile?.userId?.slice(0, 8),
        status: o.status,
    ...
    console.log('[BOARD] activeJobs:', (j.activeJobs || []).map((a) => ({
    ...
    if (j >= 0)
        console.log('[DISPATCH] Offers section:\n', txt.substring(j, j + 400));
