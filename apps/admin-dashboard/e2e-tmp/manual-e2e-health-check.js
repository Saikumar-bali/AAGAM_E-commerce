...
async function run() {
    ...
    if (b && Array.isArray(b.riders)) {
        ...
        console.log('openOffers:', (b.openOffers || []).map((o) => ({ job: o.deliveryJobId, status: o.status })));
        console.log('waitingJobs:', (b.waitingJobs || []).map((w) => ({ id: w.id, status: w.status })));
    ...
    if (Array.isArray(cal.body)) {
        const row = cal.body.find((d) => d.id === 'cmsuk38qt02nrd84tpaahef67');
        ...
        const stuck = cal.body.filter((d) => d.status === 'ORDER_GENERATED' && d.order?.status === 'DELIVERED');
        console.log('STUCK rows needing reconcile:', stuck.length, stuck.map((s) => s.id));
    }
    await browser.close();
    console.log('\n=== DONE ===');
}
