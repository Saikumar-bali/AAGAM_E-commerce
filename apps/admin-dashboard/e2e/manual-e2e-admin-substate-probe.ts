...
async function run() {
  ...
  const api = async (path: string) =>
    page.evaluate(async (p) => {
      ...
      if (!res.ok) return { httpStatus: res.status, body: (await res.text().catch(() => '')).substring(0, 300) };
      return res.json();
    }, API + path);
  ...
  if (Array.isArray(cal)) {
    ...
    if (!rows.length) console.log('not found in first page; total rows:', cal.length);
  } else {
