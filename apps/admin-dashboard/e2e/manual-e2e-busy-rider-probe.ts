...
async function run() {
  ...
  if (b && Array.isArray(b.riders)) {
    ...
    console.log('openOffers:', (b.openOffers || []).map((o: any) => ({ job: o.deliveryJobId, rider: o.riderProfile?.userId, status: o.status, createdAt: o.createdAt, expiresAt: o.expiresAt })));
    console.log('waitingJobs:', (b.waitingJobs || []).map((w: any) => ({ id: w.id, status: w.status, currentRiderId: w.currentRiderId, orderId: w.orderId })));
