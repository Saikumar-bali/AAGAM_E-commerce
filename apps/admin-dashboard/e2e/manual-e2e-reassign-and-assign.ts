...
async function run() {
  ...
  page.on('response', async (res) => {
    ...
    if (res.url().includes('/orders/dispatch/board')) {
      try {
        ...
        console.log('[BOARD] riders:', JSON.stringify(j.riders?.map((r: any) => ({ name: r.user?.name, status: r.status, activeJobCount: r.activeJobCount, activeStoreId: r.activeStoreId, acceptingSameStoreOrders: r.acceptingSameStoreOrders, available: r.available }))));
        console.log('[BOARD] waitingJobs:', (j.waitingJobs || []).map((w: any) => ({ id: w.id?.slice(0,8).toUpperCase(), status: w.status, rider: w.currentRiderId?.slice(0,8) })));
  ...
  if (msgIdx >= 0) console.log('[EXCEPTIONS] Success toast:', excText.substring(msgIdx, msgIdx + 120));
  if (errIdx >= 0) console.log('[EXCEPTIONS] Error toast:', excText.substring(Math.max(0, errIdx - 160), errIdx + 120));
