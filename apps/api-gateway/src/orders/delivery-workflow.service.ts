...
const TRANSITIONS: Record<DeliveryJobStatusType, DeliveryJobStatusType[]> = {
  ...
  RIDER_ASSIGNED: ['RIDER_EN_ROUTE_TO_STORE', 'WAITING_FOR_DISPATCH', 'CANCELLED'],
  ...
  OUT_FOR_DELIVERY: ['RIDER_AT_CUSTOMER', 'DELIVERED', 'DELIVERY_FAILED', 'RETURNING_TO_STORE', 'CANCELLED'],
  RIDER_AT_CUSTOMER: ['DELIVERED', 'DELIVERY_FAILED', 'RETURNING_TO_STORE', 'CANCELLED'],
  DELIVERED: [],
  DELIVERY_FAILED: ['WAITING_FOR_DISPATCH', 'RETURNING_TO_STORE', 'CANCELLED'],
...
const STORE_TRANSITIONS = new Set([
...
export class DeliveryWorkflowService {
  ...
  private legacyOrderStatus(status: DeliveryJobStatusType): OrderStatus {
    if (status === DeliveryJobStatus.WAITING_FOR_DISPATCH) return OrderStatus.PACKED;
    if (
      [
  ...
  private timestampData(status: OrderStatus) {
    if (status === OrderStatus.RIDER_ASSIGNED) return { riderAssignedAt: new Date() };
    if (status === OrderStatus.OUT_FOR_DELIVERY) return { outForDeliveryAt: new Date() };
  ...
  private async assertRole(job: any, nextStatus: DeliveryJobStatusType, actor: Actor, tx: DbClient) {
    if (actor.role === Role.ADMIN) return;

    const key = `${job.status}:${nextStatus}`;
    if (actor.role === Role.STORE_OWNER) {
      if (job.order.store.ownerId !== actor.id) {
        throw new ForbiddenException('Not allowed to update deliveries for this store');
      }
    ...
    if (actor.role === Role.RIDER) {
      const rider = await tx.riderProfile.findUnique({ where: { userId: actor.id } });
      if (!rider || job.currentRiderId !== rider.id) {
        throw new ForbiddenException('You can only update your active delivery');
      }
  ...
  private async finalizeDeliveredInventory(tx: DbClient, order: any, actor: Actor) {
  ...
  async transitionWithinTransaction(
    ...
    if (options.expectedStatus && currentStatus !== options.expectedStatus) {
      throw new ConflictException(`Delivery job changed from ${options.expectedStatus} to ${currentStatus}`);
    }
    if (currentStatus === nextStatus) return job;

    const allowed = TRANSITIONS[currentStatus] || [];
    if (!allowed.includes(nextStatus)) {
      throw new BadRequestException(`Cannot transition delivery from ${currentStatus} to ${nextStatus}`);
    }
    ...
    const effectiveRiderId = options.assignedRiderId !== undefined
  ...
  transition(
    ...
    return prisma.$transaction(
      (tx) => this.transitionWithinTransaction(tx, deliveryJobId, nextStatus, actor, { metadata }),
  ...
  async legacyPickup(deliveryJobId: string, actor: Actor) {
    return prisma.$transaction(async (tx) => {
      let job = await tx.deliveryJob.findUnique({ where: { id: deliveryJobId } });
      if (!job) throw new NotFoundException('Delivery job not found');
      const sequence: DeliveryJobStatusType[] = [];
      if (job.status === DeliveryJobStatus.RIDER_ASSIGNED) {
        sequence.push(DeliveryJobStatus.RIDER_EN_ROUTE_TO_STORE, DeliveryJobStatus.RIDER_AT_STORE);
      } else if (job.status === DeliveryJobStatus.RIDER_EN_ROUTE_TO_STORE) {
        sequence.push(DeliveryJobStatus.RIDER_AT_STORE);
      }
      if ([DeliveryJobStatus.RIDER_ASSIGNED, DeliveryJobStatus.RIDER_EN_ROUTE_TO_STORE, DeliveryJobStatus.RIDER_AT_STORE].includes(job.status)) {
        sequence.push(DeliveryJobStatus.PICKUP_VERIFIED, DeliveryJobStatus.OUT_FOR_DELIVERY);
      ...
      for (const status of sequence) {
        job = await this.transitionWithinTransaction(tx, deliveryJobId, status, actor, {
          skipRoleCheck: true,
  ...
  async legacyDeliver(deliveryJobId: string, actor: Actor, proof: Record<string, unknown> = {}) {
    return prisma.$transaction(async (tx) => {
      let job = await tx.deliveryJob.findUnique({ where: { id: deliveryJobId } });
      if (!job) throw new NotFoundException('Delivery job not found');
      if (job.status === DeliveryJobStatus.OUT_FOR_DELIVERY) {
        job = await this.transitionWithinTransaction(tx, deliveryJobId, DeliveryJobStatus.RIDER_AT_CUSTOMER, actor, {
          skipRoleCheck: true,
          metadata: { legacyAdapter: true },
        });
      }
      job = await this.transitionWithinTransaction(tx, deliveryJobId, DeliveryJobStatus.DELIVERED, actor, {
        skipRoleCheck: true,
