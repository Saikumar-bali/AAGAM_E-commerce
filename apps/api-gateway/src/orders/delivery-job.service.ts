...
export class DeliveryJobService {
  ...
  private initialStatus(orderStatus: OrderStatus): DeliveryJobStatusType {
    if (orderStatus === OrderStatus.RIDER_ASSIGNED) return DeliveryJobStatus.RIDER_ASSIGNED;
    if (orderStatus === OrderStatus.OUT_FOR_DELIVERY) return DeliveryJobStatus.OUT_FOR_DELIVERY;
    if (orderStatus === OrderStatus.DELIVERED) return DeliveryJobStatus.DELIVERED;
    if (orderStatus === OrderStatus.CANCELLED) return DeliveryJobStatus.CANCELLED;
    return DeliveryJobStatus.WAITING_FOR_DISPATCH;
  }

  async ensureForPackedOrder(orderId: string, actor: Actor, tx: DbClient = prisma) {
    ...
    if (!allowedStatuses.includes(order.status as OrderStatus)) {
      throw new ForbiddenException('Delivery jobs are created only after the order is packed');
    }
  ...
  getByOrderId(orderId: string) {
    return prisma.deliveryJob.findUnique({ where: { orderId }, include: jobInclude });
  }

  getById(id: string) {
    return prisma.deliveryJob.findUnique({ where: { id }, include: jobInclude });
  }

  async getBoard(actor: Actor) {
    if (actor.role !== Role.ADMIN && actor.role !== Role.STORE_OWNER) {
      throw new ForbiddenException('Only admin or store owner can view dispatch operations');
    }
    ...
    for (const job of jobs) {
      if (job.currentRiderId && ACTIVE_JOB_STATUSES.includes(job.status)) {
        activeCounts.set(job.currentRiderId, (activeCounts.get(job.currentRiderId) || 0) + 1);
      }
    }

    const waitingJobs = jobs.filter((job) => job.status === DeliveryJobStatus.WAITING_FOR_DISPATCH);
    const activeJobs = jobs.filter((job) => ACTIVE_JOB_STATUSES.includes(job.status));
    const completedJobs = jobs.filter((job) => TERMINAL_JOB_STATUSES.includes(job.status));
    ...
    return {
      ...
      openOffers: waitingJobs.flatMap((job) =>
        job.assignments.filter((assignment) =>
          [DispatchAssignmentStatus.CREATED, DispatchAssignmentStatus.OFFERED].includes(assignment.status as any),
        ),
      ),
  ...
  async getRiderWorkspace(riderUserId: string) {
    ...
    const [pendingOffers, activeJob, assignmentHistory] = await Promise.all([
      ...
      prisma.dispatchAssignment.findMany({
        ...
        include: { deliveryJob: { include: { order: { include: { store: true } } } } },
