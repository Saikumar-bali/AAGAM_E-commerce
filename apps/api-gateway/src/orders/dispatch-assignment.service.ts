...
import { DeliveryJobService, ACTIVE_JOB_STATUSES } from './delivery-job.service';
...
export class DispatchAssignmentService {
  ...
  private async assertDispatcher(job: any, actor: Actor) {
    ...
    if (actor.role === Role.STORE_OWNER && job.order.store.ownerId === actor.id) return;
    throw new ForbiddenException('Only admin or the owning store can offer a delivery job');
  }
  ...
  async offer(
    ...
    const safeExpiry = Math.max(15, Math.min(300, Math.floor(expiresInSeconds || 60)));
    ...
    try {
      return await prisma.$transaction(async (tx) => {
        ...
        if (job.status !== DeliveryJobStatus.WAITING_FOR_DISPATCH || job.currentRiderId) {
          throw new ConflictException('Delivery job is no longer waiting for dispatch');
        }

        const riderUser = await tx.user.findUnique({ where: { id: riderUserId } });
        if (!riderUser || riderUser.role !== Role.RIDER) {
          throw new BadRequestException('Selected user is not a rider');
        }
        const rider = await tx.riderProfile.findUnique({ where: { userId: riderUserId } });
        ...
        if (activeJob) {
          throw new ConflictException(`Rider already has active delivery ${activeJob.id}`);
        }
        ...
        const assignment = await tx.dispatchAssignment.create({
          ...
          include: {
            ...
            deliveryJob: { include: { order: { include: { customer: true, store: true, items: { include: { product: true } } } } } },
          },
        });
        ...
        await this.events.record({
          ...
          actor,
          metadata: { riderProfileId: rider.id, riderUserId, expiresInSeconds: safeExpiry },
        }, tx);
      ...
      if (error?.code === 'P2002') {
        throw new ConflictException('This delivery already has an active assignment offer');
      }
      throw error;
    }
  }
  ...
  async accept(assignmentId: string, riderUserId: string) {
    return prisma.$transaction(async (tx) => {
      ...
      if (!assignment) throw new NotFoundException('Assignment offer not found');
      if (assignment.riderProfile.userId !== riderUserId) {
        throw new ForbiddenException('This assignment was offered to another rider');
      }
      ...
      if (assignment.expiresAt && assignment.expiresAt.getTime() <= Date.now()) {
        await tx.dispatchAssignment.update({
          ...
          data: { status: DispatchAssignmentStatus.EXPIRED, respondedAt: new Date() },
        });
        await this.events.record({
      ...
      if (activeJob) {
        throw new ConflictException(`Complete active delivery ${activeJob.id} before accepting another`);
      }

      const accepted = await tx.dispatchAssignment.updateMany({
        where: { id: assignment.id, status: DispatchAssignmentStatus.OFFERED },
        data: { status: DispatchAssignmentStatus.ACCEPTED, respondedAt: new Date() },
      });
      if (accepted.count !== 1) {
        throw new ConflictException('Assignment was already answered by another request');
      }
      ...
      const job = await tx.deliveryJob.findUnique({
        ...
        include: {
          order: { include: { customer: true, store: true, items: { include: { product: true } }, rider: { include: { user: true } } } },
  ...
  async reject(assignmentId: string, riderUserId: string, reason?: string) {
    return prisma.$transaction(async (tx) => {
      ...
      if (assignment.riderProfile.userId !== riderUserId) {
        throw new ForbiddenException('This assignment belongs to another rider');
      }
      if (![DispatchAssignmentStatus.OFFERED, DispatchAssignmentStatus.ACCEPTED].includes(assignment.status as any)) {
        throw new ConflictException(`Assignment is already ${assignment.status}`);
      }
      ...
      if (updated.count !== 1) throw new ConflictException('Assignment changed before rejection completed');

      if (previousStatus === DispatchAssignmentStatus.ACCEPTED) {
        if (assignment.deliveryJob.status !== DeliveryJobStatus.RIDER_ASSIGNED) {
          throw new BadRequestException('Accepted assignment can only be rejected before travel begins');
        }
        await this.workflow.transitionWithinTransaction(
          tx,
          assignment.deliveryJobId,
        ...
        await tx.riderProfile.update({ where: { id: assignment.riderProfileId }, data: { status: 'ONLINE' } });
      }
  ...
  async findCurrentForOrderAndRider(orderId: string, riderUserId: string) {
    ...
    const assignment = await prisma.dispatchAssignment.findFirst({
      where: {
        riderProfileId: rider.id,
        deliveryJob: { orderId },
        status: { in: [DispatchAssignmentStatus.OFFERED, DispatchAssignmentStatus.ACCEPTED] as any },
      },
      include: { deliveryJob: true },
      orderBy: { createdAt: 'desc' },
    });
    if (!assignment) throw new NotFoundException('No active assignment found for this order');
    return assignment;
  }
}
