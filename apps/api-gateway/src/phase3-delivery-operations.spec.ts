...
async function cleanup() {
  ...
  await prisma.notification.deleteMany({
    where: { OR: [{ orderId: { in: ids.orderIds } }, { deliveryJobId: { in: ids.jobIds } }] },
  });
  await prisma.outboxEvent.deleteMany({
    where: { OR: [{ aggregateId: { in: ids.orderIds } }, { aggregateId: { in: ids.jobIds } }] },
  });
  ...
  await prisma.refund.deleteMany({ where: { payment: { orderId: { in: ids.orderIds } } } });
...
async function seed(cod = false) {
  ...
  const otherRiderUser = await prisma.user.create({
    data: { email: `${PREFIX}other_rider_${suffix}@test.com`, role: Role.RIDER, name: 'Other Rider' },
  });
  const rider = await prisma.riderProfile.create({
    data: { userId: riderUser.id, status: 'ONLINE', latitude: 17.7, longitude: 83.3 },
  });
  await prisma.riderProfile.create({
    data: { userId: otherRiderUser.id, status: 'ONLINE', latitude: 17.71, longitude: 83.31 },
  });
...
describe('Phase 3 delivery exceptions, COD, returns, and OTP', () => {
  ...
  it('stores only an OTP hash and completes delivery after customer-scoped verification', async () => {
    ...
    await expect(api.operations.getCustomerOtp(job.id, riderActor)).rejects.toThrow('order customer');
    const customerOtp = await api.operations.getCustomerOtp(job.id, customerActor);
    expect(customerOtp.code).toMatch(/^\d{6}$/);

    await expect(api.operations.completeDelivery(job.id, riderActor, { otpCode: '000000' }))
    ...
    const delivered = await api.operations.completeDelivery(
      job.id,
      riderActor,
      { otpCode: customerOtp.code, proofType: 'CUSTOMER_OTP' },
      'complete-with-otp',
    );
  ...
  it('requires exact COD collection before delivery and makes collection idempotent', async () => {
    ...
    const { api, job, riderActor } = await atCustomer(true);

    await expect(api.operations.completeDelivery(job.id, riderActor, {}))
      .rejects.toThrow('Collect the full COD amount');
    await expect(api.operations.collectCod(job.id, riderActor, { amountPaise: 29999 }))
  ...
  it('records a failure and enforces the return-to-store lifecycle', async () => {
    ...
    await expect(api.operations.recordFailure(
      job.id,
      { id: data.otherRiderUser.id, role: Role.RIDER },
      { reason: DeliveryFailureReason.CUSTOMER_UNAVAILABLE },
    )).rejects.toThrow('assigned rider');

    const failed = await api.operations.recordFailure(
      job.id,
      riderActor,
      { reason: DeliveryFailureReason.CUSTOMER_UNAVAILABLE, note: 'No response at the address' },
      'failure-once',
    );
  ...
  it('restores only sellable returned stock and prevents duplicate inspection', async () => {
    ...
    const input = {
      lines: [
        { orderItemId: data.order.items[0].id, disposition: ReturnDisposition.SELLABLE, quantity: 2 },
        { orderItemId: data.order.items[0].id, disposition: ReturnDisposition.DAMAGED, quantity: 1 },
      ],
      note: 'Two units sealed, one damaged',
    };
    const operation = await api.operations.inspectReturn(job.id, storeActor, input, 'inspection-once');
    const duplicate = await api.operations.inspectReturn(job.id, storeActor, input, 'inspection-once');
    expect(duplicate.id).toBe(operation.id);
    await expect(api.operations.inspectReturn(job.id, storeActor, input, 'inspection-second-key'))
      .rejects.toThrow('already completed');

    const inventory = await prisma.inventory.findUnique({
      where: { storeId_productId: { storeId: data.store.id, productId: data.product.id } },
    });
  ...
  it('records exact COD settlement once for the owning store', async () => {
    ...
    await api.operations.collectCod(job.id, riderActor, { amountPaise: 30000 }, 'settlement-collection');
    await expect(api.operations.settleCod(
      job.id,
  ...
  it('restricts the queue and creates custom outbox audit events', async () => {
    ...
    const event = await prisma.outboxEvent.findFirst({
      where: { aggregateId: job.id, idempotencyKey: { contains: 'delivery-operation:' } },
