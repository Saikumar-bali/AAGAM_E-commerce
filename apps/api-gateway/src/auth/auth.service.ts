...
export class AuthService {
  ...
  private buildAuthResponse(user: { id: string; email: string; role: Role; name: string | null; avatarUrl?: string | null }) {
    ...
    return { session: { access_token: token }, user: { id: user.id, email: user.email, role: user.role, name: user.name, avatarUrl: user.avatarUrl || null } };
  }
  ...
  async updateProfile(userId: string, data: { name?: string }) {
    const updated = await prisma.user.update({ where: { id: userId }, data: { ...(data.name !== undefined && { name: data.name }) }, select: { id: true, email: true, role: true, name: true, avatarUrl: true, emailVerified: true, createdAt: true } });
    return updated;
  }
