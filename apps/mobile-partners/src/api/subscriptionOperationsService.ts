...
export const subscriptionOperationsService = {
  ...
  getCalendar: async (from?: string, to?: string): Promise<any[]> => {
    ...
    if (to) params.to = to;
    const response = await apiClient.get('/store/subscriptions/delivery-calendar', { params });
    return Array.isArray(response.data) ? response.data : [];
  },
  ...
  autoDispatchDefaultRiders: async (dateStr: string, slot?: 'AM' | 'PM' | 'ALL') => {
    const response = await apiClient.post('/store/subscriptions/auto-dispatch-default-riders', { dateStr, slot });
    return response.data;
  },
};
