...
export function RiderApplicationScreen({ navigation }: any) {
  ...
  const useLocation = async () => {
    if (Platform.OS === 'android') {
      const permission = await PermissionsAndroid.request(PermissionsAndroid.PERMISSIONS.ACCESS_FINE_LOCATION, {
        ...
        message: 'Aagaam uses your location to suggest service areas and verify operational availability.',
  ...
  return <OnboardingShell title="Become an Aagaam Rider" subtitle={`Step ${step + 1} of 4 · ${STEP_TITLES[step]}`} onBack={() => step > 0 ? setStep((value) => value - 1) : navigation.goBack()}>
    ...
    <PrimaryButton testID="onboarding_save_button" label={step === 3 ? 'Save and continue to documents' : 'Save and continue'} onPress={next} loading={isLoading} />
    {step > 0 ? <PrimaryButton testID="onboarding_previous_button" label="Previous step" onPress={() => setStep((value) => value - 1)} secondary disabled={isLoading} /> : null}
...
const styles = StyleSheet.create({
  datePicker: { minHeight: 50, borderRadius: 15, borderWidth: 1, borderColor: '#CBD5E1', backgroundColor: '#F8FAFC', paddingHorizontal: 14, justifyContent: 'center' }, dateValue: { color: palette.ink, fontSize: 14, fontWeight: '600' }, datePlaceholder: { color: '#94A3B8', fontSize: 14, fontWeight: '500' }, zoneEmpty: { borderRadius: 14, backgroundColor: '#FFF7ED', padding: 12, color: '#9A3412', fontSize: 12, fontWeight: '600' },
  stepIcon: { width: 44, height: 44, borderRadius: 15, backgroundColor: '#ECFEFF', alignItems: 'center', justifyContent: 'center' }, choiceGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 }, choice: { minHeight: 42, borderRadius: 13, borderWidth: 1, borderColor: '#CBD5E1', backgroundColor: '#F8FAFC', paddingHorizontal: 12, paddingVertical: 8, flexDirection: 'row', alignItems: 'center', gap: 4 }, choiceActive: { borderColor: '#2DD4BF', backgroundColor: '#CCFBF1' }, choiceText: { color: palette.muted, fontSize: 12, fontWeight: '600' }, choiceTextActive: { color: palette.teal }, fieldLabel: { color: palette.ink, fontSize: 12, fontWeight: '600' }, locationButton: { minHeight: 48, borderRadius: 15, borderWidth: 1, borderColor: '#99F6E4', backgroundColor: '#F0FDFA', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 }, locationText: { color: palette.teal, fontSize: 12, fontWeight: '600' }, savedAccount: { borderRadius: 15, backgroundColor: '#ECFDF5', padding: 12, flexDirection: 'row', alignItems: 'center', gap: 8 }, savedAccountText: { color: palette.green, fontSize: 12, fontWeight: '600' }, note: { color: palette.muted, fontSize: 11, lineHeight: 17, textAlign: 'center' },
});
