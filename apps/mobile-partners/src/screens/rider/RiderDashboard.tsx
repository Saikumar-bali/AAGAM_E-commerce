...
import {
  ...
  StatusBar, Alert, Linking, Platform, ActivityIndicator, PermissionsAndroid,
} from 'react-native';
...
import {
  Package, DollarSign, Power, ShieldCheck, Zap, ArrowRight, Clock, MapPin, Phone, RefreshCw,
...
const User = ({ size, color }: any) => <MapPin size={size} color={color} />;
const Truck = ({ size, color }: any) => <Package size={size} color={color} />;
...
export const RiderDashboard = () => {
  ...
  useEffect(() => {
    ...
    const newSocket = io(API_URL, { transports: ['websocket'], withCredentials: true });
    ...
    newSocket.on('newOrderNearby', (data: any) => { setNewOrder(data); handleRefresh(); });
    newSocket.on('connect_error', (error: any) => { console.warn('[Rider Socket] connect_error', error?.message || error); });
    setSocket(newSocket);
  ...
  useEffect(() => {
    const handleNotificationOpen = (remoteMessage: any) => {
      const orderId = remoteMessage?.data?.orderId || remoteMessage?.data?.order_id || remoteMessage?.data?.id || null;
      if (!orderId) return;
    ...
    messaging().getInitialNotification().then((message) => { if (message) handleNotificationOpen(message); }).catch(() => {});
    return () => unsubscribeOpened();
  }, []);

  useEffect(() => { if (socket) handleGoOnline(); }, [socket]);
  ...
  useEffect(() => {
    ...
    if (isOnline && previousIds.length > 0 && newIds.length > 0) {
      ...
      Alert.alert('New nearby order', newest ? `${newest.store?.name || 'Aagam store'} • ₹${newest.grandTotal}` : 'A new order is available to accept.');
    }
  ...
  useEffect(() => {
    ...
    const queueList = Array.isArray(queueOrders) ? queueOrders : [];
    const matched = queueList.find((order: any) => order.id === pendingNotificationOrderId || order.orderId === pendingNotificationOrderId);
    if (matched) { setNewOrder(matched); setPendingNotificationOrderId(null); }
  }, [pendingNotificationOrderId, queueOrders]);

  const handleRefresh = () => { refetchAssigned(); refetchQueue(); };

  const handleGoOffline = async () => {
    setIsOnline(false);
    try { await riderService.updateMyStatus('OFFLINE', currentLocation ? { latitude: currentLocation.lat, longitude: currentLocation.lng } : undefined); } catch {}
  };

  const requestLocationPermission = async () => {
    if (Platform.OS !== 'android') return true;
    const result = await PermissionsAndroid.request(PermissionsAndroid.PERMISSIONS.ACCESS_FINE_LOCATION, {
      title: 'Allow Aagam rider location', message: 'Aagam needs your location to show nearby orders.', buttonPositive: 'Allow', buttonNegative: 'Not now',
    });
  ...
  const handleGoOnline = async () => {
    ...
    if (!hasPermission) { Alert.alert('Location permission needed', 'Allow location permission to go online.'); setLocating(false); return; }
    Geolocation.getCurrentPosition(
      async (position) => {
        const { latitude, longitude } = position.coords;
        setCurrentLocation({ lat: latitude, lng: longitude });
        try { await riderService.updateMyStatus('ONLINE', { latitude, longitude }); } catch {}
        if (socket) socket.emit('joinRiderZone', { latitude, longitude });
        setIsOnline(true);
        setLocating(false);
      },
      () => { Alert.alert('Location Error', 'Please enable GPS to go online'); setLocating(false); },
    ...
    Geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          await riderService.sendLocationPing(orderId, { latitude, longitude, accuracy: accuracy ?? undefined, speed: speed ?? undefined, heading: heading ?? undefined });
          socket?.emit('updateRiderLocation', { riderId: user?.id, orderId, latitude, longitude, bearing: heading || 0, status: 'LIVE' });
        } catch {}
      },
      () => {},
  ...
  useEffect(() => {
    const activeTrackableOrder = Array.isArray(assignedOrders) ? assignedOrders.find((order: any) => ['RIDER_ASSIGNED', 'OUT_FOR_DELIVERY'].includes(order.status)) : null;
    if (!activeTrackableOrder || !isOnline) return;
    sendLiveLocation(activeTrackableOrder.id);
    const interval = setInterval(() => { sendLiveLocation(activeTrackableOrder.id); }, activeTrackableOrder.status === 'OUT_FOR_DELIVERY' ? 8000 : 20000);
  ...
  const handleAcceptOrder = async (orderId: string) => {
    ...
    try { await riderService.assignOrder(orderId); setNewOrder(null); Alert.alert('Success', 'Order accepted!'); handleRefresh(); }
    catch (error: any) { Alert.alert('Error', error.message || 'Failed to accept order'); }
    finally { setAcceptingOrderId(null); }
  };

  const confirmAcceptOrder = (orderId: string) => {
    Alert.alert('Accept this order?', 'This will assign the delivery to you.', [
  ...
  const updateStatusMutation = useMutation({
    mutationFn: ({ orderId, status }: { orderId: string, status: string }) => riderService.updateOrderStatus(orderId, status),
    onSuccess: () => handleRefresh(),
    onError: (error: any) => Alert.alert('Error', error.message || 'Failed to update status'),
  });
  ...
  const openDeliveryRoute = (order: any) => {
    if (!order?.deliveryLat || !order?.deliveryLng) { Alert.alert('Location missing', 'Customer location is not available.'); return; }
  ...
  const callCustomer = (phone?: string | null) => {
    if (!phone) { Alert.alert('Phone unavailable', 'Customer phone number is not available.'); return; }
  ...
  return (
    <View style={styles.mainContainer}>
      ...
      {newOrder && (
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            {pendingNotificationOrderId ? <Text style={styles.modalHint}>Refreshing nearby queue for this order...</Text> : null}
            <View style={styles.modalActions}>
              <TouchableOpacity style={styles.declineBtn} onPress={() => setNewOrder(null)}><Text style={styles.declineText}>Decline</Text></TouchableOpacity>
              <TouchableOpacity style={styles.acceptBtn} onPress={() => confirmAcceptOrder(newOrder.orderId || newOrder.id)} disabled={!newOrder.orderId && !newOrder.id}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity style={[styles.statusToggle, isOnline ? styles.onlineBg : styles.offlineBg]} onPress={isOnline ? handleGoOffline : handleGoOnline} disabled={locating}>
          {locating ? <ActivityIndicator size="small" color="#0F766E" /> : (
            <>
              <Power size={18} color={isOnline ? '#10B981' : '#64748B'} />
              <Text style={[styles.statusText, isOnline ? styles.onlineText : styles.offlineText]}>{isOnline ? 'ONLINE' : 'OFFLINE'}</Text>
            </>
          )}
        </TouchableOpacity>
      </View>

      <ScrollView showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={loadingAssigned || loadingQueue} onRefresh={handleRefresh} />}>
        ...
        {Array.isArray(assignedOrders) && assignedOrders.length > 0 ? (
                ...
                return (
                  <>
                    <View style={styles.cardHeader}>
                      <View style={styles.orderInfo}><Package size={18} color="#0F766E" /><Text style={styles.orderIdText}>Order #{order.id.slice(-8).toUpperCase()}</Text></View>
                      <StatusChip status={order.status} />
                    </View>
                    <View style={styles.progressTracker}><View style={[styles.progressBar, { width: getProgressWidth(order.status) }]} /></View>
                    <View style={styles.detailRow}>
                      ...
                      <View style={styles.detailText}>
                        ...
                        <TouchableOpacity onPress={() => openMaps(order.store?.latitude, order.store?.longitude)}><Text style={styles.linkText}>Get Directions →</Text></TouchableOpacity>
                      </View>
                    </View>
                    <View style={styles.detailRow}>
                      ...
                      <View style={styles.detailText}>
                        ...
                        <View style={styles.customerActionsRow}>
                          <TouchableOpacity onPress={() => openDeliveryRoute(order)}><Text style={styles.linkText}>Navigate to customer →</Text></TouchableOpacity>
                          <TouchableOpacity onPress={() => callCustomer(customerPhone)}>
                            <View style={styles.callAction}><Phone size={13} color="#0F766E" /><Text style={styles.callActionText}>Call</Text></View>
                          </TouchableOpacity>
                        </View>
                      </View>
                    </View>
                    {order.items && (
                      <View style={styles.itemsBox}>
                        ...
                        {order.items.map((item: any, idx: number) => (<Text key={idx} style={styles.itemText}>• {item.product?.name} x{item.quantity}</Text>))}
                      </View>
                    )}
                    <TouchableOpacity style={styles.actionButton} onPress={() => handleUpdateStatus(order.id, order.status)}>
          ...
          <View style={styles.emptyContainer}><Truck size={48} color="#E2E8F0" /><Text style={styles.emptyText}>No active deliveries at the moment</Text></View>
        )}

        <View style={styles.sectionHeader}>
          <Text style={styles.sectionTitle}>Available Orders</Text>
          <View style={styles.queueCountBadge}><Text style={styles.queueCountText}>{Array.isArray(queueOrders) ? queueOrders.length : 0}</Text></View>
          <TouchableOpacity onPress={handleRefresh} style={styles.inlineRefreshBtn}>
            <RefreshCw size={14} color="#0F766E" /><Text style={styles.inlineRefreshText}>Refresh</Text>
          </TouchableOpacity>
        </View>
          ...
          queueOrders.map((order: any) => (
            <TouchableOpacity key={order.id} style={styles.queueCard} onPress={() => confirmAcceptOrder(order.id)}>
              <View style={styles.queueInfo}>
                ...
                <View><Text style={styles.queueTitle}>₹{order.grandTotal} Order Nearby</Text><Text style={styles.queueSub}>{order.store?.name}</Text></View>
              </View>
              <View style={styles.acceptBadge}><Text style={styles.acceptText}>ACCEPT</Text></View>
            </TouchableOpacity>
          ))
        ) : (
          <View style={styles.emptyContainer}><Clock size={40} color="#E2E8F0" /><Text style={styles.emptyText}>No nearby orders found</Text></View>
        )}
      </ScrollView>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  header: { paddingHorizontal: 24, paddingTop: 60, paddingBottom: 24, backgroundColor: '#FFFFFF', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderBottomWidth: 1, borderBottomColor: '#F1F5F9' },
  ...
  modalOverlay: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.7)', zIndex: 1000, justifyContent: 'center', alignItems: 'center', padding: 24 },
  modalCard: { width: '100%', backgroundColor: '#FFF', borderRadius: 32, padding: 32, alignItems: 'center', elevation: 20, shadowColor: '#000', shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.3, shadowRadius: 20 },
  pulseContainer: { width: 80, height: 80, borderRadius: 40, backgroundColor: '#ECFDF5', justifyContent: 'center', alignItems: 'center', marginBottom: 24 },
  pulseInner: { width: 24, height: 24, borderRadius: 12, backgroundColor: '#10B981' },
  modalTitle: { fontSize: 20, fontWeight: 'bold', color: '#1E293B', marginBottom: 8 },
  modalAmount: { fontSize: 48, fontWeight: '900', color: '#10B981', marginBottom: 8 },
  modalStore: { fontSize: 16, color: '#64748B', marginBottom: 12, textAlign: 'center' },
  modalHint: { fontSize: 12, color: '#64748B', fontWeight: '600', marginBottom: 20, textAlign: 'center' },
  modalActions: { flexDirection: 'row', gap: 12 },
  declineBtn: { flex: 1, height: 56, borderRadius: 16, backgroundColor: '#F1F5F9', justifyContent: 'center', alignItems: 'center' },
  declineText: { fontSize: 16, fontWeight: 'bold', color: '#64748B' },
  acceptBtn: { flex: 2, height: 56, borderRadius: 16, backgroundColor: '#1E293B', justifyContent: 'center', alignItems: 'center', elevation: 4 },
  acceptBtnText: { fontSize: 16, fontWeight: 'bold', color: '#FFF' },
  modalRefreshBtn: { marginTop: 14, paddingVertical: 8, paddingHorizontal: 12, borderRadius: 10, backgroundColor: '#ECFDF5', borderWidth: 1, borderColor: '#A7F3D0' },
  modalRefreshText: { color: '#0F766E', fontSize: 12, fontWeight: '800' },
});
