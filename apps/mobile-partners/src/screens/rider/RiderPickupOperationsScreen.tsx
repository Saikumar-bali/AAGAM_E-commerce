...
export const RiderPickupOperationsScreen = () => {
  ...
  const reportProblem = useMutation({
    mutationFn: async () => {
      ...
      return pickupOperationsService.reportProblem(job.id, {
        problemType,
    ...
    onSuccess: async () => {
      ...
      setProblemNote('');
      Toast.show({ type: 'success', text1: 'Pickup problem reported', text2: 'Re-check every item after the store corrects the parcel.' });
      await refresh();
    },
    ...
    >
      ...
      {pickupQuery.isLoading ? (
        <View style={styles.center}>
          ...
          <Text style={styles.muted}>Loading pickup task…</Text>
        </View>
      ) : pickupQuery.error ? (
        <View style={styles.errorCard}>
          <AlertTriangle size={36} color="#B91C1C" />
          <Text style={styles.errorTitle}>Pickup task unavailable</Text>
          <Text style={styles.errorText}>{errorMessage(pickupQuery.error)}</Text>
        </View>
      ) : !job || !task ? (
        <View style={styles.emptyCard}>
          <PackageCheck size={48} color="#94A3B8" />
          <Text style={styles.emptyTitle}>No pickup waiting</Text>
          <Text style={styles.emptyText}>Use Dashboard to arrive at the assigned store.</Text>
        </View>
      ) : (
        <>
          <View style={styles.summaryCard}>
            ...
            <View style={[styles.badge, checklistVerified ? styles.badgeReady : styles.badgePending]}>
              <Text style={styles.badgeText}>{checklistVerified ? 'VERIFIED' : 'CHECK ITEMS'}</Text>
            </View>
          </View>
          ...
          <View style={styles.card}>
            <View style={styles.sectionHeader}>
              ...
              <View style={styles.flex}>
                <Text style={styles.sectionTitle}>Item checklist</Text>
                <Text style={styles.sectionText}>Tap each line only after confirming its full quantity.</Text>
              </View>
            </View>
              ...
              return (
                <TouchableOpacity
                  ...
                  onPress={() => setChecked((current) => ({
                    ...current,
                    [item.orderItemId]: !current[item.orderItemId],
                  }))}
                >
                  ...
                  <View style={styles.flex}>
                    ...
                    <Text style={styles.itemMeta}>Expected quantity: {item.expectedQuantity}</Text>
                  </View>
            ...
            <TextInput
              testID="rider_pickup_parcel_code"
              value={parcelCode}
              onChangeText={setParcelCode}
              editable={!checklistVerified && !busy}
              placeholder="Optional parcel or seal code"
            ...
            {checklistVerified ? (
              <View style={styles.successCard}>
                ...
                <Text style={styles.successText}>Checklist verified. Complete the store handoff below.</Text>
              </View>
            ) : (
              <TouchableOpacity
                testID="rider_pickup_verify_checklist"
                style={[styles.primaryButton, (!checklistComplete || busy) && styles.disabled]}
                disabled={!checklistComplete || busy}
                onPress={() => Alert.alert(
                  'Verify complete checklist?',
                  'Every quantity must match the packed parcel.',
                  [
                    { text: 'Back', style: 'cancel' },
                    { text: 'Verify checklist', onPress: () => verifyChecklist.mutate() },
                  ],
                )}
              >
                {verifyChecklist.isPending ? <ActivityIndicator color="#FFFFFF" /> : <CheckCircle2 size={18} color="#FFFFFF" />}
                <Text style={styles.buttonText}>Verify complete checklist</Text>
              </TouchableOpacity>
            )}
          </View>
          ...
          {checklistVerified ? (
            <View style={styles.card}>
              <View style={styles.sectionHeader}>
                <KeyRound size={22} color="#0F766E" />
                <View style={styles.flex}>
                  <Text style={styles.sectionTitle}>Store pickup PIN</Text>
                  <Text style={styles.sectionText}>Enter the six-digit PIN shown by the owning store.</Text>
                </View>
              </View>
              <TextInput
                testID="rider_pickup_pin"
                value={pickupPin}
                onChangeText={(value) => setPickupPin(value.replace(/\D/g, '').slice(0, 6))}
                placeholder="6-digit pickup PIN"
                placeholderTextColor="#94A3B8"
              ...
              />
              <TextInput
                testID="rider_pickup_parcel_count"
                value={parcelCount}
                onChangeText={(value) => setParcelCount(value.replace(/\D/g, '').slice(0, 3))}
                placeholder="Parcel count"
                placeholderTextColor="#94A3B8"
                keyboardType="number-pad"
                style={styles.input}
              />
              ...
              >
                ...
                <Text style={styles.buttonText}>Verify store handoff</Text>
              </TouchableOpacity>
              <View style={styles.infoCard}>
                <Store size={19} color="#0F766E" />
                <Text style={styles.infoText}>The store can also confirm physical handoff directly. Pull to refresh after confirmation.</Text>
              </View>
            </View>
          ) : null}
          ...
          <View style={styles.problemCard}>
            <TouchableOpacity
              testID="rider_pickup_problem_toggle"
              style={styles.problemHeader}
              onPress={() => setShowProblem((value) => !value)}
            >
              <XCircle size={20} color="#B91C1C" />
              <View style={styles.flex}>
                <Text style={styles.problemTitle}>Parcel does not match?</Text>
                <Text style={styles.problemText}>Report the exact mismatch before pickup.</Text>
              </View>
              <Text style={styles.problemLink}>{showProblem ? 'Close' : 'Report'}</Text>
            </TouchableOpacity>
              ...
              <View style={styles.problemBody}>
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.problemTypes}>
                  {PROBLEM_TYPES.map(([value, label]) => (
                    <TouchableOpacity
                      key={value}
                      style={[styles.problemChip, problemType === value && styles.problemChipActive]}
                      onPress={() => setProblemType(value)}
                    >
                      <Text style={[styles.problemChipText, problemType === value && styles.problemChipTextActive]}>{label}</Text>
                    </TouchableOpacity>
                  ))}
                </ScrollView>
                <TextInput
                  testID="rider_pickup_problem_note"
                  value={problemNote}
                  onChangeText={setProblemNote}
                  placeholder="Describe what is missing, damaged, or incorrect"
                  placeholderTextColor="#94A3B8"
                  multiline
                  maxLength={500}
                ...
                />
                <TouchableOpacity
                  testID="rider_pickup_report_problem"
                  style={[styles.dangerButton, busy && styles.disabled]}
                  disabled={busy}
                  onPress={() => reportProblem.mutate()}
                >
                  {reportProblem.isPending ? <ActivityIndicator color="#FFFFFF" /> : <AlertTriangle size={18} color="#FFFFFF" />}
                  <Text style={styles.buttonText}>Submit pickup problem</Text>
                </TouchableOpacity>
              </View>
            ) : null}
          </View>
...
const styles = StyleSheet.create({
  ...
  emptyCard: { margin: 20, minHeight: 260, borderRadius: 26, backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: '#E2E8F0', alignItems: 'center', justifyContent: 'center', padding: 28 },
  ...
  summaryCard: { margin: 18, marginBottom: 0, borderRadius: 24, backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: '#E2E8F0', padding: 17, flexDirection: 'row', gap: 12 },
  orderCode: { color: '#0F766E', fontSize: 10, fontWeight: '900' },
  storeName: { color: '#0F172A', fontSize: 19, fontWeight: '900', marginTop: 5 },
  summaryText: { color: '#64748B', fontSize: 12, marginTop: 5 },
  badge: { borderRadius: 999, paddingHorizontal: 10, paddingVertical: 7, alignSelf: 'flex-start' },
  badgeReady: { backgroundColor: '#DCFCE7' },
  badgePending: { backgroundColor: '#FEF3C7' },
  badgeText: { color: '#334155', fontSize: 9, fontWeight: '900' },
  card: { marginHorizontal: 18, marginTop: 14, borderRadius: 24, backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: '#E2E8F0', padding: 17 },
  ...
  itemCard: { borderRadius: 17, borderWidth: 1, borderColor: '#E2E8F0', backgroundColor: '#F8FAFC', padding: 13, flexDirection: 'row', alignItems: 'center', gap: 11, marginBottom: 10 },
  itemChecked: { borderColor: '#5EEAD4', backgroundColor: '#F0FDFA' },
  checkbox: { width: 28, height: 28, borderRadius: 9, borderWidth: 2, borderColor: '#94A3B8', alignItems: 'center', justifyContent: 'center' },
  checkboxChecked: { borderColor: '#0F766E', backgroundColor: '#0F766E' },
  itemName: { color: '#0F172A', fontSize: 13, fontWeight: '900' },
  itemMeta: { color: '#64748B', fontSize: 10, marginTop: 3 },
  itemQty: { color: '#0F172A', fontSize: 15, fontWeight: '900' },
  input: { minHeight: 48, borderRadius: 14, borderWidth: 1, borderColor: '#CBD5E1', backgroundColor: '#FFFFFF', paddingHorizontal: 13, color: '#0F172A', fontWeight: '700', marginTop: 10 },
  pinInput: { textAlign: 'center', fontSize: 22, letterSpacing: 4 },
  multiline: { minHeight: 90, paddingTop: 12, textAlignVertical: 'top' },
  primaryButton: { minHeight: 50, borderRadius: 15, backgroundColor: '#0F766E', marginTop: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 },
  ...
  successCard: { marginTop: 10, borderRadius: 14, borderWidth: 1, borderColor: '#BBF7D0', backgroundColor: '#F0FDF4', padding: 12, flexDirection: 'row', alignItems: 'center', gap: 8 },
  successText: { flex: 1, color: '#166534', fontSize: 12, fontWeight: '800' },
  infoCard: { marginTop: 13, borderRadius: 14, backgroundColor: '#F0FDFA', borderWidth: 1, borderColor: '#99F6E4', padding: 12, flexDirection: 'row', alignItems: 'flex-start', gap: 8 },
  infoText: { flex: 1, color: '#115E59', fontSize: 11, fontWeight: '700' },
  problemCard: { marginHorizontal: 18, marginTop: 14, borderRadius: 22, backgroundColor: '#FEF2F2', borderWidth: 1, borderColor: '#FECACA', padding: 15 },
  problemHeader: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  problemTitle: { color: '#991B1B', fontSize: 15, fontWeight: '900' },
  problemText: { color: '#B91C1C', fontSize: 11, marginTop: 3 },
  problemLink: { color: '#991B1B', fontWeight: '900' },
  problemBody: { marginTop: 12, borderTopWidth: 1, borderTopColor: '#FECACA', paddingTop: 12 },
  problemTypes: { gap: 8 },
  problemChip: { borderRadius: 999, borderWidth: 1, borderColor: '#FCA5A5', backgroundColor: '#FFFFFF', paddingHorizontal: 11, paddingVertical: 8 },
  problemChipActive: { backgroundColor: '#B91C1C', borderColor: '#B91C1C' },
  problemChipText: { color: '#991B1B', fontSize: 10, fontWeight: '900' },
  problemChipTextActive: { color: '#FFFFFF' },
});
