    ...
    <View style={styles.choices}>
      {values.map((value: string) => {
        ...
        return (
          ...
          >
            {active ? <Check size={14} color={palette.teal} /> : null}
...
export function StoreApplicationScreen({ navigation }: any) {
  ...
  useEffect(() => {
    setForm((current) => ({
      ...
      businessType: String(saved.businessType || 'Grocery'),
      ...
      operatingHours: String(saved.operatingHours || '7 AM–11 PM'),
  ...
  const useLocation = () => {
    Geolocation.getCurrentPosition(
      (position) => {
        ...
        Alert.alert('Pickup location added', 'Admin can verify this pin during review.');
      },
  ...
  const validate = () => {
    if (step === 0) {
      if (!form.legalName.trim() || !form.displayName.trim() || !form.categories.length) {
        throw new Error('Enter the business names and choose at least one category.');
      }
    }
    if (step === 1) {
      if (!form.storeAddress.trim() || !form.city.trim() || !form.state.trim() || !/^\d{6}$/.test(form.pincode)) {
        throw new Error('Complete the store address and valid 6-digit pincode.');
      }
      if (!Number.isFinite(Number(form.latitude)) || !Number.isFinite(Number(form.longitude))) {
        throw new Error('Use current location to add the store pickup pin.');
      }
    }
  ...
  return (
    <OnboardingShell
      title="Join AAGAM as a Store"
      subtitle={`Step ${step + 1} of 4 · ${STEP_TITLES[step]}`}
      onBack={() => (step > 0 ? setStep((value) => value - 1) : navigation.goBack())}
    >
      <ProgressBar value={(step + 1) * 25} />

      {step === 0 ? (
        <Section title="Business identity" subtitle="Use the name customers will recognise.">
          <View style={styles.icon}><Store size={22} color={palette.teal} /></View>
          <FormField label="Legal business name" value={form.legalName} onChangeText={(value) => set('legalName', value)} placeholder="Registered name" />
          <FormField label="Store display name" value={form.displayName} onChangeText={(value) => set('displayName', value)} placeholder="Name shown to customers" />
          <Text style={styles.label}>Business type</Text>
          <Choices values={BUSINESS_TYPES} selected={form.businessType} onChange={(value: string) => set('businessType', value)} />
          <Text style={styles.label}>What do you sell?</Text>
          <Choices values={CATEGORIES} selected={form.categories} multiple onChange={(value: string[]) => set('categories', value)} />
        </Section>
      ) : null}

      {step === 1 ? (
        <Section title="Store location" subtitle="Use the exact entrance where Riders collect orders.">
          <FormField label="Store address" value={form.storeAddress} onChangeText={(value) => set('storeAddress', value)} placeholder="Building, street and area" multiline />
          ...
          <TouchableOpacity style={styles.locationButton} onPress={useLocation}>
            ...
            <Text style={styles.locationText}>{form.latitude ? 'Update pickup location' : 'Use current pickup location'}</Text>
          </TouchableOpacity>
        </Section>
      ) : null}

      {step === 2 ? (
        <Section title="Store operations" subtitle="These choices help AAGAM plan serviceability and pickup capacity.">
          <Text style={styles.label}>Operating hours</Text>
          <Choices values={HOURS} selected={form.operatingHours} onChange={(value: string) => set('operatingHours', value)} />
          <Text style={styles.label}>Service radius</Text>
          <Choices values={RADII.map((value) => `${value} km`)} selected={`${form.serviceRadiusKm} km`} onChange={(value: string) => set('serviceRadiusKm', value.replace(' km', ''))} />
          <Text style={styles.label}>Estimated orders per day</Text>
          <Choices values={CAPACITY} selected={form.orderCapacity} onChange={(value: string) => set('orderCapacity', value)} />
          <FormField label="Rider pickup instructions" value={form.pickupInstructions} onChangeText={(value) => set('pickupInstructions', value)} placeholder="Entrance, counter or parking guidance" multiline />
        </Section>
      ) : null}

      {step === 3 ? (
        <Section title="Settlement account" subtitle="Bank details are encrypted and used only for Store settlements.">
          <View style={styles.icon}><WalletCards size={22} color={palette.teal} /></View>
          {saved.bankAccountLast4 ? (
            <View style={styles.saved}><Check size={18} color={palette.green} /><Text style={styles.savedText}>Account saved ···· {saved.bankAccountLast4}</Text></View>
          ) : null}
          <FormField label="Account holder name" value={form.bankAccountHolderName} onChangeText={(value) => set('bankAccountHolderName', value)} placeholder="Name shown by the bank" />
          ...
          {form.bankAccountNumber ? <FormField label="Confirm account number" value={form.bankAccountConfirmation} onChangeText={(value) => set('bankAccountConfirmation', value.replace(/\D/g, ''))} keyboardType="number-pad" secureTextEntry placeholder="Re-enter account number" /> : null}
          ...
          <FormField label="GST, FSSAI or licence number" value={form.taxIdentifier} onChangeText={(value) => set('taxIdentifier', value.toUpperCase())} autoCapitalize="characters" placeholder="Optional when not applicable" />
        </Section>
      ) : null}
...
const styles = StyleSheet.create({
  icon: { width: 44, height: 44, borderRadius: 15, backgroundColor: '#ECFEFF', alignItems: 'center', justifyContent: 'center' },
  ...
  locationButton: { minHeight: 50, borderRadius: 15, borderWidth: 1, borderColor: '#99F6E4', backgroundColor: '#F0FDFA', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 },
  locationText: { color: palette.teal, fontSize: 12, fontWeight: '900' },
  saved: { borderRadius: 15, backgroundColor: '#ECFDF5', padding: 13, flexDirection: 'row', alignItems: 'center', gap: 8 },
  savedText: { color: palette.green, fontSize: 12, fontWeight: '900' },
