  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <Text style={styles.subtitle}>After Admin approval, use the verified application phone number. No separate activation password is required for phone-verified accounts.</Text>
        <View style={styles.securityCard}><ShieldCheck size={21} color="#0F766E" /><Text style={styles.securityText}>Rider and Store roles are loaded from the same secure account and stored in Android Keychain.</Text></View>
        <View style={styles.tabs}>
          <TouchableOpacity accessibilityLabel="Use phone OTP sign in" onPress={() => setMode('PHONE')} style={[styles.tab, mode === 'PHONE' && styles.tabActive]}><Phone size={17} color={mode === 'PHONE' ? '#fff' : '#64748B'} /><Text style={[styles.tabText, mode === 'PHONE' && styles.tabTextActive]}>Phone OTP</Text></TouchableOpacity>
          <TouchableOpacity accessibilityLabel="Use password sign in" onPress={() => setMode('PASSWORD')} style={[styles.tab, mode === 'PASSWORD' && styles.tabActive]}><Lock size={17} color={mode === 'PASSWORD' ? '#fff' : '#64748B'} /><Text style={[styles.tabText, mode === 'PASSWORD' && styles.tabTextActive]}>Password</Text></TouchableOpacity>
        </View>
        <View style={styles.form}>
          {mode === 'PHONE' ? !masked ? <><View style={styles.inputRow}><Phone size={18} color="#0F766E" /><TextInput accessibilityLabel="Approved partner mobile number" style={styles.input} placeholder="Approved mobile number" placeholderTextColor="#94A3B8" value={phone} onChangeText={setPhone} keyboardType="phone-pad" /></View><TouchableOpacity accessibilityLabel="Send partner OTP" style={[styles.button, loading && styles.disabled]} onPress={requestCode} disabled={loading}>{loading ? <ActivityIndicator color="#fff" /> : <><Text style={styles.buttonText}>Send OTP</Text><ArrowRight size={18} color="#fff" /></>}</TouchableOpacity></> : <><Text style={styles.sent}>Code sent to {masked}</Text><TouchableOpacity style={styles.otpRow} onPress={() => inputRef.current?.focus()}>{Array.from({ length: 6 }).map((_, index) => <View key={index} style={[styles.otpCell, code.length === index && styles.otpActive]}><Text style={styles.otpDigit}>{code[index] || ''}</Text></View>)}</TouchableOpacity><TextInput accessibilityLabel="Partner OTP code" ref={inputRef} value={code} onChangeText={updateCode} keyboardType="number-pad" textContentType="oneTimeCode" autoComplete="sms-otp" maxLength={6} autoFocus style={styles.hidden} /><TouchableOpacity accessibilityLabel="Verify partner OTP" style={[styles.button, loading && styles.disabled]} onPress={() => verifyCode()} disabled={loading || code.length !== 6}>{loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.buttonText}>Verify and sign in</Text>}</TouchableOpacity><TouchableOpacity onPress={() => countdown === 0 ? requestCode() : undefined}><Text style={styles.link}>{countdown > 0 ? `Resend in 00:${String(countdown).padStart(2, '0')}` : 'Resend OTP'}</Text></TouchableOpacity><TouchableOpacity onPress={() => { setMasked(''); setCode(''); }}><Text style={styles.secondaryLink}>Change mobile number</Text></TouchableOpacity></> : <><View style={styles.inputRow}><Mail size={18} color="#64748B" /><TextInput accessibilityLabel="Partner login identifier" style={styles.input} placeholder="Phone number or email" placeholderTextColor="#94A3B8" value={identifier} onChangeText={setIdentifier} autoCapitalize="none" /></View><View style={styles.inputRow}><Lock size={18} color="#64748B" /><TextInput accessibilityLabel="Partner login password" style={styles.input} placeholder="Password" placeholderTextColor="#94A3B8" value={password} onChangeText={setPassword} secureTextEntry autoCapitalize="none" /></View><TouchableOpacity accessibilityLabel="Partner login submit" style={[styles.button, loading && styles.disabled]} onPress={passwordLogin} disabled={loading}>{loading ? <ActivityIndicator color="#fff" /> : <><Text style={styles.buttonText}>Sign in</Text><ArrowRight size={18} color="#fff" /></>}</TouchableOpacity></>}
        </View>
