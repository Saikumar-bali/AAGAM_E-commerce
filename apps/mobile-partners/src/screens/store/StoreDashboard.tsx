...
export const StoreDashboard = () => {
  ...
  const stats: StoreStats = { totalInventory: 124, lowStockItems: 5, totalOrders: 48, revenue: 15420.50 };
  ...
  return (
    <ScrollView style={styles.container} refreshControl={<RefreshControl refreshing={isLoading} onRefresh={refetch} />}>
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.profileButton}><Settings size={24} color="#666" /></TouchableOpacity>
      </View>
      ...
      {Array.isArray(stores) && stores.map((store: any) => (
        <TouchableOpacity key={store.id} style={styles.storeCard} activeOpacity={0.7}>
          <View style={styles.storeInfo}>
            <View style={styles.storeAvatar}><Text style={styles.storeAvatarText}>{store.name[0]}</Text></View>
            <View>
...
const styles = StyleSheet.create({
  ...
  header: { padding: 24, paddingTop: 60, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#FFF', borderBottomWidth: 1, borderBottomColor: '#F1F5F9' },
  greeting: { fontSize: 14, color: '#64748B' },
  storeName: { fontSize: 24, fontWeight: 'bold', color: '#1E293B' },
  profileButton: { padding: 8, borderRadius: 12, backgroundColor: '#F1F5F9' },
  ...
  addButton: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#0F766E', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 10 },
