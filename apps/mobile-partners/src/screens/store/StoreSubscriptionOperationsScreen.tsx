...
function errorMessage(error: unknown) {
  ...
  return Array.isArray(value)
    ? value.join(", ")
    : value || candidate?.message || "The operation could not be completed.";
}
...
export const StoreSubscriptionOperationsScreen = ({
  navigation,
}: {
  navigation: NavigationProp<StoreTabParamList>;
}) => {
  ...
  const runsQuery = useQuery({
    queryKey: [...STORE_SUBSCRIPTION_KEY, "runs"],
    queryFn: subscriptionOperationsService.getStoreRuns,
    refetchInterval: 12_000,
    retry: 1,
  });
  const demandQuery = useQuery({
    queryKey: [...STORE_SUBSCRIPTION_KEY, "demand"],
    queryFn: () => subscriptionOperationsService.getStoreDemand(14),
    retry: 1,
  });
  const exceptionsQuery = useQuery({
    queryKey: [...STORE_SUBSCRIPTION_KEY, "exceptions"],
    queryFn: subscriptionOperationsService.getStoreExceptions,
    retry: 1,
  });
  const cashQuery = useQuery({
    queryKey: [...STORE_SUBSCRIPTION_KEY, "cash"],
    queryFn: subscriptionOperationsService.getStoreCashBatches,
  ...
  const stops = runs.reduce(
    (sum, run) => sum + Number(run.totalStopCount || run.stops.length || 0),
    0
  );
  const products = demand.reduce(
    (sum, row) =>
      sum +
      row.productTotals.reduce((itemSum, item) => itemSum + item.quantity, 0),
    0
  );
  const submittedCash = cashBatches
  ...
  const packingMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!Number.isInteger(packed) || packed < 1)
        throw new Error("Enter the number of packed bags.");
      ...
      if (packed !== expected && exceptionNote.trim().length < 5)
        throw new Error("Explain any bag-count difference.");
    ...
    onSuccess: async () => {
      ...
      Toast.show({
        type: "success",
        text1: "Route packing confirmed",
        text2: "Each order is packed and ready for rider pickup.",
      });
    },
    onError: (error) =>
      Toast.show({
        type: "error",
        text1: "Packing not confirmed",
        text2: errorMessage(error),
      }),
  });

  const pickupMutation = useMutation({
    mutationFn: (run: StoreRun) =>
      subscriptionOperationsService.confirmRunPickup(run.id, run.version),
    onSuccess: async () => {
      await refresh();
      Toast.show({
        type: "success",
        text1: "Store handoff confirmed",
        text2: "The rider must now independently verify the route bags.",
      });
    },
    onError: (error) =>
      Toast.show({
        type: "error",
        text1: "Handoff not confirmed",
        text2: errorMessage(error),
      }),
  });
  ...
  const verifyMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!Number.isFinite(paise) || paise < 0)
        throw new Error("Enter the independently counted physical amount.");
      if (settlementReference.trim().length < 3)
        throw new Error("Enter the store settlement reference.");
      if (
        paise !== cashBatch.expectedAmountPaise &&
        varianceReason.trim().length < 3
      )
        throw new Error(
          "A variance reason is required when the amount differs."
        );
    ...
    onSuccess: async (batch) => {
      ...
      await refresh();
      Toast.show({
        type: batch.status === "VARIANCE_REVIEW" ? "info" : "success",
        text1:
        ...
        text2:
          batch.status === "VARIANCE_REVIEW"
            ? "Admin review is required; individual COD ledgers remain auditable."
            : "All included individual COD ledgers were deposited.",
      });
    },
    onError: (error) =>
      Toast.show({
        type: "error",
        text1: "Cash verification failed",
        text2: errorMessage(error),
      }),
  });
    ...
    <View style={styles.screen}>
        ...
        refreshControl={
          <RefreshControl
            refreshing={
              ...
              demandQuery.isRefetching ||
              cashQuery.isRefetching ||
              exceptionsQuery.isRefetching
            }
      ...
      >
        <View style={styles.hero}>
          ...
          <View style={styles.heroMetrics}>
            <View style={styles.heroMetric}>
              ...
              <Text style={styles.heroMetricLabel}>routes today</Text>
            </View>
            <View style={styles.heroDivider} />
            <View style={styles.heroMetric}>
              <Text style={styles.heroMetricValue}>{stops}</Text>
              <Text style={styles.heroMetricLabel}>customer bags</Text>
            </View>
            <View style={styles.heroDivider} />
            <View style={styles.heroMetric}>
              <Text style={styles.heroMetricValue}>{money(submittedCash)}</Text>
          ...
          {(
            [
              ...
              [
                "cash",
                "Cash",
                cashBatches.filter((batch) => batch.status === "SUBMITTED")
                  .length,
              ],
            ...
            >
              ...
              {count > 0 ? (
                <View
                  style={[
                    styles.segmentCount,
                    section === value && styles.segmentCountActive,
                  ]}
                >
                  <Text
                    style={[
                      styles.segmentCountText,
                      section === value && styles.segmentCountTextActive,
                    ]}
                  >
                    {count > 99 ? "99+" : count}
                  </Text>
        ...
        {loading ? (
          ...
          <View style={styles.stateCard}>
            <CircleAlert size={42} color="#B42318" />
            <Text style={styles.stateTitle}>Operations unavailable</Text>
            <Text style={styles.stateText}>{errorMessage(currentError)}</Text>
            <TouchableOpacity
        ...
        {!loading && !currentError && section === "runs" ? (
            ...
            {runs.length ? (
              runs.map((run) => (
                <RunCard
                  ...
                  onPickup={() => pickupMutation.mutate(run)}
                  pickupBusy={pickupMutation.isPending}
                />
              ))
            ) : (
              <EmptyState
        ...
        {!loading && !currentError && section === "forecast" ? (
          <View style={styles.sectionBody}>
            {demand.length ? (
              demand.map((row) => (
                <View
                ...
                >
                  <View style={styles.forecastTop}>
                    <View>
                      ...
                      <Text style={styles.forecastStops}>
                        {row.stopCount} customer bag
                        {row.stopCount === 1 ? "" : "s"}
                      </Text>
                    </View>
        ...
        {!loading && !currentError && section === "cash" ? (
          <View style={styles.sectionBody}>
            {cashBatches.length ? (
              cashBatches.map((batch) => (
                <View key={batch.id} style={styles.cashCard}>
                  <View style={styles.cashHeader}>
                    <View style={styles.cashIcon}>
                      <Banknote size={22} color="#A15C00" />
                    </View>
                    <View style={styles.cashCopy}>
                      <Text style={styles.cashReference}>
                        {batch.reference}
                      </Text>
                  ...
                  {batch.status === "SUBMITTED" ? (
                    ...
                    >
                      <ClipboardCheck size={19} color="#FFFFFF" />
        ...
        {!loading && !currentError && section === "exceptions" ? (
          <View style={styles.sectionBody}>
            {exceptions.length ? (
              exceptions.map((item) => (
                <View key={item.id} style={styles.exceptionCard}>
                  <CircleAlert size={22} color="#B42318" />
                  <View style={styles.exceptionCopy}>
                    <Text style={styles.exceptionTitle}>
                      Stop {item.sequenceNumber} · {label(item.status)}
                    </Text>
                    <Text style={styles.exceptionText}>
                      {item.failureReason || "Operational follow-up required"}
                    </Text>
                    <Text style={styles.exceptionRoute}>
                      {item.deliveryRun?.routeCode || "Route"} ·{" "}
                      {item.deliveryRun?.store?.name || "Store"}
                    </Text>
                  </View>
                </View>
              ))
      ...
      <Modal
        visible={Boolean(packingRun)}
        transparent
        animationType="slide"
        onRequestClose={() => setPackingRun(null)}
      >
        <View style={styles.modalBackdrop}>
          <View style={styles.sheet}>
            ...
            <View style={styles.sheetHeader}>
              <View>
                <Text style={styles.sheetEyebrow}>ROUTE PACKING</Text>
                <Text style={styles.sheetTitle}>{packingRun?.routeCode}</Text>
              </View>
              <TouchableOpacity
                style={styles.closeButton}
                onPress={() => setPackingRun(null)}
              >
            ...
            >
              ...
              <Text style={styles.inputLabel}>Packed bag count</Text>
              <TextInput
                style={styles.input}
                keyboardType="number-pad"
                value={packedBagCount}
                onChangeText={(value) =>
                  setPackedBagCount(value.replace(/\D/g, ""))
                }
                placeholder="0"
              ...
              <View style={styles.scanInput}>
                ...
                <TextInput
                  ...
                  value={crateCode}
                  onChangeText={setCrateCode}
                  placeholder="Scan or enter crate code"
                  placeholderTextColor="#94A3B8"
                  autoCapitalize="characters"
                />
              </View>
              <Text style={styles.inputLabel}>Exception note</Text>
              <TextInput
                style={[styles.input, styles.noteInput]}
                multiline
                value={exceptionNote}
                onChangeText={setExceptionNote}
                placeholder="Required only for a bag-count or packing exception"
                placeholderTextColor="#94A3B8"
              />
              <TouchableOpacity
                style={styles.sheetPrimary}
                disabled={packingMutation.isPending}
                onPress={() => packingMutation.mutate()}
              >
                <PackageCheck size={20} color="#FFFFFF" />
                <Text style={styles.sheetPrimaryText}>
                  {packingMutation.isPending
                    ? "Confirming…"
                    : "Confirm route packing"}
                </Text>
              </TouchableOpacity>
            </ScrollView>
          </View>
      ...
      <Modal
        visible={Boolean(cashBatch)}
        transparent
        animationType="slide"
        onRequestClose={() => setCashBatch(null)}
      >
        <View style={styles.modalBackdrop}>
          <View style={styles.sheet}>
            <View style={styles.handle} />
            <View style={styles.sheetHeader}>
              <View>
                <Text style={styles.sheetEyebrow}>INDEPENDENT CASH COUNT</Text>
                <Text style={styles.sheetTitle}>{cashBatch?.reference}</Text>
              </View>
              <TouchableOpacity
                style={styles.closeButton}
                onPress={() => setCashBatch(null)}
              >
                <X size={21} color="#475569" />
              </TouchableOpacity>
            </View>
            <ScrollView
              ...
              keyboardShouldPersistTaps="handled"
            >
              <View style={styles.cashCompare}>
                <Metric
                  label="Server expected"
                  value={money(cashBatch?.expectedAmountPaise || 0)}
                />
                <Metric
                  label="Rider submitted"
                  value={money(cashBatch?.submittedAmountPaise || 0)}
                />
              </View>
              <Text style={styles.inputLabel}>Amount physically counted</Text>
              <TextInput
              ...
              <TextInput
                ...
                placeholder="Store receipt / register reference"
                placeholderTextColor="#94A3B8"
                autoCapitalize="characters"
              />
              <Text style={styles.inputLabel}>
                Variance reason (required when different)
              </Text>
              <TextInput
                style={[styles.input, styles.noteInput]}
                multiline
                ...
                placeholder="Count discrepancy, damaged note, missing cash…"
                placeholderTextColor="#94A3B8"
              />
              <Text style={styles.auditNote}>
                Verification updates every included COD ledger with immutable
                deposit entries. A difference creates VARIANCE_REVIEW; it is
...
function RunCard({
  ...
  for (const stop of run.stops)
    for (const item of stop.subscriptionDelivery.order?.items || [])
      productTotals.set(
        item.product.name,
        (productTotals.get(item.product.name) || 0) + item.quantity
      );
  return (
    <View style={styles.runCard}>
      <View style={styles.runHeader}>
        ...
        <View style={styles.runCopy}>
          ...
          {(run as any).slotStart && (run as any).slotEnd ? <Text style={styles.runWindow}>{new Date((run as any).slotStart).toLocaleDateString("en-IN", { weekday: "short", day: "numeric", month: "short", timeZone: "Asia/Kolkata" })} · {new Date((run as any).slotStart).toLocaleTimeString("en-IN", { hour: "numeric", minute: "2-digit", timeZone: "Asia/Kolkata" })}–{new Date((run as any).slotEnd).toLocaleTimeString("en-IN", { hour: "numeric", minute: "2-digit", timeZone: "Asia/Kolkata" })}</Text> : null}
          <Text style={styles.runRider}>
            {run.rider?.user?.name
      ...
      <View style={styles.runMetrics}>
        ...
        <Metric
          label="Bags"
          value={String(
            run.expectedBagCount || run.totalStopCount || run.stops.length
          )}
        />
        <Metric label="Cash due" value={money(run.expectedCashPaise)} />
      </View>
      {productTotals.size ? (
        <View style={styles.productSummary}>
          {[...productTotals.entries()].map(([name, quantity]) => (
            <Text key={name} style={styles.productSummaryText}>
              {quantity} × {name}
            </Text>
          ))}
        </View>
      ...
      <View style={styles.customerList}>
        {run.stops.slice(0, 4).map((stop) => (
          <View key={stop.id} style={styles.customerRow}>
            <Text style={styles.customerSequence}>{stop.sequenceNumber}</Text>
            <Text style={styles.customerText} numberOfLines={1}>
              {stop.subscriptionDelivery.order?.customer?.name || "Customer"} ·{" "}
              {stop.expectedParcelCount} bag
            </Text>
            <Text style={styles.customerProof}>
              {stop.cashDuePaise > 0 ? money(stop.cashDuePaise) : "₹0 funded"}
            </Text>
          </View>
        ))}
        {run.stops.length > 4 ? (
          <Text style={styles.moreCustomers}>
            + {run.stops.length - 4} more customer bags
          </Text>
        ) : null}
      </View>
      {run.status === "PLANNED" ? (
        <TouchableOpacity style={styles.runPrimary} onPress={onPack}>
          <PackageCheck size={19} color="#FFFFFF" />
          <Text style={styles.runPrimaryText}>Verify and confirm packing</Text>
        </TouchableOpacity>
      ) : null}
      {run.status === "READY_FOR_PICKUP" && !run.storeHandoffConfirmedAt ? (
        <TouchableOpacity
        ...
        >
          ...
          <Text style={styles.runPrimaryText}>
            {pickupBusy ? "Confirming…" : "Confirm store handoff"}
          </Text>
        </TouchableOpacity>
      ) : null}
      {run.status === "READY_FOR_PICKUP" && run.storeHandoffConfirmedAt ? (
        <View style={styles.productSummary}>
          <Text style={styles.productSummaryText}>
            Store handoff recorded · waiting for rider bag receipt
          </Text>
        </View>
      ) : null}
    </View>
...
function Metric({
  ...
  return (
    <View style={styles.metric}>
      <Text style={[styles.metricValue, danger && styles.metricDanger]}>
        {value}
      </Text>
...
const styles = StyleSheet.create({
  ...
  scanTextInput: { flex: 1, color: "#17211D", fontSize: 14 },
  sheetPrimary: {
    minHeight: 54,
    marginTop: 16,
    borderRadius: 16,
