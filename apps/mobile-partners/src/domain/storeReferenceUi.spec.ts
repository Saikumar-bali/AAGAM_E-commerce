...
describe('store reference UI helpers', () => {
  it('maps live order statuses into the approved order tabs', () => {
    ...
    expect(orderStatusTab('CANCELLED')).toBeNull();
  });

  it('summarizes the visible queue without inventing totals', () => {
    expect(summarizeStoreOrders([
      ...
      { status: 'DELIVERED' },
    ])).toEqual({ NEW: 1, PREPARING: 1, READY: 1, PICKUP: 1, DELIVERED: 1 });
  });
