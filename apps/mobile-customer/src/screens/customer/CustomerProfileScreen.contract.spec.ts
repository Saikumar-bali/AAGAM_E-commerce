...
describe('Customer profile address validation contract', () => {
  ...
  test('requires textual address fields while location evidence follows the selected mode', () => {
    expect(source).toContain("{ key: 'recipientName', label: 'Name', required: true");
    expect(source).toContain("{ key: 'phoneE164', label: 'Phone', required: true");
    expect(source).toContain("{ key: 'line1', label: 'House / Street', required: true");
    ...
    expect(source).toContain('Use my location');
    expect(source).toContain('Search on map');
  ...
  test('keeps optional address details optional', () => {
    ...
    expect(source).toContain("{ key: 'line2', label: 'Area / Locality'");
    expect(source).toContain("{ key: 'landmark', label: 'Nearby'");
    expect(source).toContain("{ key: 'instructions', label: 'Note for rider'");
  });
});
