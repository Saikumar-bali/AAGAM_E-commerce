...
import { LeafletMap, useAuthStore, apiClient } from '@aagam/mobile-shared';
...
export const CustomerProfileScreen = () => {
  ...
  const useCurrentLocation = async () => {
    ...
    Geolocation.getCurrentPosition(
      (position) => { setPinnedLocation(position.coords.latitude, position.coords.longitude); },
      ...
      {isLoading ? (
        ...
        addresses.map((address: any) => (
          <View key={address.id} style={styles.addressCard}>
            <Text style={styles.addressLabel}>{address.label || 'Address'} {address.isDefault ? '• Default' : ''}</Text>
        ...
        <View style={styles.formCard}>
          ...
          <View style={styles.locationPanel}>
            ...
            <Text style={styles.locationHelp}>
              {hasPinnedLocation ? `Pinned: ${pinnedLatitude.toFixed(5)}, ${pinnedLongitude.toFixed(5)}` : 'Tap the map or use current location to pin delivery point.'}
            </Text>
          </View>
          {[
            ['label', 'Label'],
          ...
          <View style={styles.switchRow}>
            ...
            <Switch value={draft.isDefault} onValueChange={(value) => setDraft((prev) => ({ ...prev, isDefault: value }))} />
          </View>
          <TouchableOpacity style={styles.saveButton} onPress={() => saveAddressMutation.mutate()}>
            <Text style={styles.saveButtonText}>{saveAddressMutation.isPending ? 'Saving...' : 'Save Address'}</Text>
          </TouchableOpacity>
        </View>
      ) : null}
