...
const createNativeDraft = (
  profile?: { name?: string | null; phone?: string | null },
  isDefault = false,
) => ({
  ...createCustomerAddressDraft(profile, isDefault),
  latitude: "",
  longitude: "",
});
...
export const CustomerProfileScreen = () => {
  ...
  const [draft, setDraft] = useState(() => createNativeDraft());
  ...
  useEffect(() => {
    if (!showForm)
      setDraft(createNativeDraft(user || undefined, addresses.length === 0));
  }, [addresses.length, showForm, user]);
  ...
  const saveAddressMutation = useMutation({
    mutationFn: async () =>
      apiClient.post(
        ...
        buildCustomerAddressPayload(draft, {
          fallbackProfile: user || undefined,
          isDefault: draft.isDefault || addresses.length === 0,
        }),
      ),
    onSuccess: async () => {
      setDraft(createNativeDraft(user || undefined));
      setShowForm(false);
      await refetch();
    ...
    onError: (error: any) =>
      Toast.show({
        ...
        text2: getApiErrorMessage(error, "Please check the form."),
      }),
  });
    ...
    try {
      ...
      if (response.data?.ok && address)
        setDraft((prev) => ({
          ...
          pincode: prev.pincode || cleanIndianPincode(address.pincode),
        }));
    } catch {}
  };
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.menuCard}>
        <MenuRow
          title="My Orders"
          subtitle="Track, reorder, and review deliveries"
          onPress={() => navigation.navigate("Orders")}
        />
        <MenuRow
          title="Alerts"
          subtitle="Order and support notifications"
          onPress={() => navigation.navigate("Alerts")}
        />
        <MenuRow
          title="Push Notifications"
          subtitle="Register this device for updates"
          onPress={enablePush}
        />
        <MenuRow
          title="Customer Support"
          subtitle="Open support from delivered order details"
          onPress={() => navigation.navigate("Orders")}
        />
        <MenuRow
          title="Account Security"
          subtitle="Google OAuth primary, email password fallback"
          onPress={() =>
            Alert.alert(
              "Account security",
              "Google sign-in is the preferred customer login. Email/password remains available as fallback.",
            )
          }
        />
      </View>
      ...
      <View style={styles.sectionHeader}>
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            if (!showForm)
              setDraft(
                createNativeDraft(user || undefined, addresses.length === 0),
              );
          ...
          <View key={address.id} style={styles.addressCard}>
            <View style={styles.addressTop}>
              ...
              {!address.isDefault ? (
                ...
                >
                  <Text style={styles.smallAction}>Make default</Text>
                </TouchableOpacity>
              ) : null}
            </View>
        ...
        <View style={styles.formCard}>
          ...
          {[
            ...
            <TextInput
              ...
              onChangeText={(value) =>
                setDraft((prev) => ({
                  ...prev,
                  [key]: key === "pincode" ? cleanIndianPincode(value) : value,
                }))
              }
              placeholder={label}
          ...
          <View style={styles.switchRow}>
            ...
            <Switch
              ...
              onValueChange={(value) =>
                setDraft((prev) => ({ ...prev, isDefault: value }))
              }
            />
          </View>
          <TouchableOpacity
            style={[
              styles.saveButton,
              saveAddressMutation.isPending && { opacity: 0.6 },
            ]}
...
const styles = StyleSheet.create({
  ...
  headerLogoutText: { color: "#115E59", fontSize: 22, fontWeight: "900" },
  avatar: {
    width: 56,
    height: 56,
    borderRadius: 20,
