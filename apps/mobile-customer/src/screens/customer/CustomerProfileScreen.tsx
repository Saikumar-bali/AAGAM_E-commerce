...
const emptyDraft = {
  label: 'Home', recipientName: '', phoneE164: '', alternatePhoneE164: '', line1: '', line2: '', landmark: '', city: '', state: '', pincode: '', country: 'IN', latitude: '', longitude: '', instructions: '', isDefault: false,
};
...
export const CustomerProfileScreen = () => {
  ...
  const deleteAddressMutation = useMutation({
    ...
    onSuccess: async () => { await refetch(); Toast.show({ type: 'success', text1: 'Address removed' }); },
    onError: (error: any) => Toast.show({ type: 'error', text1: 'Could not remove address', text2: error.response?.data?.message || 'Try again.' }),
  });

  const setDefaultMutation = useMutation({
    mutationFn: async (id: string) => apiClient.patch(`/customer/addresses/${id}`, { isDefault: true }),
    onSuccess: async () => { await refetch(); Toast.show({ type: 'success', text1: 'Default address updated' }); },
    onError: (error: any) => Toast.show({ type: 'error', text1: 'Could not update address', text2: error.response?.data?.message || 'Try again.' }),
  });
  ...
  const reverseGeocode = async (latitude: number, longitude: number) => {
    try {
      ...
      if (response.data?.ok && address) setDraft((prev) => ({ ...prev, line1: prev.line1 || address.line1 || '', landmark: prev.landmark || address.landmark || '', city: prev.city || address.city || '', state: prev.state || address.state || '', pincode: prev.pincode || address.pincode || '' }));
    } catch {}
  };

  const setPinnedLocation = async (latitude: number, longitude: number) => {
    setDraft((prev) => ({ ...prev, latitude: String(latitude), longitude: String(longitude) }));
    await reverseGeocode(latitude, longitude);
  };

  const useCurrentLocation = async () => {
    const hasPermission = await requestLocationPermission();
    if (!hasPermission) { Alert.alert('Location permission needed', 'Allow location permission or tap the map to pin manually.'); return; }
    Geolocation.getCurrentPosition((position) => setPinnedLocation(position.coords.latitude, position.coords.longitude), () => Alert.alert('Location error', 'Could not get current location. You can still tap the map to pin.'), { enableHighAccuracy: true, timeout: 15000, maximumAge: 10000 });
  };

  const confirmDeleteAddress = (address: any) => {
    Alert.alert('Delete address?', `Remove ${address.label || 'this address'} from your profile?`, [
  ...
  const enablePush = async () => {
    ...
    Toast.show({ type: 'success', text1: 'Notification setup checked', text2: 'If Firebase is configured, this device will receive alerts.' });
  };
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <View style={styles.heroCard}>
        ...
        <View style={{ flex: 1 }}><Text style={styles.name}>{user?.name || 'Customer'}</Text><Text style={styles.email}>{user?.email}</Text></View>
      </View>
      ...
      <View style={styles.menuCard}>
        ...
        <MenuRow title="Customer Support" subtitle="Open order support from delivered order details" onPress={() => navigation.navigate('Orders')} />
        <MenuRow title="Account Security" subtitle="Email login, Google login, phone login readiness" onPress={() => Alert.alert('Account security', 'Phone login will be enabled after Firebase token verification is added to the backend.')} />
      </View>
      ...
      {showForm ? <View style={styles.formCard}>
        ...
        ].map(([key, label]) => <TextInput key={key} value={(draft as any)[key]} onChangeText={(value) => setDraft((prev) => ({ ...prev, [key]: value }))} placeholder={label} placeholderTextColor="#94A3B8" style={styles.input} />)}
        <View style={styles.switchRow}><Text style={styles.switchText}>Set as default</Text><Switch value={draft.isDefault} onValueChange={(value) => setDraft((prev) => ({ ...prev, isDefault: value }))} /></View>
      ...
      <TouchableOpacity style={styles.logoutButton} onPress={logout}><Text style={styles.logoutButtonText}>Logout</Text></TouchableOpacity>
    </ScrollView>
  );
};
...
const styles = StyleSheet.create({
  ...
  heroCard: { flexDirection: 'row', gap: 14, alignItems: 'center', borderRadius: 26, backgroundColor: '#0F766E', padding: 20 },
  avatar: { width: 56, height: 56, borderRadius: 20, backgroundColor: '#CCFBF1', alignItems: 'center', justifyContent: 'center' },
  avatarText: { color: '#115E59', fontSize: 24, fontWeight: '900' },
  name: { fontSize: 24, fontWeight: '900', color: '#FFFFFF' },
  email: { marginTop: 6, color: '#CCFBF1', fontWeight: '700' },
  statsRow: { flexDirection: 'row', gap: 10, marginTop: 14 },
  statCard: { flex: 1, backgroundColor: '#FFFFFF', borderRadius: 18, padding: 14, borderWidth: 1, borderColor: '#E2E8F0' },
  statValue: { fontSize: 22, fontWeight: '900', color: '#0F172A' },
  statLabel: { marginTop: 4, color: '#64748B', fontSize: 12, fontWeight: '800' },
  menuCard: { marginTop: 16, backgroundColor: '#FFFFFF', borderRadius: 22, borderWidth: 1, borderColor: '#E2E8F0', overflow: 'hidden' },
  menuRow: { flexDirection: 'row', alignItems: 'center', padding: 16, borderBottomWidth: 1, borderBottomColor: '#F1F5F9' },
  menuTitle: { color: '#0F172A', fontWeight: '900', fontSize: 15 },
  menuSubtitle: { marginTop: 4, color: '#64748B', fontWeight: '700', fontSize: 12 },
  ...
  linkButtonText: { color: '#115E59', fontWeight: '900' },
  emptyCard: { marginTop: 12, borderRadius: 18, borderWidth: 1, borderColor: '#E2E8F0', backgroundColor: '#FFFFFF', padding: 16 },
  emptyTitle: { color: '#0F172A', fontWeight: '900' },
  emptyText: { marginTop: 4, color: '#64748B' },
  addressCard: { marginTop: 12, backgroundColor: '#FFFFFF', borderRadius: 18, borderWidth: 1, borderColor: '#E2E8F0', padding: 16 },
  addressTop: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: 10 },
  addressLabel: { fontSize: 12, fontWeight: '900', color: '#0F766E', textTransform: 'uppercase', flex: 1 },
  smallAction: { color: '#0F766E', fontWeight: '900', fontSize: 12 },
  addressName: { marginTop: 6, fontSize: 16, fontWeight: '900', color: '#0F172A' },
  ...
  locationPanel: { borderRadius: 18, backgroundColor: '#F0FDFA', borderWidth: 1, borderColor: '#CCFBF1', padding: 10, marginBottom: 12 },
  locationButton: { alignItems: 'center', borderRadius: 14, backgroundColor: '#0F766E', paddingVertical: 12, marginBottom: 10 },
  locationButtonText: { color: '#FFFFFF', fontWeight: '900' },
  locationHelp: { marginTop: 8, color: '#115E59', fontWeight: '700', fontSize: 12, textAlign: 'center' },
  input: { borderWidth: 1, borderColor: '#E2E8F0', backgroundColor: '#FFFFFF', borderRadius: 14, paddingHorizontal: 14, paddingVertical: 12, color: '#0F172A', marginBottom: 10 },
  ...
  logoutButtonText: { color: '#FFFFFF', fontWeight: '900' },
});
