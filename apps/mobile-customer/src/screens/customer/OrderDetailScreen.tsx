        ...
        <View style={styles.bannerContainer}>
          <View style={[styles.banner, styles.bannerAssigned]}>
            ...
            <Text style={styles.bannerText}>
              {riderName ? `${riderName} is heading to pick up your order` : 'Rider is heading to pick up your order'}
            </Text>
          </View>
        </View>
      );
      ...
      return (
        <View style={styles.bannerContainer}>
          <View style={[styles.banner, styles.bannerStale]}>
            ...
            <Text style={[styles.bannerText, { color: '#92400E' }]}>Tracking paused — waiting for rider location update</Text>
          </View>
        </View>
      );
    case 'DELIVERED':
...
export const OrderDetailScreen = () => {
  ...
  const buildRoutePath = () => {
    if (!tracking?.routePath || tracking.routePath.length < 2) return [];
    return tracking.routePath.map((p: any) => ({ latitude: p.latitude, longitude: p.longitude }));
  };
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
          ...
          {trackingPayload?.rider && (
            <View style={styles.riderInfo}>
              ...
              {trackingPayload.rider.phone && (
                <TouchableOpacity style={styles.callBtn} onPress={() => Linking.openURL(`tel:${trackingPayload.rider.phone}`)}>
      ...
      <View style={styles.card}>
        ...
        {(trackingPayload.timeline || []).map((event: any) => (
          <View key={event.id} style={styles.timelineRow}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={[styles.boldText, event.toStatus === order.status && styles.timelineTextActive]}>
      ...
      <View style={styles.card}>
        ...
        <Text style={styles.bodyText}>Method: {order.payment?.method || 'N/A'}</Text>
        <Text style={styles.bodyText}>Status: {order.payment?.status || 'N/A'}</Text>
