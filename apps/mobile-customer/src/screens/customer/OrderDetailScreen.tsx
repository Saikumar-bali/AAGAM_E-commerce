...
const TrackingStateBanner = ({
  ...
  switch (state) {
    ...
    case "ASSIGNED_NO_LOCATION":
      return (
        <View style={styles.bannerContainer}>
          <View style={[styles.banner, styles.bannerAssigned]}>
            <View style={[styles.bannerDot, { backgroundColor: "#3B82F6" }]} />
            <Text style={styles.bannerText}>
              {riderName
                ? `${riderName} is heading to pick up your order`
                : "Rider is heading to pick up your order"}
            </Text>
          </View>
        </View>
      );
    case "STALE":
      return (
        <View style={styles.bannerContainer}>
          <View style={[styles.banner, styles.bannerStale]}>
            <View style={[styles.bannerDot, { backgroundColor: "#F59E0B" }]} />
            <Text style={[styles.bannerText, { color: "#92400E" }]}>
              Tracking paused — waiting for rider location update
            </Text>
          </View>
        </View>
      );
...
export const OrderDetailScreen = () => {
  const route =
    useRoute<RouteProp<Record<string, { orderId: string }>, string>>();
  ...
  const {
    ...
    queryFn: async () =>
      (await apiClient.get(`/tracking/my/order/${orderId}`)).data,
  ...
  useEffect(() => {
    ...
    on("riderLocationUpdated", (payload: any) => {
      if (payload.orderId === orderId) setLiveTracking(payload);
    });
    on("riderMoved", (payload: any) => {
      if (payload.orderId === orderId) setLiveTracking(payload);
    });
    on("orderTimelineUpdated", (payload: any) => {
      if (payload.order?.id === orderId) void refetch();
    });
    on("orderStatusUpdated", (payload: any) => {
      if (payload.orderId === orderId) void refetch();
    });
    on("trackingStopped", (payload: any) => {
      if (payload.orderId === orderId) void refetch();
    });
    return () => {
  ...
  const buildRoutePath = () =>
    !tracking?.routePath || tracking.routePath.length < 2
      ? []
      : tracking.routePath.map((point: any) => ({
  ...
  const getPingAgeText = () => {
    ...
    const ageSeconds = Math.floor(
      (Date.now() - new Date(lastPingAt).getTime()) / 1000
    );
  ...
  const orderItems =
    currentItems.length > 0
      ...
      ? order.itemsSnapshot
      : [];
  const currentItemSubtotal = currentItems.reduce(
    (sum: number, item: any) => sum + normalizeOrderLine(item).lineTotal,
    0
  );
  const pricing = normalizeOrderPricing(
    currentItems.length > 0
      ? { ...order, subtotal: currentItemSubtotal, subtotalPaise: undefined }
      : order,
    orderItems
  );
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
          ...
          <View style={styles.trackingInfo}>
            {etaMinutes ? (
              <View style={styles.trackingInfoItem}>
                ...
                <Text style={styles.trackingInfoValue}>{etaMinutes} min</Text>
              </View>
            ) : null}
            {distanceKm != null ? (
              <View style={styles.trackingInfoItem}>
                <Text style={styles.trackingInfoLabel}>Distance</Text>
                <Text style={styles.trackingInfoValue}>{distanceKm} km</Text>
              </View>
            ) : null}
            <View style={styles.trackingInfoItem}>
              <Text style={styles.trackingInfoLabel}>Last Update</Text>
              <Text style={styles.trackingInfoValue}>{getPingAgeText()}</Text>
            </View>
          </View>
            ...
            <View style={styles.riderInfo}>
              <View style={styles.riderInfoRow}>
                ...
                <Text style={styles.riderName}>
                  {trackingPayload.rider.name || "Assigned"}
                </Text>
              </View>
        ...
        {(trackingPayload.timeline || []).map((event: any) => (
          <View key={event.id} style={styles.timelineRow}>
            <View
              style={[
                styles.timelineDot,
                event.toStatus === order.status && styles.timelineDotActive,
              ]}
            />
            <View style={{ flex: 1 }}>
              <Text
                style={[
                  styles.boldText,
                  event.toStatus === order.status && styles.timelineTextActive,
                ]}
              >
                {String(event.toStatus).replace(/_/g, " ")}
              </Text>
      ...
      <View style={styles.card}>
        ...
        <Text style={styles.bodyText}>
          Method: {trackingPayload.payment?.method || "N/A"}
        </Text>
        <Text style={styles.bodyText}>
