...
const bottomNavScrollClearance = 124;
const issueTypes = [
...
export const SubscriptionDetailScreen = () => {
  ...
  const route =
    useRoute<RouteProp<CustomerStackParamList, "SubscriptionDetail">>();
  ...
  const query = useQuery({
    ...
    queryFn: () => subscriptionService.one(id),
  });
  const deliveriesQuery = useQuery({
    queryKey: ["subscription-deliveries", id],
    queryFn: () => subscriptionService.deliveries(id),
  });
  const refresh = async () => {
    await Promise.all([
  ...
  const lifecycle = useMutation({
    mutationFn: async (kind: LifecycleAction) => {
      if (kind === "pause")
        return subscriptionService.pause(
          id,
          tomorrow(),
          "Customer requested pause"
        );
      if (kind === "resume") return subscriptionService.resume(id);
      const next = (deliveriesQuery.data ?? []).find(
        (delivery) => delivery.status === "SCHEDULED"
      );
      if (next)
        return subscriptionService.skip(id, next.id, "Customer requested skip");
  ...
  const tracking = useMutation({
    ...
    onSuccess: (data) => {
      if (data.orderId)
        ...
        notify.info(
          "No active delivery",
          "Tracking becomes available after the next order is generated."
        );
    },
    onError: (error) =>
      notify.error(
  ...
  const reportableDelivery = useMemo(
    () =>
  ...
  const issue = useMutation({
    mutationFn: () => {
      if (!reportableDelivery)
        throw new Error("No delivery is available to report.");
      return subscriptionService.reportIssue(
        id,
        reportableDelivery.id,
        issueType,
    ...
    onSuccess: () => {
      notify.success(
        "Issue reported",
        "Aagaam operations can now review this delivery."
      );
      setIssueDescription("");
      setSheet(null);
    },
  ...
  const revokeQr = useMutation({
    ...
    onSuccess: () => {
      ...
      setQrExpiresAt(null);
      setSheet(null);
      notify.success("Trusted Drop QR revoked");
    },
    onError: (error) =>
      notify.error(
        "Revoke failed",
  ...
  const cancel = useMutation({
    ...
    onSuccess: async () => {
      notify.success(
        ...
        "Future ungenerated deliveries have been cancelled safely."
      );
      setSheet(null);
      await refresh();
    },
  ...
  if (!query.data)
    return (
      <View style={styles.center}>
        <Text>Subscription could not be loaded.</Text>
      </View>
    );
  const subscription = query.data;
  const total = Number(
    subscription.planVersion?.totalDeliveries || deliveries.length || 1
  );
  const percent = Math.min(
    100,
    Math.round((Number(subscription.completedDeliveries || 0) / total) * 100)
  );
  const next = deliveries.find((delivery) =>
    activeDeliveryStatuses.includes(delivery.status)
  );
    ...
    <SafeAreaView style={styles.screen}>
      <View style={styles.header}>
        <Pressable onPress={() => navigation.goBack()} style={styles.icon}>
          <ArrowLeft size={22} color="#173D32" />
        </Pressable>
      ...
      >
        <View style={styles.hero}>
          <View style={styles.heroTop}>
            <View>
              <Text style={styles.heroLabel}>PLAN PROGRESS</Text>
              <Text style={styles.heroNumber}>
                {subscription.completedDeliveries} of {total}
              </Text>
              <Text style={styles.heroSub}>deliveries completed</Text>
            </View>
          ...
          <View style={styles.heroFacts}>
            <Text style={styles.heroFact}>
              ...
              <Text style={styles.heroFactStrong}>
                {subscription.remainingFundedDeliveries}
              </Text>
            </Text>
            <Text style={styles.heroFact}>
              Skipped{" "}
        ...
        <View style={styles.actions}>
          ...
          {subscription.status === "PAUSED" ? (
            <Pressable
              disabled={lifecycle.isPending || !canChange}
          ...
          <Pressable onPress={() => setSheet("issue")} style={styles.action}>
        ...
        <View style={styles.section}>
          ...
          <Info
            ...
            label="Funding"
            value={
              subscription.fundingCycle === "WEEKLY"
                ? "Cash every seven deliveries"
                : "Full amount on first delivery"
            }
          />
        </View>
        <View style={styles.section}>
        ...
        {subscription.fundingAllocations?.length ? (
          <View style={styles.section}>
              ...
              <View key={allocation.id} style={styles.receipt}>
                ...
                <View style={styles.flex}>
                  ...
                  <Text style={styles.deliveryMeta}>
                    Deliveries {allocation.startsAtSequence}–
                    {allocation.endsAtSequence} · {date(allocation.createdAt)}
                  </Text>
                </View>
      ...
      >
        <Text style={styles.sheetCopy}>
          {reportableDelivery
            ? `Reporting day ${reportableDelivery.sequenceNumber} · ${date(
                reportableDelivery.serviceDate
              )}`
        ...
        <View style={styles.chips}>
          {issueTypes.map((type) => (
            <Pressable
              ...
              style={[
                styles.chip,
                issueType === type.value && styles.chipActive,
              ]}
            >
              <Text
                style={[
                  styles.chipText,
                  issueType === type.value && styles.chipTextActive,
                ]}
              >
                {type.label}
              </Text>
        ...
        <PrimaryButton
          ...
          disabled={!reportableDelivery || issueDescription.trim().length < 5}
      ...
      {subscription.deliveryMethod === "TRUSTED_DROP" ? (
        ...
        >
          <Text style={styles.sheetCopy}>
            Show this one-time signed QR only at the delivery point. The rider
            must scan it and capture a fresh delivery photo. Never send the
            token as text.
          </Text>
          {qrImage ? (
            <Image
          ...
          <Text style={styles.sheetCopy}>
            {qrExpiresAt
              ? `Expires ${new Date(qrExpiresAt).toLocaleString("en-IN")}`
              : ""}
          </Text>
          <PrimaryButton
            label="Rotate QR"
            busy={showQr.isPending}
            onPress={() => showQr.mutate(true)}
          />
          <Pressable
            onPress={() => revokeQr.mutate()}
            style={styles.cancelButton}
          >
            <ShieldX size={18} color="#A33131" />
            <Text style={styles.cancelText}>Revoke QR</Text>
          </Pressable>
        </ActionSheet>
      ) : null}
      <ActionSheet
      ...
      >
        <Text style={styles.sheetCopy}>
          Generated orders remain in the normal order workflow. Future
          ungenerated occurrences will be cancelled with an audit record.
        </Text>
        <TextInput
          value={cancelReason}
...
const DeliveryRow = ({ delivery, startMinute, endMinute }: { delivery: SubscriptionDelivery; startMinute: number; endMinute: number }) => (
  <View style={styles.deliveryRow}>
    <View
      style={[
        styles.dot,
        delivery.status === "DELIVERED" && styles.dotDone,
        delivery.status === "SKIPPED" && styles.dotSkipped,
      ]}
    >
    ...
    <View style={styles.flex}>
      <Text style={styles.deliveryDate}>{date(delivery.serviceDate)}</Text>
      <Text style={styles.deliveryMeta}>
        Day {delivery.sequenceNumber} · {delivery.status.replaceAll("_", " ")}
      </Text>
    </View>
    <Text style={styles.deliveryWindow}>{minuteTime(startMinute)}–{minuteTime(endMinute)}</Text>
    <Text style={[styles.cash, delivery.cashDuePaise > 0 && styles.cashDue]}>
      {delivery.cashDuePaise > 0
        ? `₹${(delivery.cashDuePaise / 100).toLocaleString("en-IN")}`
        : "₹0"}
    </Text>
  </View>
);
...
const styles = StyleSheet.create({
  screen: { flex: 1, backgroundColor: "#F4F8F6" },
  flex: { flex: 1 },
  ...
  fill: { height: "100%", backgroundColor: "#D4F7E9" },
  heroFacts: {
    flexDirection: "row",
    justifyContent: "space-between",
    marginTop: 12,
  ...
  modalBackdrop: {
    ...
    backgroundColor: "rgba(9,31,24,.45)",
  },
  modalDismiss: { flex: 1 },
  sheet: {
    maxHeight: "84%",
