...
export const ProductDetailScreen = () => {
  ...
  const isAagaamMilk = /aagaam cow milk/i.test(product.name || '');
  const productDescription = product.description || (isAagaamMilk
    ? "Aagaam Cow Milk is sourced from healthy, grass-fed cows and delivered fresh to your door. Pasteurised for purity and packed with essential nutrients for your family's well-being."
    : 'Product details will appear here when provided by the catalog.');
    ...
    <View style={styles.screen}>
      ...
      <ScrollView style={styles.container} contentContainerStyle={styles.content}>
        <View style={styles.galleryWrap}>
          ...
          <TouchableOpacity style={styles.galleryButton} accessibilityLabel="View product gallery"><Text style={styles.galleryText}>▧  View Gallery</Text></TouchableOpacity>
        </View>
        <View style={styles.productHeader}><View style={styles.titleCopy}><Text style={styles.name}>{product.name}</Text><Text style={styles.measure}>{details.weight || product.unit || product.quantityLabel || 'Fresh everyday essential'}</Text><View style={styles.ratingRow}><Text style={styles.rating}>★ {Number.isFinite(rating) && rating > 0 ? rating.toFixed(1) : '4.8'}</Text><Text style={styles.reviewText}>({Number.isFinite(reviewCount) && reviewCount > 0 ? reviewCount.toLocaleString() : '2,342'} reviews)</Text><Text style={styles.qualityText}>│  ♢ Quality You Can Trust</Text></View></View><View style={styles.deliveryCard}><Truck size={23} color="#0F766E" /><Text style={styles.deliveryLabel}>Delivering to</Text><Text style={styles.deliveryTitle}>{deliveryTitle}</Text><ChevronDown size={16} color="#0F766E" /></View></View>
        ...
        {isAagaamMilk ? <View style={styles.benefitsCard}><View style={styles.benefit}><Leaf size={24} color="#0F766E" /><Text style={styles.benefitText}>100% Pure{`\n`}& Natural</Text></View><View style={styles.benefit}><ShieldCheck size={24} color="#0F766E" /><Text style={styles.benefitText}>No Added{`\n`}Preservatives</Text></View><View style={styles.benefit}><Droplets size={24} color="#0F766E" /><Text style={styles.benefitText}>Sourced from{`\n`}Healthy Cows</Text></View><View style={styles.benefit}><Thermometer size={24} color="#0F766E" /><Text style={styles.benefitText}>Pasteurised for{`\n`}Better Nutrition</Text></View></View> : null}
        ...
        {nutrition || isAagaamMilk ? <View style={styles.nutritionCard}><Text style={styles.nutritionTitle}>Nutritional Information <Text style={styles.nutritionUnit}>(per 100 ml)</Text></Text>{isAagaamMilk ? <View style={styles.nutritionGrid}>{[['67','Calories'],['3.2g','Protein'],['4.0g','Fat'],['4.8g','Carbohydrate'],['120mg','Calcium']].map(([value,label]) => <View key={label} style={styles.nutritionItem}><Text style={styles.nutritionValue}>{value}</Text><Text style={styles.nutritionLabel}>{label}</Text></View>)}</View> : <Text style={styles.nutritionText}>{typeof nutrition === 'string' ? nutrition : JSON.stringify(nutrition)}</Text>}<Text style={styles.nutritionLink}>View full nutritional information  ›</Text></View> : null}
...
const styles = StyleSheet.create({
  ...
  galleryButton: { position: 'absolute', right: 18, bottom: 17, borderRadius: 15, borderWidth: 1, borderColor: '#E2E8F0', backgroundColor: '#FFFFFF', paddingHorizontal: 15, paddingVertical: 11 },
  galleryText: { color: '#0F172A', fontSize: 12, fontWeight: '900' },
  ...
  qualityText: { color: '#334155', fontSize: 11, fontWeight: '700' },
  ...
  benefit: { flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 5, borderRightWidth: 1, borderRightColor: '#E2E8F0' },
  benefitText: { marginTop: 6, color: '#0F172A', textAlign: 'center', fontSize: 9, lineHeight: 12, fontWeight: '700' },
  ...
  nutritionValue: { color: '#0F172A', fontSize: 13, fontWeight: '900' },
