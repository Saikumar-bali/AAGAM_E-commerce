      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>Your order history will appear here after checkout.</Text>
          </View>
        }
        renderItem={({ item }) => (
          <TouchableOpacity
          ...
          >
            ...
            <Text style={styles.orderMeta}>{item.store?.name || 'Assigned Store'}</Text>
            <Text style={styles.orderMeta}>{new Date(item.createdAt).toLocaleString()}</Text>
            <View style={styles.orderFooter}>
              <Text style={styles.totalText}>₹{item.grandTotal ?? item.totalAmount}</Text>
