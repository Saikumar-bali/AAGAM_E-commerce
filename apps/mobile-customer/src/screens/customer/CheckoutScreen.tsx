...
export const CheckoutScreen = () => {
  ...
  const { data: quote, isLoading: loadingQuote, refetch: refetchQuote } = useQuery({
    queryKey: ['quote', itemsPayload, selectedAddressId],
    queryFn: async () => {
      const response = await apiClient.post('/checkout/quote', { items: itemsPayload, addressId: selectedAddressId });
  ...
  const placeOrderMutation = useMutation({
    mutationFn: async () =>
      apiClient.post('/checkout/place-order', { items: itemsPayload, addressId: selectedAddressId, paymentMethod }),
    onSuccess: (response) => {
      clearCart();
      const orderId = response.data?.id;
      Alert.alert(
        'Order placed',
        paymentMethod === 'ONLINE' ? 'Your order is waiting for payment confirmation.' : 'Your COD order has been confirmed.',
          ...
          return (
            ...
            >
              <Text style={[styles.paymentLabel, active && styles.paymentLabelActive]}>
                {option === 'COD' ? 'Cash on Delivery' : 'Pay Online'}
              </Text>
              <Text style={styles.paymentMeta}>
                {option === 'COD' ? 'Pay when the order arrives' : 'Simulated payment capture'}
              </Text>
            </TouchableOpacity>
          );
        })}
      ...
      <View style={styles.summaryCard}>
        {items.map((item) => (
          <View key={item.product.id} style={styles.summaryRow}>
            <Text style={styles.summaryText}>{item.product.name} x {item.quantity}</Text>
        ...
        <View style={styles.summaryRow}>
          ...
          <Text style={styles.summaryText}>₹{quote?.invoice?.subtotal ?? total()}</Text>
        </View>
        <View style={styles.summaryRow}>
          <Text style={styles.summaryText}>Delivery Fee</Text>
          <Text style={styles.summaryText}>₹{quote?.invoice?.deliveryFee ?? 0}</Text>
        </View>
        <View style={styles.summaryRow}>
          <Text style={styles.totalLabel}>Grand Total</Text>
          <Text style={styles.totalValue}>₹{quote?.invoice?.grandTotal ?? total()}</Text>
        </View>
        {quote && quote.serviceable === false ? (
          <Text style={styles.errorText}>This address is currently outside the delivery radius.</Text>
        ) : null}
      </View>

      <TouchableOpacity
        style={[styles.placeOrderButton, (!selectedAddressId || placeOrderMutation.isPending || quote?.serviceable === false) && styles.placeOrderButtonDisabled]}
        onPress={() => placeOrderMutation.mutate()}
        disabled={!selectedAddressId || placeOrderMutation.isPending || quote?.serviceable === false}
      >
        {loadingQuote || placeOrderMutation.isPending ? (
          <ActivityIndicator color="#FFFFFF" />
        ) : (
          <Text style={styles.placeOrderText}>{paymentMethod === 'COD' ? 'Place COD Order' : 'Continue to Pay'}</Text>
        )}
      </TouchableOpacity>
    </ScrollView>
  );
