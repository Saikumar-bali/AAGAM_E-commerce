...
export const CheckoutScreen = () => {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <View style={styles.couponHeadingRow}><Text style={styles.sectionTitle}>Coupon</Text><TouchableOpacity style={styles.viewDeals} onPress={() => navigation.navigate('Deals')}><Tag size={14} color="#0F766E" /><Text style={styles.viewDealsText}>Browse deals</Text></TouchableOpacity></View>
      {appliedCoupon ? <View style={styles.appliedCard}><View style={styles.appliedCopy}><BadgePercent size={20} color="#0F766E" /><View style={styles.appliedTextWrap}><Text style={styles.appliedTitle}>{appliedCoupon.name || appliedCoupon.code} applied</Text><Text style={styles.appliedText}>You save ₹{Number(appliedCoupon.discountAmount || discountAmount)}</Text></View></View>{appliedCoupon.applicationMode === 'CODE' ? <TouchableOpacity onPress={removeCoupon} accessibilityLabel="Remove coupon"><X size={20} color="#475569" /></TouchableOpacity> : <Text style={styles.autoBadge}>AUTO</Text>}</View> : <View style={styles.couponRow}><TextInput value={couponInput} onChangeText={(value) => { setCouponInput(value.toUpperCase()); setCouponError(''); }} onSubmitEditing={() => void applyCoupon()} placeholder="Enter coupon code" placeholderTextColor="#94A3B8" autoCapitalize="characters" style={styles.couponInput} /><TouchableOpacity disabled={couponApplying} style={[styles.applyButton, couponApplying && styles.placeOrderButtonDisabled]} onPress={() => void applyCoupon()}>{couponApplying ? <ActivityIndicator color="#FFFFFF" /> : <Text style={styles.applyButtonText}>Apply</Text>}</TouchableOpacity></View>}
