...
export default function App() {
  ...
  const handleLogin = async () => {
    ...
    setLoading(true);
    try {
      // For RN CLI on Android Emulator, localhost is 10.0.2.2
      const response = await apiClient.post('/auth/login', { email, password });
      
      // SAVE TOKEN SECURELY (RN CLI version)
      await SInfo.setItem('user_token', response.data.session.access_token, {
      ...
      console.error(error);
      const message = error.response?.data?.message || 'Login failed. Ensure backend is running.';
