...
test.describe('Live Cloudflare Pages - Login & Role Switch', () => {
  ...
  test('full role switch flow - login admin then shop then rider', async ({ page }) => {
    ...
    await page.goto(`${LIVE_SITE}/login`);
    await page.waitForSelector('input[autocomplete="username"]', { timeout: 15000 });
    await page.fill('input[autocomplete="username"]', process.env.LOGIN_TEST_ADMIN_EMAIL ?? '');
    ...
    await page.goto(`${LIVE_SITE}/login`);
    await page.waitForSelector('input[autocomplete="username"]', { timeout: 15000 });
    await page.fill('input[autocomplete="username"]', process.env.LOGIN_TEST_CUSTOMER_EMAIL ?? '');
