...
The Firebase JSON must include non-empty `project_id`, `client_email`, and `private_key` fields. The dedicated protected credential is authoritative: deployment writes its `project_id` as the final `FIREBASE_PROJECT_ID` value, then validates both values together.
...
When one of the dedicated Firebase secrets is configured, deployment safely appends final `FIREBASE_PROJECT_ID` and `FIREBASE_SERVICE_ACCOUNT_JSON` assignments. These override stale embedded Firebase values without printing the credential. When no dedicated secret is configured, embedded Firebase values must already match and pass validation.
...
2. Overlays the protected Firebase credential and its authoritative project ID when configured.
