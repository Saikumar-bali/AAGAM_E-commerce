...
REPLACEMENTS = {
    'apps/admin-dashboard/src/lib/pushNotifications.ts': [("'AAGAM update'", "'AAGAAM update'")],
    'apps/admin-dashboard/src/app/(admin)/admin/stores/page.tsx': [('Aagam Commerce Operations', 'Aagaam Commerce Operations')],
    ...
    'apps/admin-dashboard/src/app/(shop)/shop/orders/[id]/page.tsx': [('ordering with Aagam.', 'ordering with Aagaam.')],
    'apps/admin-dashboard/src/app/(shop)/shop/delivery-code/[deliveryJobId]/page.tsx': [('AAGAM staff should never ask', 'AAGAAM staff should never ask')],
    'apps/admin-dashboard/src/components/Sidebar.tsx': [('"Aagam Customer"', '"Aagaam Customer"')],
    'apps/mobile-customer/src/ui/CustomerToast.tsx': [("text1 || 'AAGAM'", "text1 || 'AAGAAM'")],
    'apps/mobile-customer/src/screens/customer/CheckoutScreen.tsx': [('AAGAM uses precise location', 'Aagaam uses precise location')],
    'apps/mobile-customer/src/components/promotions/PromotionCarousel.tsx': [('Published offers from Aagam', 'Published offers from Aagaam')],
    'apps/mobile-partners/src/onboarding/applicationReviewProgress.ts': [('Submitted to AAGAM', 'Submitted to AAGAAM')],
    'apps/mobile-partners/src/screens/PartnerVerificationScreen.tsx': [('AAGAM never asks you', 'AAGAAM never asks you')],
    'apps/mobile-partners/src/screens/PartnerActivationScreen.tsx': [('AAGAM Admin cannot view', 'AAGAAM Admin cannot view')],
    ...
    'apps/mobile-partners/src/screens/PartnerDocumentsScreen.tsx': [('ready for AAGAM review', 'ready for Aagaam review')],
    ...
    'apps/mobile-partners/src/screens/rider/RiderPickupOperationsScreen.tsx': [("'AAGAM store'", "'Aagaam store'")],
    'apps/mobile-partners/src/screens/PartnerApplicationStartScreen.tsx': [('primary AAGAM login', 'primary Aagaam login')],
...
for filename, pairs in REPLACEMENTS.items():
    ...
    for old, new in pairs:
        if old in updated:
            ...
            raise SystemExit(f'Neither old nor expected new controlled string found in {filename}: {old}')
    if updated != source:
...
'''
Path('scripts/aagaam-visible-brand.contract.test.js').write_text(audit)
