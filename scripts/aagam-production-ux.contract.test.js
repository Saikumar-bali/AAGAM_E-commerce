...
contains(map, '.setView(center, 16)', 'Tracking map must use a close-range default zoom.');
...
contains(application, 'OPERATIONS_CHANNEL_ID = "aagam_priority_operations_v3"', 'Partner alerts must retain the internal AAGAM namespace with a versioned sound profile.');
contains(application, '"Aagaam urgent operations"', 'The visible Android channel name must use the Aagaam brand.');
...
contains(application, 'RingtoneManager.TYPE_NOTIFICATION', 'Background partner alerts must use notification audio rather than the phone alarm sound.');
excludes(application, 'RingtoneManager.TYPE_ALARM', 'Partner alerts must not use the device alarm clock tone.');
...
excludes(foregroundTone, 'TYPE_ALARM', 'Foreground partner notifications must not use the phone alarm tone.');
